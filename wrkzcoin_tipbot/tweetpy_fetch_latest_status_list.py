        ...
        try:
            ...
            async with store.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    await cur.execute(
                        sql,
                        (rt_by_uids, rt_counts, int(time.time()), tweet_link, guild_id),
                    )
                    if data_rows:
    ...
    while True:
        ...
        try:
            ...
            if get_list and len(get_list) > 0:
                try:
                    list_ids = [
                        int(each["tweet_link"].split("/")[-1]) for each in get_list
                    ]
                    existing_list = {}
                    twitter_status_id = {}
                    expired_date = {}
                    for each_one in get_list:
                        ...
                        twitter_status_id[
                            each_one["tweet_link"].split("/")[-1]
                        ] = each_one["tweet_link"]
                        expired_date[each_one["tweet_link"].split("/")[-1]] = each_one[
                            "expired_date"
                        ]
                    ...
                    if fetch_st and len(fetch_st) > 0:
                        for each_status in fetch_st:
                            try:
                                ...
                                if (
                                    each_status_json
                                    and each_status_json["retweet_count"] > 0
                                ):
