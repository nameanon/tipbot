            ...
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if result and len(result) > 0:
                        if len(result) > 15:
                            msg = "[TWITTER] - has {} tips messages to respond.".format(
                                len(result)
                            )
                            logchanbot(msg)
                        for each_msg in result:
                            try:
                                ...
                                if like is not None:
                                    await update_like(
                                        each_msg["twitter_id"], json.dumps(like)
                                    )
                                    msg = "[TWITTER] - Liked tweet ID {} after tipping.".format(
                                        each_msg["twitter_id"]
                                    )
                                    logchanbot(msg)
                                else:
                                    msg = "[TWITTER] - failed to like {} .".format(
                                        each_msg["twitter_id"]
                                    )
                                    logchanbot(msg)
