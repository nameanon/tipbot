...
class Coininfo(commands.Cog):
    ...
    async def get_coininfo(
        ...
        if (
            ...
            and len(getattr(getattr(self.bot.coin_list, coin_name), "contract")) > 4
        ):
        ...
        try:
            ...
            response_text += (
                "Tipping / Depositing / Withdraw:\n   {} / {} / {}\n".format(
                    tip_deposit_withdraw_stat[0],
                    tip_deposit_withdraw_stat[1],
                    tip_deposit_withdraw_stat[2],
                )
            )
            get_tip_min_max = (
                "Tip Min/Max:\n   "
                + num_format_coin(min_tip)
                + " / "
                + num_format_coin(max_tip)
                + " "
                + coin_name
            )
            ...
            if coin_name in self.bot.cexswap_coins:
                ...
                cexswap_min = getattr(
                    getattr(self.bot.coin_list, coin_name), "cexswap_min"
                )
            ...
            get_tx_min_max = (
                "Withdraw Min/Max:\n   "
                + num_format_coin(min_tx)
                + " / "
                + num_format_coin(max_tx)
                + " "
                + coin_name
            )
            ...
            if (
                getattr(
                    getattr(self.bot.coin_list, coin_name), "withdraw_use_gas_ticker"
                )
                == 1
            ):
                ...
                if GAS_COIN and fee_limit > 0:
                    gas_coin_msg = (
                        " and a reserved {} {} (actual tx fee is less).".format(
                            fee_limit, GAS_COIN
                        )
                    )
            if coin_name == "ADA":
                response_text += "Withdraw Tx reserved Node Fee: {} {} (actual tx fee is less).\n".format(
                    num_format_coin(fee_tx), coin_name
                )
            else:
                response_text += "Withdraw Tx Node Fee: {} {}{}\n".format(
                    num_format_coin(fee_tx), coin_name, gas_coin_msg
                )
            if deposit_fee > 0:
                response_text += "Deposit Tx Fee: {} {}\n".format(
                    num_format_coin(deposit_fee), coin_name
                )
                ...
                if per_unit and per_unit["price"] and per_unit["price"] > 0:
                    ...
                    if per_unit >= 0.0001:
                        ...
                        response_text += "Price 1 {} {} based on {}\n".format(
                            coin_name, equivalent_usd, price_with.lower()
                        )
        except Exception:
            traceback.print_exc(file=sys.stdout)
        response_text += "```"
        ...
        if (
            ...
            and len(getattr(getattr(self.bot.coin_list, coin_name), "explorer_link"))
            > 0
        ):
            other_links.append(
                "[{}]({})".format(
        ...
        if getattr(getattr(self.bot.coin_list, coin_name), "id_gecko"):
            other_links.append(
                "[{}]({})".format(
                    ...
                    + getattr(getattr(self.bot.coin_list, coin_name), "id_gecko")
                    + ">",
                )
            )
        if getattr(getattr(self.bot.coin_list, coin_name), "id_paprika"):
            other_links.append(
                "[{}]({})".format(
                    ...
                    + getattr(getattr(self.bot.coin_list, coin_name), "id_paprika")
                    + ">",
                )
            )
        if getattr(getattr(self.bot.coin_list, coin_name), "repo_link"):
            other_links.append(
                "[{}]({})".format(
                    "Repo",
                    "<"
                    + getattr(getattr(self.bot.coin_list, coin_name), "repo_link")
                    + ">",
                )
            )
    ...
    async def coininfo(self, ctx, coin: str):
    ...
    async def async_coinlist(self, ctx):
        ...
        if self.bot.coin_name_list and len(self.bot.coin_name_list) > 0:
            ...
            for k, v in network.items():
                ...
                if k != "Others":
                    embed.add_field(
                        name=f"Network: {k}", value=f"{list_coins}", inline=False
                    )
            # Add Other last
            ...
            embed.set_footer(
                text=f"Requested by {ctx.author.name}#{ctx.author.discriminator} | Total: {str(len(self.bot.coin_name_list))} "
            )
            await ctx.edit_original_message(content=None, embed=embed)
        else:
            await ctx.edit_original_message(
                content=f"{ctx.author.mention}, error loading. check back later."
            )

    @commands.slash_command(
        usage="coinlist", description="List of all coins supported by TipBot."
    )
