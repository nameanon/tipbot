...
class FreeTip_Verify(disnake.ui.Modal):
    def __init__(self, ctx, bot, question, answer, from_user_id, message_id) -> None:
        ...
        super().__init__(
            title=f"FreeTip Verification! {question}",
            custom_id="modal_freetip_verify",
    ...
    async def callback(self, interaction: disnake.ModalInteraction) -> None:
        ...
        await interaction.response.send_message(
            content=f"{interaction.author.mention}, verification starts...",
            ephemeral=True,
        )
        ...
        if answer == "":
            await interaction.edit_original_message(
                f"{interaction.author.mention}, answer is empty!"
            )
            return
        try:
            if int(answer) != self.answer:
                await interaction.edit_original_message(
                    f"{interaction.author.mention}, incorrect answer!"
                )
                return
                ...
                try:
                    ...
                    if _msg:
                        ...
                        embed.clear_fields()
                        get_freetip = await store.get_discord_freetip_by_msgid(
                            str(self.message_id)
                        )
                        if get_freetip is not None:
                            ...
                            indiv_amount = (
                                num_format_coin(truncate(amount / len(collectors), 12))
                                if len(collectors) > 0
                                else num_format_coin(truncate(amount, 12))
                            )
                            if (
                                get_freetip["airdrop_content"]
                                and len(get_freetip["airdrop_content"]) > 0
                            ):
                                embed.add_field(
                                    name="Comment",
                            ...
                            if len(collectors) > 0:
                                ...
                                embed.add_field(
                                    name="Attendees", value="N/A", inline=False
                                )
                            embed.add_field(
                                name="Num. Attendees",
                            ...
                            try:
                                time_left = get_freetip["airdrop_time"] - int(
                                    time.time()
                                )
                                owner_displayname = ""
                                get_owner = self.bot.get_user(
                                    int(get_freetip["from_userid"])
                                )
                                if get_owner:
                ...
                try:
                    ...
                    traceback.print_exc(file=sys.stdout)
                msg = "Sucessfully joined airdrop id: {}".format(
                    str(interaction.message.id)
                )
...
class FreeTip_Button(disnake.ui.View):
    ...
    @disnake.ui.button(
        label="🎁 Collect", style=ButtonStyle.green, custom_id="collect_freetip"
    )
    async def row_enter_airdrop(
        ...
        try:
            if self.owner_id == interaction.author.id:
                await interaction.response.send_message(
                    content=f"{interaction.author.mention}, you can not join your own /freetip ...",
                    ephemeral=True,
                )
                return
            ...
            try:
                ...
                get_message = await store.get_discord_freetip_by_msgid(
                    str(original_message.id)
                )

            if get_message is None:
                await interaction.response.send_message(
                    content=f"{interaction.author.mention}, failed to collect /freetip!",
                    ephemeral=True,
                )
                await logchanbot(
            ...
            if check_if_in:
                await interaction.response.send_message(
                    content=f"{interaction.author.mention}, you're already in this /freetip!",
                    ephemeral=True,
                )
                return
            else:
                ...
                if int(time.time()) > get_message["airdrop_time"]:
                    await interaction.response.send_message(
                        content=f"{interaction.author.mention}, this /freetip already passed!",
                        ephemeral=True,
                    )
                    return
                else:
                    ...
                    if self.verify == "ON":
                        try:
                            ...
                            answer = a + b
                            # nofity freetip owner
                            freetip_owner = self.bot.get_user(
                                int(get_message["from_userid"])
                            )
                            try:
                                freetip_link = (
                            ...
                            modal_inter: disnake.ModalInteraction = (
                                await self.bot.wait_for(
                                    "modal_submit",
                                    check=lambda i: i.custom_id
                                    == "modal_freetip_verify"
                            ...
                            await interaction.response.send_message(
                                f"{interaction.author.mention}, timeout to join /freetip!",
                                ephemeral=True,
                            )
                            return
                    else:
...
class Tips(commands.Cog):
    ...
    async def freetip_check(self):
        ...
        get_inactive_freetip = await store.get_inactive_discord_freetip(
            lap=7 * 24 * 3600
        )
            ...
            for each_message_data in get_active_freetip:
                ...
                try:
                    ...
                    if guild is None and self.bot.is_ready():
                        print(
                            "Guild {} found None".format(each_message_data["guild_id"])
                        )
                        await logchanbot(
                            "[FREETIP]: can not find guild for message ID: {} of channel {} in guild: {} by {}. Set that to FAILED.".format(
                        ...
                        change_status = await store.discord_freetip_update(
                            each_message_data["message_id"], "FAILED"
                        )
                    ...
                    if channel is None:
                        ...
                        find_owner = self.bot.get_user(
                            int(each_message_data["from_userid"])
                        )
                        if find_owner is not None:
                            await find_owner.send(
                                "I cancelled your /freetip id: {} in guild {} because I can't find channel.".format(
                    ...
                    if get_owner is None:
                        ...
                        print(
                            "Airdroper None {}".format(each_message_data["from_userid"])
                        )
                        await asyncio.sleep(2.0)
                        get_owner = guild.get_member(
                            int(each_message_data["from_userid"])
                        )
                    if get_owner:
                        owner_displayname = (
                            f" by {get_owner.name}#{get_owner.discriminator}"
                        )
                    ...
                    if "fetched_msg" not in self.bot.other_data:
                        ...
                        if (
                            ...
                            last_fetched = self.bot.other_data["fetched_msg"][
                                each_message_data["message_id"]
                            ]
                            if (
                    ...
                    _msg: disnake.Message = await channel.fetch_message(
                        int(each_message_data["message_id"])
                    )
                    if _msg:
                        # check if coin exist?
                        ...
                        view = FreeTip_Button(
                            self.bot, verify, int(each_message_data["from_userid"])
                        )
                        try:
                            embed = _msg.embeds[0]  # embeds is list, we take 0
                            embed.clear_fields()
                            ...
                            if (
                                each_message_data["airdrop_content"]
                                and len(each_message_data["airdrop_content"]) > 0
                            ):
                                embed.add_field(
                                    name="Comment",
                            ...
                            if time_left > 0:
                                if (
                                    "fetched_msg" in self.bot.other_data
                                    and each_message_data["message_id"]
                                    in self.bot.other_data["fetched_msg"]
                                ):
                                    last_fetched = self.bot.other_data["fetched_msg"][
                                        each_message_data["message_id"]
                                    ...
                                    if int(time.time()) - last_fetched < 30:
                                        ...
                                        continue
                                if (
                                    int(time.time()) - int(_msg.edited_at.timestamp())
                                    > 30
                                ):
                                    embed.set_footer(
                                        text=f"FreeTip {owner_displayname}, Time Left: {seconds_str_days(time_left)}"
                                    ...
                                    try:
                                        ...
                                        change_status = (
                                            await store.discord_freetip_update(
                                                each_message_data["message_id"],
                                                "FAILED",
                                            )
                                        )
                                        ...
                                        if find_owner is not None:
                                            await find_owner.send(
                                                "I cancelled your /freetip id: {} in guild {} because I got no permission to edit embed message.".format(
                                                    each_message_data["message_id"],
                                                    guild.name,
                                                )
                                            )
                                ...
                                if each_message_data["status"] == "ONGOING":
                                    if len(collectors) == 0:
                                        embed.set_footer(
                                            text=f"FreeTip by {owner_displayname}, and no one collected!"
                                        )
                                        try:
                                        ...
                                        userdata_balance = (
                                            await store.sql_user_balance_single(
                                                each_message_data["from_userid"],
                                                coin_name,
                                                wallet_address,
                                                type_coin,
                                        ...
                                        actual_balance = float(
                                            userdata_balance["adjust"]
                                        )

                                        # We need only to check if balance > 0 his balance already pending.
                                        if actual_balance < 0:
                                            embed.set_footer(
                                                text=f"FreeTip by {owner_displayname}, failed!"
                                            )
                                            try:
                                                await _msg.edit(embed=embed, view=None)
                                                # update status
                                            ...
                                            # Multiple tip here
                                            amount_div = truncate(
                                                amount / len(collectors), 12
                                            )
                                            tipAmount = num_format_coin(amount)
                                            ActualSpend_str = num_format_coin(
                                                amount_div * len(collectors)
                                            )
                                            ...
                                            if price_with:
                                                per_unit = each_message_data[
                                                    "unit_price_usd"
                                                ]
                                                if per_unit and per_unit > 0:
                                                    amount_in_usd = per_unit * float(
                                                        amount_div
                                                    )
                                            ...
                                            try:
                                                ...
                                                embed.set_footer(
                                                    text=f"Completed! Collected by {len(collectors)} member(s)"
                                                )
                                                await _msg.edit(embed=embed, view=None)
                                                if tips:
                                                    link_to_msg = "https://discord.com/channels/{}/{}/{}".format(
                                                    ...
                                                    try:
                                                        ...
                                                        msg_freetip = (
                                                            f"{EMOJI_ARROW_RIGHTHOOK} FreeTip of {coin_emoji}{tipAmount} {token_display} "
                    ...
                    await logchanbot(
                        "[FREETIP]: can not find message ID: {} of channel {} in guild: {} by {}. Set that to FAILED.".format(
                            each_message_data["message_id"],
                            each_message_data["channel_id"],
                            each_message_data["guild_id"],
                            each_message_data["from_ownername"],
                    ...
                    await asyncio.sleep(0.5)
                    await logchanbot(
                        "[FREETIP]: I have no permission for message ID: {} of channel {} in guild: {} by {}. Set that to FAILED.".format(
                            each_message_data["message_id"],
                            each_message_data["channel_id"],
                            each_message_data["guild_id"],
                            each_message_data["from_ownername"],
                    ...
                    if get_member is not None:
                        await get_member.send(
                            "/freetip failed: I have no permission to get message: {} of channel {} in guild: {}.".format(
                                each_message_data["message_id"],
                                each_message_data["channel_id"],
                                each_message_data["guild_id"],
                            )
    ...
    @commands.slash_command(
        ...
        options=[Option("onoff", "onoff", OptionType.string, required=True)],
    ...
    # RandomTip
    async def async_randtip(
        self, ctx, amount: str, token: str, rand_option: str = None
    ):
        ...
        if (
            ...
            msg = (
                f"{ctx.author.mention}, **{coin_name}** is not allowed here. Currently, allowed __{allowed_coins}__. "
        ...
        try:
            ...
            if len(coin_emoji) > 0:
                await ctx.edit_original_message(
                    content=f"{EMOJI_INFORMATION} {ctx.author.mention}, executing /randtip with {coin_emoji}..."
                )
        except Exception:
            traceback.print_exc(file=sys.stdout)
        ...
        try:
            ...
            if rand_option is None or rand_option.upper().startswith("ALL"):
                listMembers = [
                    member for member in ctx.guild.members if member.bot is False
                ]
            elif rand_option and rand_option.upper().startswith("ONLINE"):
                listMembers = [
                    member
                    for member in ctx.guild.members
                    if member.bot is False and member.status != disnake.Status.offline
                ]
                ...
                if len(argument) == 2:
                    ...
                    if (
                        "u" in num_user
                        or "user" in num_user
                        or "users" in num_user
                        or "person" in num_user
                        or "people" in num_user
                    ):
                        ...
                        try:
                            ...
                            if num_user < minimum_users:
                                msg = (
                                    f"{EMOJI_RED_NO} {ctx.author.mention}, number of random users cannot "
                                ...
                                if len(message_talker) < minimum_users:
                                    ...
                                    try:
                                        msg = (
                                            f"{EMOJI_INFORMATION} {ctx.author.mention} I could not find sufficient talkers up "
                            ...
                            msg = (
                                f"{EMOJI_RED_NO} {ctx.author.mention}, invalid param after **LAST** for random tip. "
            ...
            if has_last is False and listMembers and len(listMembers) >= minimum_users:
                ...
                if rand_user == ctx.author:
                    while True:
                        ...
                        if max_loop >= 5:
                            msg = (
                                f"{EMOJI_RED_NO} {ctx.author.mention} {token_display}, "
                ...
                has_last is True
                and message_talker
                and len(message_talker) >= minimum_users
            ):
                ...
                if rand_user_id == ctx.author.id:
                    while True:
                        ...
                        if (
                            rand_user
                            and rand_user != ctx.author
                            and rand_user in ctx.guild.members
                        ):
                            break
                        else:
                            rand_user_id = random.choice(message_talker)
        ...
        if amount > max_tip or amount < min_tip:
            ...
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, insufficient balance to do a random tip of "
        ...
        if tip:
            ...
            try:
                msg = (
                    f"{EMOJI_ARROW_RIGHTHOOK} {rand_user.name}#{rand_user.discriminator} got your random tip of "
    ...
    async def randtip(self, ctx, amount: str, token: str, rand_option: str = None):
        try:
    ...
    async def randtip_token_name_autocomp(
        ...
        try:
            ...
            try:
                ...
                if (
                    ...
                    msg = (
                        f"{EMOJI_INFORMATION} {ctx.author.mention}, there are still some ongoing drops or tips in this guild. "
                        "Please wait for them to complete first!"
                    )
                    await ctx.edit_original_message(content=msg)
                    return
                    ...
                    await ctx.edit_original_message(content=msg)
                    await logchanbot(
                        f"[FREETIP] server {str(ctx.guild.id)} has no data in discord_server."
                    )
                    return
            except Exception:
                traceback.print_exc(file=sys.stdout)
            ...
            if amount > max_tip or amount < min_tip:
                ...
                msg = (
                    f"{EMOJI_RED_NO} {ctx.author.mention}, insufficient balance to do a free tip of "
                    f"**{num_format_coin(amount)} {token_display}**."
                )
                await ctx.edit_original_message(content=msg)
                return
            ...
            embed = disnake.Embed(
                ...
                timestamp=datetime.fromtimestamp(int(time.time()) + duration),
            )
            try:
                if comment and len(comment) > 0:
                    embed.add_field(name="Comment", value=comment, inline=True)
                embed.add_field(
                    name="Attendees", value="Click to collect!", inline=False
                )
                embed.add_field(
                ...
                embed.add_field(
                    name="Num. Attendees", value="**0** members", inline=True
                )
                embed.set_footer(
    ...
    async def freetip_token_name_autocomp(
        self, inter: disnake.CommandInteraction, string: str
    ):
        string = string.lower()
        return [name for name in self.bot.coin_name_list if string in name.lower()][:10]
    ...
    async def async_tipall(self, ctx, amount: str, token: str, ping: str, user: str):
        ...
        if amount > max_tip or amount < min_tip:
            ...
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, insufficient balance to tip of "
                f"**{num_format_coin(amount)} {token_display}**."
            )
            await ctx.edit_original_message(content=msg)
            return
        ...
        if user.upper() == "ANY" or user.upper() == "ALL":
            ...
            listMembers = [
                member
                for member in ctx.guild.members
                if member.status != disnake.Status.offline and member.bot is False
            ]
        ...
        if len(listMembers) > max_allowed:
            msg = (
                f"{ctx.author.mention}, there are more than maximum allowed __{str(max_allowed)}__. "
        ...
        if amount / len(memids) < min_tip:
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, transactions cannot be smaller "
        ...
        if tips:
            ...
            for member in listMembers:
                if send_tipped_ping >= self.bot.config["discord"]["maxTipAllMessage"]:
                    ...
                    if ctx.author.id != member.id and member.id != self.bot.user.id:
                        if mute_tip is True:
                            list_user_not_mention.append(
                                "{}#{}".format(member.name, member.discriminator)
                            )
                        elif (
                            mention_all is True and str(member.id) not in notifying_list
                        ):
                    ...
                    if numb_mention > 0 and numb_mention % max_mention == 0:
                        ...
                        try:
                            if len(mention_users) > 0:
                                msg = (
                                    f"{EMOJI_MONEYFACE} {mention_users}, "
            ...
            if len(list_user_mention) + len(list_user_not_mention) >= 1:
                ...
                try:
                    ...
                    if numb_mention < total_found:
                        remaining_str = " and other {} members".format(
                            total_found - numb_mention
                        )
                    msg = (
                        f"{EMOJI_MONEYFACE} {mention_users}{remaining_str}, "
                        f"you got a tip of {coin_emoji}**{amountDiv_str} {token_display}** {equivalent_usd} from "
                        f"{ctx.author.name}#{ctx.author.discriminator}\n{extra_msg}"
                    )
                    await ctx.followup.send(msg)
            ...
            try:
                msg = (
                    f"{EMOJI_ARROW_RIGHTHOOK}, you tip {coin_emoji}{tipAmount} {token_display} to ({len(memids)}) members "
    ...
    @commands.slash_command(
        ...
        options=[
            ...
            Option(
                "user",
                "user option (ONLINE or ALL)",
                OptionType.string,
                required=False,
                choices=[
                    OptionChoice("ONLINE", "ONLINE"),
                    OptionChoice(
                        "ONLINE EXCEPT FOR NO NOTIFICATION",
                        "ONLINE_EXCEPT_NO_NOTIFICATION",
                    ),
                    OptionChoice(
                        "ALL EXCEPT FOR NO NOTIFICATION", "ALL_EXCEPT_NO_NOTIFICATION"
                    ),
    ...
    async def tipall(
        self, ctx, amount: str, token: str, ping: str = "YES", user: str = "ONLINE"
    ):
        try:
    ...
    async def tipall_token_name_autocomp(
        self, inter: disnake.CommandInteraction, string: str
    ):
        string = string.lower()
        return [name for name in self.bot.coin_name_list if string in name.lower()][:10]
    ...
    # Tip Normal
    async def async_tip(
        self, ctx, amount: str, token: str, args, split: str, ping: str
    ):
        ...
        if len(get_list_member_n_role) > 0:
            get_list_member_n_role = [
                ...
                for each in get_list_member_n_role
            ]
                    ...
                    if (
                        ...
                        try:
                            ...
                            # Check if we really have that many user in the guild 20%
                                ...
                                try:
                                    msg = (
                                        f"{ctx.author.mention}, you want to tip more than the number of people in this guild!? "
                                ...
                                if len(message_talker) == 0:
                                    ...
                                    len(message_talker) < len(ctx.guild.members) - 1
                                ):  # minus bot
                                    msg = (
                                        f"{EMOJI_INFORMATION} {ctx.author.mention}, I could not find sufficient talkers up to **{num_user}**. "
                                ...
                                if len(message_talker) == 0:
                                    ...
                                    try:
                                        msg = (
                                            f"{EMOJI_INFORMATION} {ctx.author.mention}, I could not find sufficient talkers up to "
                                ...
                                msg = (
                                    f"{EMOJI_RED_NO} {ctx.author.mention}, what is this **{num_user}** number? "
                        ...
                        try:
                            time_string = (
                                time_string.replace("years", "y")
                                .replace("yrs", "y")
                                .replace("yr", "y")
                                .replace("year", "y")
                            )
                            time_string = (
                                time_string.replace("months", "mon")
                                .replace("month", "mon")
                                .replace("mons", "mon")
                                .replace("weeks", "w")
                                .replace("week", "w")
                            ...
                            time_string = (
                                time_string.replace("minutes", "mn")
                                .replace("mns", "mn")
                                .replace("mins", "mn")
                                .replace("min", "mn")
                            )
                            ...
                            await logchanbot("tips " + str(traceback.format_exc()))
                            msg = (
                                f"{EMOJI_RED_NO} {ctx.author.mention}, invalid time given. "
    ...
    @commands.slash_command(
        ...
        options=[
            ...
            Option(
                ...
                OptionType.string,
                required=False,
                choices=[OptionChoice("SPLIT", "SPLIT"), OptionChoice("EACH", "EACH")],
            ),
            Option(
                "ping",
                "mention or no mention",
    ...
    async def tip_token_name_autocomp(
        self, inter: disnake.CommandInteraction, string: str
    ):
        string = string.lower()
        return [name for name in self.bot.coin_name_list if string in name.lower()][:10]
    ...
    async def async_gtip(self, ctx, amount: str, token: str, args, split: str):
        coin_name = token.upper()
        msg = (
            f"{EMOJI_INFORMATION} {ctx.author.mention}, executing guild tip command..."
        )
        await ctx.response.send_message(msg)
    ...
    async def guildtip(
        self, ctx, amount: str, token: str, args: str, split: str = "EACH"
    ):
        try:
    ...
    async def guildtip_token_name_autocomp(
        self, inter: disnake.CommandInteraction, string: str
    ):
        string = string.lower()
        return [name for name in self.bot.coin_name_list if string in name.lower()][:10]
    ...
    async def z(self, ctx, amount_coin_list_to: str):
        try:
    ...
    async def async_ztip(self, ctx, amount_list_to):
        ...
        try:
            msg = (
                f"{EMOJI_INFORMATION} {ctx.author.mention}, executing z tip command..."
            )
            await ctx.response.send_message(msg)
            try:
                self.bot.commandings.append(
            ...
            # check lock
            try:
                is_user_locked = self.utils.is_locked_user(
            ...
            list_member_ids = []
            if (
                "@everyone" in amount_list_to.lower()
                or "@here" in amount_list_to.lower()
            ):
                list_member_ids = [
                    str(member.id)
                    for member in ctx.guild.members
                ...
                if len(get_list_member_n_role) > 0:
                    ...
                    if len(get_list_member_n_role) > 0:
                        for each_r in get_list_member_n_role:
                            try:
                                # get list users in role
                                get_role = disnake.utils.get(
                                    ctx.guild.roles, id=int(each_r)
                                )
                                role_list_members = [
                                    str(member.id)
                                    for member in ctx.guild.members
            ...
            if len(list_member_ids) == 0 and amount_list_to.lower().endswith(
                ("all online", "online")
            ):
                list_member_ids = [
                    str(member.id)
            ...
            elif len(list_member_ids) == 0 and amount_list_to.lower().endswith(
                ("all offline", "offline")
            ):
                list_member_ids = [
                    str(member.id)
            ...
            if len(list_member_ids) == 0:
                await ctx.edit_original_message(
                    content=f"{ctx.author.mention}, there is no one to tip to."
                )
                return
                ...
                if len(each_token) > 0:
                    ...
                    if len(split_amount_token) >= 2:
                        ...
                        if hasattr(self.bot.coin_list, coin_name):
                            if (
                                getattr(
                                    getattr(self.bot.coin_list, coin_name), "enable_tip"
                                )
                                != 1
                            ):
                            ...
                            try:
                                if (
                                    ...
                                    and serverinfo["tiponly"] != "ALLCOIN"
                                    and coin_name
                                    not in serverinfo["tiponly"].split(",")
                                ):
                                    allowed_coins = serverinfo["tiponly"]
                                    error_msg = (
                                        f"**{coin_name}** is not allowed here. Currently, allowed __{allowed_coins}__. "
                                        f"You can ask guild owner to allow. `/SETTING TIPONLY coin1,coin2,...`"
                                    )
                                    has_amount_error = True
                                    break
                            ...
                            # If $ is in amount, let's convert to coin/token
                                ...
                                if amount is None:
                                    error_msg = (
                                        f"invalid given amount __{amount_original}__."
                                    )
                                    has_amount_error = True
                                    break
                            try:
                                ...
                                if coin_name in list_tokens:
                                    ...
                                    if amount < min_tip or amount > max_tip:
                                        error_msg = (
                                            f"tipping for {coin_name} cannot be smaller than "
                                            f"**{num_format_coin(min_tip)} {token_display}** "
            ...
            if has_amount_error is True:
                await ctx.edit_original_message(
                    content=f"{ctx.author.mention}, {error_msg}"
                )
                return
            else:
                if len(list_amount_and_tokens) == 0:
                    await ctx.edit_original_message(
                        content=f"{ctx.author.mention}, invalid amount or token."
                    )
                    return
                    ...
                    for k, v in list_amount_and_tokens.items():
                        try:
                            ...
                            passed_tips.append(
                                "{}{} {}".format(
                                    coin_emoji,
                                    num_format_coin(v * len(list_member_ids)),
                                    k,
                                )
                            )
                            each_tips.append(
                                "{}{} {}".format(coin_emoji, num_format_coin(v), k)
                            )
                        except Exception:
                            traceback.print_exc(file=sys.stdout)
                except Exception:
                ...
                if len(passed_tips) > 0:
                    ...
                    try:
                        if len(list_member_ids) > 20:
                            msg = (
                                f"{EMOJI_ARROW_RIGHTHOOK} tip of **{joined_tip_list}** was sent to ({str(len(list_member_ids))}) "
                            ...
                            for each_m in list_member_ids:
                                ...
                                if (
                                    mute_tip is False
                                    and ctx.author.id != int(each_m)
                                    and each_m not in notifying_list
                                ):
                                    ...
                                    incl_msg.append(each_user.mention)
                                if (
                                    ctx.author.id != int(each_m)
                                    and each_m in notifying_list
                                ):
                                    incl_msg.append(
                                        "{}#{}".format(
                                            each_user.name, each_user.discriminator
                                        )
                                    )
                            if len(incl_msg) > 0:
                                incl_msg_str = ", ".join(incl_msg)
                            msg = (
                                f"{EMOJI_ARROW_RIGHTHOOK} tip of **{joined_tip_list}** was sent to {incl_msg_str} "
                                f"in server __{ctx.guild.name}__."
                            )
                            if len(list_member_ids) > 1:
                                msg += f"\nEach member got: **{each_tips_list}{sum_in_usd_text}**\n"
                    ...
                    if 20 >= len(list_mentions) >= 1:
                        ...
                        for member in list_mentions:
                            ...
                            if (
                                ...
                                try:
                                    msg = (
                                        f"{EMOJI_MONEYFACE}, you got {tip_text} of **{each_tips_list}{sum_in_usd_text}** "
    ...
    async def multiple_tip(
        ...
        if_guild: bool = False,
        ping: bool = True,
        split_amount: bool = False,
    ):
        coin_name = coin.upper()
        "**{}**".format(ctx.guild.name) if if_guild else ""
        tip_type_text = "guild tip" if if_guild else "tip"
        "GUILDTIP" if if_guild else "TIPS"
        ...
        # Check if tx in progress
        if (
            id_tipper in self.bot.tipping_in_progress
            and int(time.time()) - self.bot.tipping_in_progress[id_tipper] < 150
        ):
        ...
        if amount < min_tip or amount > max_tip:
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, transactions cannot be smaller than "
                f"**{num_format_coin(min_tip)} {token_display}** "
                f"or bigger than **{num_format_coin(max_tip)} {token_display}**."
            )
            await ctx.edit_original_message(content=msg)
            ...
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, insufficient balance to send {tip_type_text} of "
                f"**{num_format_coin(amount)} {token_display}**."
            )
            await ctx.edit_original_message(content=msg)
            return
        ...
        if total_amount > max_tip:
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, total transaction cannot be bigger than "
        ...
        if tips:
            ...
            try:
                if len(memids) > 20:
                    msg = (
                        f"{EMOJI_ARROW_RIGHTHOOK} {tip_type_text} of {coin_emoji}**{tipAmount} {token_display}** {total_equivalent_usd} "
                    ...
                    for each_m in list_mentions:
                        if mute_tip is True:
                            incl_msg.append(
                                "{}#{}".format(each_m.name, each_m.discriminator)
                            )
                        elif (
                            ping is True
                    ...
                    if len(incl_msg) > 0:
                        incl_msg_str = ", ".join(incl_msg)
                    msg = (
                        f"{EMOJI_ARROW_RIGHTHOOK} {tip_type_text} of {coin_emoji}**{tipAmount} {token_display}** "
            ...
            if 20 >= len(list_mentions) >= 1 and ping is True and mute_tip is False:
                ...
                for member in list_mentions:
                    ...
                    if (
                        ...
                        try:
                            msg = (
                                f"{EMOJI_MONEYFACE}, you got a {tip_type_text} of {coin_emoji}**{amountDiv_str} {token_display}** "
    ...
    async def multiple_tip_talker(
            ...
            try:
                ...
                if (
                    ...
                    try:
                        ...
                        await logchanbot("tips " + str(traceback.format_exc()))
                        print(
                            "Failed creating wallet for tip talk for userid: {}".format(
                                member_id
                            )
                        )
            except Exception:
        ...
        if amount > max_tip or amount < min_tip:
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, transaction cannot be bigger than "
                f"**{num_format_coin(max_tip)} {token_display}** "
            ...
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, insufficient balance to send "
        ...
        if total_amount > max_tip:
            ...
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, total transaction cannot be smaller than "
                f"**{num_format_coin(min_tip)} {token_display}**."
            )
            await ctx.edit_original_message(content=msg)
            return
            ...
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention} {guild_name}, insufficient balance to send total "
        ...
        if len(list_receivers) < 1:
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, there is no active talker in such period. "
        ...
        if tiptalk:
            ...
            try:
                msg = (
                    f"{EMOJI_ARROW_RIGHTHOOK} {tip_type_text} of {coin_emoji}**{num_format_coin(total_amount)}"
            ...
            if ping is False:
                extra_msg = (
                    "*PING is disable within command or mention is disable by Guild.*"
                )
        ...
        try:
            ...
            async with ctx.typing():
                ...
                if amount < min_tip or amount > max_tip:
                    ...
                    msg = (
                        f"{EMOJI_RED_NO} {ctx.author.mention}, insufficient balance to send tip of "
                        f"**{num_format_coin(amount)} {token_display}**."
                    )
                    await ctx.reply(msg)
                    return
                ...
                if len(user_mentions) == 0 and len(role_mentions) == 0:
                    await ctx.reply(
                        f"{EMOJI_RED_NO} {ctx.author.mention}, there is no one to tip to."
                    )
                    return
                ...
                if len(role_mentions) >= 1:
                    for each_role in role_mentions:
                        role_list_members = [
                            ...
                            if member.bot == False and each_role in member.roles
                        ]
                        if len(role_list_members) >= 1:
                            for each_member in role_list_members:
                                if (
                                    each_member not in list_users
                                    and each_member != ctx.author
                                ):
                ...
                if len(list_users) == 0:
                    ...
                    if total_amount > max_tip:
                        ...
                        msg = (
                            f"{EMOJI_RED_NO} {ctx.author.mention}, insufficient balance to send total "
                    ...
                    if price_with:
                        ...
                        if per_unit and per_unit["price"] and per_unit["price"] > 0:
                            ...
                            total_amount_in_usd = float(
                                Decimal(per_unit) * Decimal(total_amount)
                            )
                            if total_amount_in_usd > 0.0001:
                    ...
                    if tip is not None:
                        ...
                        try:
                            msg = (
                                f"{EMOJI_ARROW_RIGHTHOOK} tip of {coin_emoji}**{num_format_coin(total_amount)}"
                        ...
                        if 0 < len(list_receivers) <= 40:
                            ...
                            for member_id in list_receivers:
                                ...
                                if (
                                    send_tipped_ping
                                    >= self.bot.config["discord"]["maxTipTalkMessage"]
                                ):
                                    total_found += 1
                                else:
                                    if (
                                    ...
                                    if (
                                        numb_mention > 0
                                        and numb_mention % max_mention == 0
                                    ):
                                        # send the batch
                                        mention_users = ""
                                        if len(list_user_mention) >= 1:
