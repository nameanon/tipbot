...
class Twitter(commands.Cog):
    ...
    async def add_guild_sub(
        ...
        guild_id: str,
        subscribe_to: str,
        subscribe_to_user_id: str,
        push_to_channel_id: str,
        added_by_uid: str,
        added_by_uname: str,
    ):
        try:
        ...
        try:
            ...
            await self.add_fetch_tw(
                ...
                "https://twitter.com/{}/status/{}".format(
                    tweet["user"]["screen_name"], tweet_id_str
                ),
                tweet["text"],
                json.dumps(tweet),
                int_timestamp,
                ...
                if len(json_t) > 0:
                    for each_t in json_t:
                        ...
                        posted_timestamp = int(
                            datetime.strptime(
                                each_t["created_at"], "%a %b %d %H:%M:%S +0000 %Y"
                            ).timestamp()
                        )
                        if posted_timestamp + 24 * 3600 < int(time.time()):
                            ...
                            check_tw = await self.check_tweet_id_posted(
                                each_t["id_str"], each_tw["guild_id"]
                            )
                            if check_tw is None:
                                ...
                                try:
                                    ...
                                    channel = self.bot.get_channel(
                                        int(each_tw["push_to_channel_id"])
                                    )
                                    if channel:
                                        embed = disnake.Embed(
                                            title="New Tweet by {}!".format(
                                                each_t["user"]["name"]
                                            ),
                                        ...
                                        embed.add_field(
                                            name="Tweet",
                                            value=each_t["full_text"],
                                            inline=False,
                                        )
                                        embed.add_field(
                                            name="Link",
                                            value="<https://twitter.com/{}/status/{}>".format(
                                                each_t["user"]["screen_name"],
                                                each_t["id_str"],
                                            ),
                                            inline=False,
                                        )
                                        ...
                                        try:
                                            if "profile_image_url_https" in each_t[
                                                "user"
                                            ] and each_t["user"][
                                                ...
                                                avatar_url = each_t["user"][
                                                    "profile_image_url_https"
                                                ]
                                                embed.set_author(
                                                    name=each_t["user"]["screen_name"],
                                                    icon_url=avatar_url,
                                                )
                                        except Exception:
                                            traceback.print_exc(file=sys.stdout)
                                        try:
                                            ...
                                            await logchanbot(
                                                "[TWITTER] - Posted to guild {} / channel {}:{}".format(
                                                    each_tw["guild_id"],
                                            ...
                                            traceback.print_exc(file=sys.stdout)
                                            await logchanbot(
                                                "[TWITTER] - Failed to post to guild {} / channel {}:{}".format(
                                                    each_tw["guild_id"],
                                                    each_tw["push_to_channel_id"],
                                                    each_t["full_text"],
                                                )
        ...
        if len(get_list) > 0:
            for each_sub in get_list:
                try:
                    ...
                    func_tweets = functools.partial(
                        g_tweets, each_sub["subscribe_to_user_id"], 20
                    )
                    fetch_tweets = await self.bot.loop.run_in_executor(
                        None, func_tweets
                    )
                    if fetch_tweets is None:
                        user = each_sub["subscribe_to_user_id"]
                        await logchanbot(
                            f"[TWITTER] - Fetch @{user} doesn't get any response."
                        )
                        await asyncio.sleep(time_lap)
                        continue
                    else:
                        ...
                        twitter_link = "https://twitter.com/{}".format(
                            each_sub["subscribe_to_user_id"]
                        )
                        # Check latest one:
                        ...
                        if last_record is None:
                            ...
                            if (
                                last_record["latest_tweet_id_str"]
                                != latest_tweet_id_str
                            ):
                                inserts = await self.add_fetch_timeline(
                                    twitter_link,
    ...
    @commands.slash_command(
        dm_permission=False, description="Various twitter's commands."
    )
    ...
    async def subscribe(self, ctx, channel: disnake.TextChannel, twitter_link: str):
        if (
        ...
        if not user.isalnum():
            ...
            if get_user is None:
                ...
                if len(check_sub_link) >= serverinfo["max_twitter_subscription"]:
                    msg = (
                        f"{EMOJI_INFORMATION} {ctx.author.mention}, your guild `{ctx.guild.name}` has maximum "
    ...
    async def unsubscribe(self, ctx, twitter_link: str):
        if (
            ctx.channel.permissions_for(
                ctx.guild.get_member(ctx.author.id)
            ).manage_channels
    ...
    @commands.guild_only()
    @twitter.sub_command(
        usage="twitter listsub", description="List of subscribed twitter link."
    )
    async def listsub(
        self,
        ctx,
        ...
        if len(check_sub_link) == 0:
            ...
            for each in check_sub_link:
                links.append(
                    "<#{}> to: <{}> added by {}.".format(
                        each["push_to_channel_id"],
                        each["subscribe_to"],
                        each["added_by_uname"],
                    )
                )
            msg = (
                f"{EMOJI_INFORMATION} {ctx.author.mention}, your guild `{ctx.guild.name}` subscribed to:\n\n"
                + "\n".join(links)
            )
            await ctx.edit_original_message(content=msg)
    ...
    async def rt_reward(
        ...
        if (
            serverinfo["rt_reward_amount"]
            and serverinfo["rt_reward_coin"]
            and serverinfo["rt_reward_channel"]
            and serverinfo["rt_link"]
            and serverinfo["rt_end_timestamp"]
            and serverinfo["rt_end_timestamp"] > int(time.time()) + 3600
        ):  # reserved 1hr
            ...
            if get_tweet is None:
                ...
                if get_tweet["retweet_count"] > 100:
                    ...
                    twitter_link = "https://twitter.com/{}/status/{}".format(
                        get_tweet["user"]["screen_name"], ids
                    )

        # Check if previously run
        check_reward_exist = await self.get_reward_guild_link(
            str(ctx.guild.id), twitter_link
        )
        ...
        # We assume at least guild need to have 100x of reward or depends on guild's population
            ...
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, you need to have at least 100x reward balance. "
            ...
            # Test message
            msg = (
                f"New Twitter RT reward link <{twitter_link}> with {num_format_coin(amount)} {token_display} by "
            ...
            if rt_reward > 0:
                msg = (
                    f"{ctx.author.mention}, successfully set reward for RT in guild {ctx.guild.name} to "
    ...
    async def linkme(self, ctx, twitter_name: str, status_link: str = None):
        await self.bot_log()
        twitter_name = twitter_name.replace("@", "")
        msg = (
            f"{EMOJI_INFORMATION} {ctx.author.mention}, Bot's checking your twitter..."
        )
        ...
        if status_link is None:
            if get_linkme and get_linkme["is_verified"] == 0:
                ...
                if screen_name.upper() != twitter_name.upper():
                    ...
                    msg = (
                        f"{EMOJI_INFORMATION} {ctx.author.mention}, you generated a key already but not `{twitter_name}`. "
                    ...
                    # He already have in record but has't verify, show existing to him
                    msg = (
                        f"{EMOJI_INFORMATION} {ctx.author.mention}, you generated a key already:"
                ...
                if add > 0:
                    msg = (
                        f"{EMOJI_INFORMATION} {ctx.author.mention}, one more step to verify:```"
            ...
            if "?" in status_link:
                status_link = status_link.split("?")[
                    0
                ]  # reported by chooseuser#0308 when using mobile
            if get_linkme is None:
                msg = (
                    f"{EMOJI_INFORMATION} {ctx.author.mention}, you didn't generate a `secret_key` yet. "
                ...
                msg = (
                    f"{EMOJI_INFORMATION} {ctx.author.mention}, you didn't link yourself with twitter "
                    f"`{twitter_name}` before. If you want to change, unlink it first by `/twitter unlink`."
                )
                await ctx.edit_original_message(content=msg)
                return
            else:
                if status_link.endswith("/"):
                    status_link = status_link[0:-1]
                ...
                if not ids.isdigit():
                    ...
                    if get_tweet is None:
                        ...
                        status_link = "https://twitter.com/{}/status/{}".format(
                            get_tweet["user"]["screen_name"], ids
                        )
                        int_timestamp = int(
                            datetime.strptime(
                                get_tweet["created_at"], "%a %b %d %H:%M:%S +0000 %Y"
                            ).timestamp()
                        )
                        if (
                            secret_key not in get_tweet["text"]
                            or bot_name.upper() not in get_tweet["text"].upper()
                            or screen_name.upper()
                            != get_linkme["twitter_screen_name"].upper()
                        ):
                            ...
                            secret_key in get_tweet["text"]
                            and bot_name.upper() in get_tweet["text"].upper()
                            and screen_name.upper()
                            == get_linkme["twitter_screen_name"].upper()
                        ):
                            update = await self.twitter_linkme_update_verify(
                                str(ctx.author.id),
                                get_tweet["user"]["id_str"],
                                screen_name,
                                status_link,
                                get_tweet["text"],
                                json.dumps(get_tweet),
                                int_timestamp,
                            )
    ...
    @twitter.sub_command(
        usage="twitter unlinkme", description="Unlink your Discord from a twitter."
    )
    async def unlinkme(self, ctx):
        await self.bot_log()
        msg = (
            f"{EMOJI_INFORMATION} {ctx.author.mention}, Bot's checking your twitter..."
        )
        ...
        if get_linkme is None:
            ...
            unlink_user = await self.twitter_unlink(
                str(ctx.author.id),
                "{}#{}".format(ctx.author.name, ctx.author.discriminator),
                get_linkme["twitter_screen_name"],
                get_linkme["is_verified"],
            )
    ...
    async def tip(self, ctx, amount: str, coin: str, twitter: str):
        old_twitter = twitter
        has_link = False
        await self.bot_log()
            ...
            if get_user is None:
                ...
                if int(get_user["discord_user_id"]) == ctx.author.id:
                    ...
                    if tip:
                        ...
                        try:
                            msg = (
                                f"{EMOJI_ARROW_RIGHTHOOK} {ctx.author.mention}, you sent a twitter tip "
                        ...
                        if get_user["discord_user_id"] not in notifyList:
                            ...
                            try:
                                tw_user = self.bot.get_user(
                                    int(get_user["discord_user_id"])
                                )
                                await tw_user.send(
    ...
    async def deposit(self, ctx, token: str, plain: str = "embed"):
        await self.bot_log()
        msg = f"{EMOJI_INFORMATION} {ctx.author.mention}, Bot's checking twitter..."
        await ctx.response.send_message(msg, ephemeral=True)
        ...
        if token is None:
            ...
            if not hasattr(self.bot.coin_list, coin_name):
                ...
                if (
                    ...
                    getattr(getattr(self.bot.coin_list, coin_name), "enable_twitter")
                    != 1
                ):
                    await ctx.edit_original_message(
        ...
        try:
            ...
            if get_linkme is None:
                ...
                get_linkme
                and get_linkme["is_verified"] == 1
                and get_linkme["id_str"].isdigit()
            ):
                twitter_screen_name = get_linkme["twitter_screen_name"]
                ...
                if (
                    ...
                    real_min_deposit = getattr(
                        getattr(self.bot.coin_list, coin_name), "real_min_deposit"
                    )
                    fee_txt = (
                        " You must deposit at least {} {} to cover fees needed to credit your account. "
                        f"This fee will be deducted from your deposit amount.".format(
                            num_format_coin(real_min_deposit), token_display
                        )
                    )
                embed = disnake.Embed(
                    title=f"Deposit for your twitter {twitter_screen_name}",
                ...
                try:
                    await self.generate_qr_address(qr_address)
                    address_path = (
                        qr_address.replace("{", "_")
                        .replace("}", "_")
                        .replace(":", "_")
                        .replace('"', "_")
                        .replace(",", "_")
                ...
                plain_msg = "Twitter @{} Your deposit address: ```{}```".format(
                    twitter_screen_name, wallet_address
                )
                embed.add_field(
                    name=f"Your Twitter Deposit {coin_name}",
    ...
    @twitter.sub_command(
        usage="twitter balances", description="Show all your linked twitter's balances."
    )
    async def balances(self, ctx):
        ...
        if get_linkme is None:
            ...
            all_pages = []
            all_names = [
                each["coin_name"] for each in mytokens if each["enable_twitter"] == 1
            ]
            total_coins = len(all_names)
            page = disnake.Embed(
                title=f"[ YOUR TWITTER @{twitter_screen_name} BALANCE LIST ]",
            ...
            for each_token in mytokens:
                if each_token["enable_twitter"] != 1:
                    continue
                try:
                    ...
                    try:
                        ...
                        await self.utils.update_user_balance_call(
                            twitter_id_str, type_coin
                        )
                    except Exception:
                        traceback.print_exc(file=sys.stdout)
            ...
            if has_none_balance is True:
                ...
                try:
                    ...
                    traceback.print_exc(file=sys.stdout)
                    await logchanbot(
                        f"[ERROR] /twitter balances with {ctx.author.name}#{ctx.author.discriminator}"
                    )
