...
class Voucher(commands.Cog):
                ...
                async with conn.cursor() as cur:
                    ...
                    data_rows.append(
                        (
                            ...
                            channel,
                            amount,
                            float(per_unit_usd) * float(amount),
                            coin_decimal,
                            tiptype,
                            currentTs,
                            user_server,
                    ...
                    data_rows.append(
                        (
                            ...
                            channel,
                            reserved_fee,
                            float(per_unit_usd) * float(reserved_fee),
                            coin_decimal,
                            tiptype,
                            currentTs,
                            user_server,
            ...
            async with store.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    if already_claimed == "YESNO":
                        ...
                        """
                        await cur.execute(
                            sql, (user_id, user_server, already_claimed, last)
                        )
                        result = await cur.fetchall()
                        return result
        except Exception:
    ...
    async def make(self, ctx, amount: str, coin: str, comment: str = None):
        ...
        try:
            ...
            if not hasattr(self.bot.coin_list, coin_name):
                ...
                if (
                    getattr(getattr(self.bot.coin_list, coin_name), "enable_voucher")
                    != 1
                ):
            ...
            getattr(getattr(self.bot.coin_list, coin_name), "real_min_tip")
            getattr(getattr(self.bot.coin_list, coin_name), "real_max_tip")
            ...
            min_voucher_amount = getattr(
                getattr(self.bot.coin_list, coin_name), "voucher_min"
            )
            max_voucher_amount = getattr(
                getattr(self.bot.coin_list, coin_name), "voucher_max"
            )
            fee_voucher_amount = getattr(
                getattr(self.bot.coin_list, coin_name), "real_voucher_fee"
            )
            ...
            if actual_balance < total_amount + total_fee_amount:
                msg = (
                    f"{EMOJI_RED_NO} {ctx.author.mention}, insufficient balance to create voucher. "
            ...
            if len(comment_str) > self.max_comment:
                msg = (
                    f"{EMOJI_RED_NO} {ctx.author.mention}, please limit your comment to max. "
            ...
            try:
                tmp_msg = await ctx.author.send(
                    f"{ctx.author.mention}, we are making voucher, hold on..."
                )
            except Exception:
                traceback.print_exc(file=sys.stdout)
            ...
            if voucher_numb > 1:
                for i in range(voucher_numb):
                    try:
                        ...
                        qrstring = (
                            self.voucher_url + "/claim/" + secret_string
                        )  # config
                        ...
                        try:
                            img_frame.save(
                                self.path_voucher_create + unique_filename + ".png"
                            )
                            if str(ctx.author.id) not in self.bot.tipping_in_progress:
                                ...
                                try:
                                    ...
                                    traceback.print_exc(file=sys.stdout)
                                    await logchanbot(
                                        "voucher " + str(traceback.format_exc())
                                    )
                            else:
                ...
                try:
                    ...
                    try:
                        ...
                        draw.text(
                            (250 - w / 2, 275 + 125 + h), msg, fill="black", font=myFont
                        )
                    ...
                    try:
                        ...
                        if str(ctx.author.id) not in self.bot.tipping_in_progress:
                            ...
                            # reject and tell to wait
                            msg = (
                                f"{EMOJI_RED_NO} {ctx.author.mention}, you have another tx in process. "
    ...
    async def voucher_make_token_name_autocomp(
    ...
    @voucher.sub_command(
        usage="voucher unclaim", description="View list of unclaimed vouchers."
    )
    async def unclaim(self, ctx):
        ...
        await self.bot_log()
        get_vouchers = await self.sql_voucher_get_user(
            str(ctx.author.id), SERVER_BOT, 50, "NO"
        )
        ...
        if get_vouchers and len(get_vouchers) >= 25:
            ...
            await ctx.edit_original_message(
                content=f"{ctx.author.mention} You have many unclaimed vouchers: {unclaim}"
            )
            return
        elif get_vouchers and len(get_vouchers) > 0:
            table_data = [["Ref Link", "Amount", "Claimed?", "Created"]]
            for each in get_vouchers:
                table_data.append(
                    [
                        ...
                        datetime.fromtimestamp(each["date_create"]).strftime(
                            "%Y-%m-%d"
                        ),
                    ]
                )
            ...
            await ctx.edit_original_message(
                content=f"{ctx.author.mention}, you did not create any voucher yet."
            )
        return
    ...
    async def getunclaim(self, ctx):
        ...
        await self.bot_log()
        get_vouchers = await self.sql_voucher_get_user(
            str(ctx.author.id), SERVER_BOT, 10000, "NO"
        )
            ...
            try:
                ...
                for item in get_vouchers:
                    ...
                    if not hasattr(self.bot.coin_list, item["coin_name"]):
                        continue
                    is_voucher = getattr(
                        getattr(self.bot.coin_list, item["coin_name"]), "enable_voucher"
                    )
                    if is_voucher != 1:
                        continue
                    voucher_url_list.append(
                        self.voucher_url + "/claim/" + item["secret_string"]
                    )
                if len(voucher_url_list) > 0:
                    voucher_url_list_str = "\n".join(voucher_url_list)
                    combined_vouchers = (
                        "Total unclaimed: "
                        + str(len(voucher_url_list))
                        + "\n\n"
                        + voucher_url_list_str
                    )
                    ...
                    await ctx.edit_original_message(
                        content=f"{ctx.author.mention}, you don't have any unclaimed voucher."
                    )
            except Exception:
                traceback.print_exc(file=sys.stdout)
                await logchanbot("voucher " + str(traceback.format_exc()))
    ...
    @voucher.sub_command(
        usage="voucher claim", description="View list of claimed vouchers."
    )
    async def claim(self, ctx):
        ...
        await self.bot_log()
        get_vouchers = await self.sql_voucher_get_user(
            str(ctx.author.id), SERVER_BOT, 50, "YES"
        )
        ...
        if get_vouchers and len(get_vouchers) >= 25:
            ...
            await ctx.edit_original_message(
                content=f"{ctx.author.mention}, you have many claimed vouchers: {claimed}"
            )
            return
        elif get_vouchers and len(get_vouchers) > 0:
            table_data = [["Ref Link", "Amount", "Claimed?", "Created"]]
            ...
            await ctx.edit_original_message(
                content=f"{ctx.author.mention}, you don't have any claimed voucher."
            )
    ...
    async def getclaim(self, ctx):
        ...
        await self.bot_log()
        get_vouchers = await self.sql_voucher_get_user(
            str(ctx.author.id), SERVER_BOT, 10000, "YES"
        )
        if get_vouchers and len(get_vouchers) > 0:
            try:
                voucher_url_list = []
    ...
    @voucher.sub_command(
        usage="voucher listcoins", description="List coins/tokens supported /voucher"
    )
