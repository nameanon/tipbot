...
class PartyButton(disnake.ui.View):
    ...
    @disnake.ui.button(
        label="Join (x1)", style=ButtonStyle.primary, custom_id="partydrop_tipbot"
    )
    ...
    @disnake.ui.button(
        label="🎉 Join (x10)", style=ButtonStyle.green, custom_id="partydrop_tipbot_10x"
    )
    ...
    @disnake.ui.button(
        ...
        custom_id="partydrop_tipbot_40x",
    )
...
class PartyDrop(commands.Cog):
                ...
                for each_party in get_list_parties:
                    ...
                    try:
                        ...
                        if each_party["partydrop_time"] < int(time.time()):
                            ...
                            embed.set_footer(
                                text=f"Initiated by {owner_displayname} | /partydrop | Ended"
                            )
                            if len(coin_emoji) > 0:
                                extension = ".png"
                            ...
                            user_tos.append(
                                {
                                    "from_user": get_message["from_userid"],
                                    "to_user": str(self.bot.user.id),
                                    "guild_id": get_message["guild_id"],
                                    "channel_id": get_message["channel_id"],
                                    "amount": get_message["init_amount"],
                                    "coin": get_message["token_name"],
                                    "decimal": get_message["token_decimal"],
                                    "contract": get_message["contract"],
                                    "real_amount_usd": get_message[
                            ...
                            if len(attend_list) > 0:
                                ...
                                name_list.append(
                                    "<@{}> : {} {}".format(
                                        ...
                                        num_format_coin(get_message["init_amount"]),
                                        token_display,
                                    )
                                )
                                for each_att in attend_list:
                                    name_list.append(
                                        "<@{}> : {} {}".format(
                                            ...
                                            num_format_coin(each_att["joined_amount"]),
                                            token_display,
                                        )
                                    )
                                    ...
                                    user_tos.append(
                                        {
                                            "from_user": each_att["attendant_id"],
                                            "to_user": str(self.bot.user.id),
                                            "guild_id": get_message["guild_id"],
                                            "channel_id": get_message["channel_id"],
                                            "amount": each_att["joined_amount"],
                                            "coin": get_message["token_name"],
                                            "decimal": get_message["token_decimal"],
                                            "contract": get_message["contract"],
                                            "real_amount_usd": get_message[
                                            ...
                                            * each_att["joined_amount"]
                                            if get_message["unit_price_usd"]
                                            and get_message["unit_price_usd"] > 0
                                            else 0.0,
                                            "extra_message": None,
                                        }
                                    )
                            ...
                            indiv_amount = total_amount / len(
                                all_name_list
                            )  # including initiator
                            amount_in_usd = (
                                indiv_amount * get_message["unit_price_usd"]
                                if get_message["unit_price_usd"]
                                and get_message["unit_price_usd"] > 0.0
                                else 0.0
                            )
                            ...
                            try:
                                ...
                                if channel is None:
                                    await logchanbot(
                                        "party_check: can not find channel ID: {} for drop ID: {}".format(
                                            each_party["channel_id"],
                                            each_party["message_id"],
                                        )
                                    )
                                    # add fail check
                                    ...
                                    if each_party["failed_check"] > 3:
                                        turn_off = True
                                    await store.update_party_failed(
                                        each_party["message_id"], turn_off
                                    )
                                    find_owner = self.bot.get_user(
                                        int(each_party["from_userid"])
                                    )
                                    if find_owner is not None:
                                        await find_owner.send(
                                            "I cancelled your /partydrop id: {} because I can't find channel.".format(
                                                each_party["message_id"]
                                            )
                                        )
                                    continue
                                _msg: disnake.Message = await channel.fetch_message(
                                    int(each_party["message_id"])
                                )
                                await _msg.edit(content=None, embed=embed, view=None)
                                # Update balance
                                mv_partydrop = (
                                    await store.sql_user_balance_mv_multple_amount(
                                        user_tos, "PARTYDROP", SERVER_BOT
                                    )
                                )
                                if mv_partydrop is True:
                                    party = await store.sql_user_balance_mv_multiple(
                                ...
                                await logchanbot(
                                    "[PARTYDROP]: can not find message ID: {} of channel {} in guild: {}. Set that to FAILED.".format(
                                        each_party["message_id"],
                                        each_party["channel_id"],
                                        each_party["guild_id"],
                                    )
                                ...
                                await store.update_party_failed(
                                    each_party["message_id"], True
                                )
                                await asyncio.sleep(1.0)
                            except disnake.errors.DiscordServerError:
                                await logchanbot(
                                    "[PARTYDROP]: DiscordServerError message ID: {} of channel {} in guild: {}.".format(
                                        each_party["message_id"],
                                        each_party["channel_id"],
                                        each_party["guild_id"],
                                    )
                            ...
                            if "fetched_msg" not in self.bot.other_data:
                                ...
                                if (
                                    ...
                                    duration = each_party["partydrop_time"] - int(
                                        time.time()
                                    )
                            ...
                            lap_div = int(
                                (get_message["partydrop_time"] - int(time.time())) / 30
                            )
                            embed.set_footer(
                            ...
                            try:
                                ...
                                if self.party_cache[key] == "{}_{}_{}".format(
                                    len(user_tos), total_amount, lap_div
                                ):
                                    continue  # to next, no need to edit
                                else:
                                    self.party_cache[key] = "{}_{}_{}".format(
                                        len(user_tos), total_amount, lap_div
                                    )
                            except Exception:
                                pass
                            ...
                            try:
                                ...
                                if channel is None:
                                    ...
                                    try:
                                        ...
                                        await logchanbot(
                                            "party_check: can not find message ID: {} in channel: {}".format(
                                                each_party["message_id"],
                                                each_party["channel_id"],
                                            )
                                        )
    ...
    async def async_partydrop(
        self, ctx, min_amount: str, sponsor_amount: str, token: str, duration: str
    ):
        ...
        await ctx.response.send_message(
            f"{ctx.author.mention}, {command_mention} preparation... "
        )
        ...
        try:
            ...
            if (
                ...
                await ctx.edit_original_message(content=msg)
                await logchanbot(
                    f"[PARTYDROP] server {str(ctx.guild.id)} has no data in discord_server."
                )
                return
        except Exception:
            traceback.print_exc(file=sys.stdout)
        ...
        if min_amount > max_tip or min_amount < min_tip:
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, minimum amount cannot be bigger than "
            ...
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, sponored amount cannot be bigger than "
            ...
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, insufficient balance to sponsor "
        ...
        embed = disnake.Embed(
            ...
            description="Each click will deduct from your TipBot's balance. Minimum entrance cost: {}`{} {}`. "
            "Party Pot will be distributed equally to all attendees after "
            "completion.".format(coin_emoji, num_format_coin(min_amount), coin_name),
        ...
        try:
            view = PartyButton(
                ctx, duration_s, self.bot.coin_list, self.bot, ctx.channel.id
            )
            ...
            await ctx.edit_original_message(
                content=f"Missing permission! Or failed to send embed message {command_mention}."
            )
        except Exception:
            traceback.print_exc(file=sys.stdout)
        try:
    ...
    @commands.slash_command(
        ...
        options=[
            ...
            Option(
                "sponsor_amount", "sponsor_amount", OptionType.string, required=True
            ),
    ...
    async def partydrop_token_name_autocomp(
