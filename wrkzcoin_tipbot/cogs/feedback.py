...
import disnake
import store
from Bot import EMOJI_INFORMATION, SERVER_BOT, log_to_channel
...
async def sql_feedback_list(limit: int = 10):
    try:
        await store.openConnection()
        async with store.pool.acquire() as conn:
            async with conn.cursor() as cur:
...
async def sql_feedback_get(ref: str):
    try:
        await store.openConnection()
        async with store.pool.acquire() as conn:
            async with conn.cursor() as cur:
...
class FeedbackAdd(disnake.ui.Modal):
    ...
    def __init__(self, bot, inquiry_type: str) -> None:
        ...
        super().__init__(
            title="Feedback/Request our TipBot",
            custom_id="modal_addtrivia_question",
    ...
    async def callback(self, inter: disnake.ModalInteraction) -> None:
        ...
        if desc_id == "" or len(desc_id) < 8:
            await inter.response.send_message(
                "Description is empty or too short!", ephemeral=True
            )
            return
        contact_id = inter.text_values["contact_id"].strip()
        if contact_id == "" or len(contact_id) < 4:
            await inter.response.send_message(
                "Contact back is empty or too short!", ephemeral=True
            )
            return
        ...
        if add:
            ...
            await inter.response.send_message(
                f"{inter.author.mention}, internal error, please report!"
            )
...
class BotFeedback(commands.Cog):
    ...
    @commands.slash_command(
        name="feedback", description="Various feedback commands and subs."
    )
    async def feedback_main(self, ctx):
        pass

    @feedback_main.sub_command(
        name="list", usage="feedback list", description="Admin to list all feedback"
    )
    async def feedback_list(self, ctx) -> None:
    ...
    async def feedback_view(
        ...
        if get_fb is None:
            await ctx.edit_original_message(
                content="There is no such feedback `ref`. Check with `/feedback list`!"
            )
            return
        else:
            try:
                ...
                embed.add_field(
                    name="Feedback", value=get_fb["feedback_text"], inline=False
                )
                embed.add_field(
    ...
    async def feedback_add(self, ctx, inquiry_type: str) -> None:
        """Sends a Modal to create a new feedback."""
        try:
            await ctx.response.send_modal(
                modal=FeedbackAdd(self.bot, inquiry_type=inquiry_type)
            )
        except Exception:
            traceback.print_exc(file=sys.stdout)
