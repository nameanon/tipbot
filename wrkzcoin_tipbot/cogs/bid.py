    ...
    async def confirm(
        ...
        if inter.author.id != self.owner_id:
            await inter.response.send_message(
                f"{inter.author.mention}, this is not your menu!", delete_after=5.0
            )
        else:
            self.value = True
            self.stop()
...
class ReportBid(disnake.ui.Modal):
    ...
    async def callback(self, interaction: disnake.ModalInteraction) -> None:
        ...
        await interaction.response.send_message(
            content=f"{interaction.author.mention}, checking bidding report...",
            ephemeral=True,
        )
        desc_id = interaction.text_values["desc_id"].strip()
        if desc_id == "":
            await interaction.edit_original_message(
                f"{interaction.author.mention}, report content can't be empty!"
            )
            return
        ...
        if contact_id == "":
            await interaction.edit_original_message(
                f"{interaction.author.mention}, contact field can't be empty!"
            )
            return
        ...
        try:
            ...
            if adding_report is True:
                msg = (
                    f"{EMOJI_INFORMATION} {self.ctx.author.mention}, we received your report! "
...
class EditBid(disnake.ui.Modal):
    def __init__(
        self, ctx, bot, message_id: str, owner_userid: str, title: str, desc: str = None
    ) -> None:
        self.ctx = ctx
        self.bot = bot
        self.utils = Utils(self.bot)
        ...
        super().__init__(
            ...
            custom_id="modal_bidding_description",
            components=components,
        )
    ...
    async def callback(self, interaction: disnake.ModalInteraction) -> None:
        # Check if type of question is bool or multipe
        await interaction.response.send_message(
            content=f"{interaction.author.mention}, checking bidding description...",
            ephemeral=True,
        )
        desc_id = interaction.text_values["desc_id"].strip()
        if desc_id == "":
            await interaction.edit_original_message(
                f"{interaction.author.mention}, description can't be empty!"
            )
            return
        try:
            ...
            if update_bid:
                ...
                try:
                    ...
                    _msg: disnake.Message = await interaction.channel.fetch_message(
                        self.message_id
                    )
                    ...
                    self.bot.other_data["fetched_msg"][str(self.message_id)] = int(
                        time.time()
                    )
                except Exception:
                    traceback.print_exc(file=sys.stdout)
...
class PlaceBid(disnake.ui.Modal):
    ...
    async def callback(self, interaction: disnake.ModalInteraction) -> None:
        # Check if type of question is bool or multipe
        await interaction.response.send_message(
            content=f"{interaction.author.mention}, checking bidding amount...",
            ephemeral=True,
        )
                ...
                if len(attend_list) > 0:
                    ...
                    if previous_user_id == interaction.author.id:
                        ...
                        if view.value is False:
                            await interaction.edit_original_message(
                                content=f"{EMOJI_INFORMATION} {interaction.author.mention}, you rejected to place another bid.",
                                view=None,
                            )
                            return
        ...
        try:
            ...
            min_bid_start = getattr(
                getattr(self.bot.coin_list, coin_name), "min_bid_start"
            )
            step_amount = self.step_amount
            if step_amount is None:
                step_amount = getattr(
                    getattr(self.bot.coin_list, coin_name), "min_bid_lap"
                )
            ...
            if amount <= 0:
                ...
                try:
                    ...
                    if previous_bid and previous_bid > 0:
                        ...
                        if additional_bid_amount > actual_balance:
                            msg = (
                                f"{EMOJI_RED_NO} {self.ctx.author.mention}, insufficient balance to place a bid of "
                ...
                msg = (
                    f"{EMOJI_RED_NO} {self.ctx.author.mention}, bid amount can't be smaller than "
            ...
            try:
                ...
                try:
                    ...
                    if previous_bid and previous_bid > 0:
                        additional_bid_amount = amount - previous_bid
                    await self.utils.async_set_cache_kv("bidding_amount", key, amount)
                except Exception:
                    traceback.print_exc(file=sys.stdout)
                ...
                if adding_bid:
                    ...
                    if (
                        ...
                        and previous_user_id != interaction.author.id
                    ):
                        try:
                            get_user = self.bot.get_user(previous_user_id)
                            if get_user is not None:
                                await get_user.send(
                                    f"{EMOJI_INFORMATION} {interaction.author.mention} placed a higher bid than "
                    ...
                    try:
                        ...
                        embed.add_field(
                            name="Started amount",
                            value=num_format_coin(get_message["minimum_amount"])
                            + " "
                            + coin_name,
                            inline=True,
                        )
                        ...
                        attend_list = await self.utils.get_bid_attendant(
                            str(self.message_id)
                        )
                        if len(attend_list) > 0:
                        ...
                        embed.add_field(name="Note", value=bid_note, inline=False)
                        await _msg.edit(content=None, embed=embed)
                        await log_to_channel(
                            "bid",
...
class OwnerWinnerInput(disnake.ui.Modal):
    def __init__(
        ...
        placeholder = (
            "Write how you want to deliver..."
            if self.method_for == "winner"
        ...
        title = (
            "Method of delivery"
            if self.method_for == "winner"
            else "Update status to winner"
        )
        super().__init__(
            title=title, custom_id="modal_winning_instruction", components=components
        )

    async def callback(self, interaction: disnake.ModalInteraction) -> None:
        # Check if type of question is bool or multipe
        await interaction.response.send_message(
            content=f"{interaction.author.mention}, checking information...",
            ephemeral=True,
        )
        instruction = interaction.text_values["instruction_id"].strip()
        if instruction == "":
            await interaction.edit_original_message(
                f"{interaction.author.mention}, instruction is empty!"
            )
            return
        try:
            get_message = await self.utils.get_bid_id(str(self.message_id))
            if (
                get_message["winner_instruction"] is None
                and self.method_for == "winner"
                and interaction.author.id != self.winner_user_id
            ):
                await interaction.edit_original_message(
                    f"{interaction.author.mention}, you clicked on wrong button!"
                )
                return
            elif (
                get_message["owner_respond"] is None
                and self.method_for == "owner"
                and interaction.author.id != self.owner_userid
            ):
                await interaction.edit_original_message(
                    f"{interaction.author.mention}, you clicked on wrong button!"
                )
                ...
                get_message["winner_instruction"] is not None
                and get_message["owner_request_to_update"] == 0
                and self.method_for == "winner"
                ...
                await interaction.edit_original_message(
                    f"{interaction.author.mention}, winner's input:\n\n{get_message['winner_instruction']}"
                )
                return
            elif (
                get_message["owner_respond"] is not None
                and self.method_for == "owner"
                and interaction.author.id
                in [
                    self.owner_userid,
                ...
                await interaction.edit_original_message(
                    f"{interaction.author.mention}, owner's input:\n\n{get_message['owner_respond']}"
                )
                return
            # check if input already
            if get_message["winner_instruction"] is None and self.method_for == "owner":
                await interaction.edit_original_message(
                    f"{interaction.author.mention}, please inform to winner to update his delivery method first!"
                )
                return
            elif (
                (
                    ...
                    or (
                        get_message["winner_instruction"] is not None
                        and get_message["owner_request_to_update"] == 1
                    )
                )
                and self.method_for == "winner"
                and interaction.author.id == self.winner_user_id
                ...
                try:
                    ...
                    if owner_user is not None:
                        ...
                        await interaction.edit_original_message(
                            f"{interaction.author.mention}, we updated your input and notified the bidding owner."
                        )
                    else:
                        await interaction.edit_original_message(
                            f"{interaction.author.mention}, we updated your input and but we failed to find the bidding owner user!"
                        )
                except disnake.errors.Forbidden:
                    await interaction.edit_original_message(
                        f"{interaction.author.mention}, we updated your input but we failed "
                ...
                view = ClearButton(
                    ...
                    int(self.bid_info["user_id"]),
                    int(self.winner_user_id),
                    self.winner_amount,
                    self.bid_info,
                    False,
                    False,
                ...
                await interaction.edit_original_message(
                    f"{interaction.author.mention}, you already input the necessary information! Here they are:\n\n{get_message['winner_instruction']}"
                )
                return
            # owner
                ...
                try:
                    ...
                    if winner_user is not None:
                        ...
                        await interaction.edit_original_message(
                            f"{interaction.author.mention}, we updated your input and notified the winner <@{str(self.winner_user_id)}>."
                        )
                    else:
                        await interaction.edit_original_message(
                            f"{interaction.author.mention}, we updated your input and but we failed to find the winner <@{str(self.winner_user_id)}>!"
                        )
                except disnake.errors.Forbidden:
                    await interaction.edit_original_message(
                        f"{interaction.author.mention}, we updated your input but we failed "
                ...
                get_message["owner_respond"] is not None and self.method_for == "owner"
            ):
                await interaction.edit_original_message(
                    f"{interaction.author.mention}, you already input the necessary information! Here they are:\n\n{get_message['owner_respond']}"
                )
                ...
                await interaction.edit_original_message(
                    f"{interaction.author.mention}, internal error!"
                )
                return
        except Exception:
            traceback.print_exc(file=sys.stdout)
...
class ClearButton(disnake.ui.View):
    ...
    def __init__(
        ...
        winner_amount: float,
        bid_info,
        disable_winner_btn: bool = False,
        disable_owner_btn: bool = True,
        complete_btn: bool = True,
        owner_request_btn: bool = True,
    ):
        super().__init__()
        self.bot = bot
        self.wallet_api = WalletAPI(self.bot)
    ...
    @disnake.ui.button(
        label="1️⃣ Winner Click",
        style=ButtonStyle.primary,
        custom_id="bidding_clearbtn_winner_input",
    )
    async def winner_input(
        ...
        if interaction.author.id == self.owner_id:
            await interaction.response.send_message(
                f"{interaction.author.mention}, checking <@{str(self.winner_id)}> 's message...",
                ephemeral=True,
            )
            get_message = await self.utils.get_bid_id(str(self.message.id))
            if get_message["winner_instruction"] is not None:
                await interaction.edit_original_message(
                    f"{interaction.author.mention}, winner <@{str(self.winner_id)}> input information as below:\n\n{get_message['winner_instruction']}"
                )
            else:
                await interaction.edit_original_message(
                    f"{interaction.author.mention}, winner <@{str(self.winner_id)}> hasn't input anything yet!"
                )
        elif interaction.author.id != self.winner_id:
            await interaction.response.send_message(
                f"{interaction.author.mention}, that's not yours!", ephemeral=True
            )
        else:
            try:
                ...
                await interaction.response.send_message(
                    f"{interaction.author.mention}, failed to retreive bidding information! Try again later!",
                    ephemeral=True,
                )
            except Exception:
                traceback.print_exc(file=sys.stdout)
    ...
    @disnake.ui.button(
        label="2️⃣ Owner Click",
        style=ButtonStyle.primary,
        custom_id="bidding_clearbtn_owner_input",
    )
    async def owner_input(
        ...
        if interaction.author.id == self.winner_id:
            await interaction.response.send_message(
                f"{interaction.author.mention}, checking <@{str(self.owner_id)}>'s respond...",
                ephemeral=True,
            )
            get_message = await self.utils.get_bid_id(str(self.message.id))
            if get_message["owner_respond"] is not None:
                await interaction.edit_original_message(
                    f"{interaction.author.mention}, <@{str(self.owner_id)}> updates information as below:\n\n{get_message['owner_respond']}"
                )
            else:
                await interaction.edit_original_message(
                    f"{interaction.author.mention}, <@{str(self.owner_id)}> hasn't update anything yet!"
                )
    ...
    @disnake.ui.button(
        label="3️⃣ Complete",
        style=ButtonStyle.primary,
        custom_id="bidding_clearbtn_winner_complete",
    )
    async def complete_all(
        ...
        if interaction.author.id != self.winner_id:
            ...
            try:
                await interaction.response.send_message(
                    f"{interaction.author.mention}, in progress...", ephemeral=True
                )
                ...
                if get_message["winner_confirmation_date"] is not None:
                    await interaction.edit_original_message(
                        f"{interaction.author.mention}, that was already paid and processed!"
                    )
                    return
                ...
                payment_logs.append(
                    (
                        "PAYMENT",
                        self.bid_info["message_id"],
                        str(self.winner_id),
                        self.bid_info["guild_id"],
                        self.bid_info["channel_id"],
                        int(time.time()),
                        "-" + num_format_coin(self.winner_amount),
                    )
                )
                remaining = self.winner_amount
                payment_list_msg.append(
                    f"Processing deduct from winner <@{str(self.winner_id)}>: "
                ...
                if self.bot.config["bidding"]["bid_collecting_fee"] > 0:
                    payment_list.append(
                        (
                            "SYSTEM",
                            self.bid_info["token_name"],
                            SERVER_BOT,
                            self.bot.config["bidding"]["bid_collecting_fee"]
                            * self.winner_amount,
                            int(time.time()),
                        )
                    )
                ...
                payment_list_msg.append(
                    f"Processing to auction owner <@{str(self.owner_id)}>: {num_format_coin(remaining)} {self.bid_info['token_name']}"
                )
                await self.utils.update_bid_winner_instruction(
                    str(self.bid_info["message_id"]),
                    "placeholder",
                    "final",
                    payment_list,
                    payment_logs,
                ...
                await interaction.edit_original_message(
                    f"{interaction.author.mention}, Completed! Payment also paid to the Owner!"
                )
                get_owner = self.bot.get_user(int(self.bid_info["user_id"]))
                if get_owner is not None:
                    await get_owner.send(
                        f"{interaction.author.mention} marked your bid as completed. "
    ...
    @disnake.ui.button(
        label="📝 Owner Request Update",
        style=ButtonStyle.primary,
        custom_id="bidding_clearbtn_owner_request",
    )
        ...
        if interaction.author.id != self.owner_id:
            ...
            try:
                ...
                if get_message["winner_confirmation_date"] is not None:
                    ...
                    await interaction.edit_original_message(
                        f"{interaction.author.mention}, winner didn't input anything yet!"
                    )
                    return
                elif get_message["owner_request_to_update"] == 1:
                    await interaction.edit_original_message(
                        f"{interaction.author.mention}, this bid previously requested to update already!"
                    )
                    return
                elif get_message["owner_respond"] is not None:
                    await interaction.edit_original_message(
                        f"{interaction.author.mention}, you already updated your respond for this bid!"
                    )
                    return
                else:
                    ...
                    if req is True:
                        ...
                        get_winner = self.bot.get_user(
                            int(get_message["winner_user_id"])
                        )
                        if get_winner is not None:
                            try:
                                await get_winner.send(
                                    f"{interaction.author.mention} requested you to re-update the information for this {link_bid}!"
                                )
                                await interaction.edit_original_message(
                                    f"{interaction.author.mention}, successfully requested him/her to updated!"
                                )
                            except Exception:
                                traceback.print_exc(file=sys.stdout)
                                await interaction.edit_original_message(
                                    f"{interaction.author.mention}, I failed to DM <@{get_message['winner_user_id']}> about requesting update!"
                                )
                        else:
                            await interaction.edit_original_message(
                                f"{interaction.author.mention}, updated to request more "
    ...
    @disnake.ui.button(
        label="⚠️Report", style=ButtonStyle.gray, custom_id="bidding_report_clear"
    )
...
class BidButton(disnake.ui.View):
    ...
    @disnake.ui.button(
        label="Place Bid", style=ButtonStyle.primary, custom_id="bidding_place_bid"
    )
    async def bid_place(
        self, button: disnake.ui.Button, interaction: disnake.MessageInteraction
    ):
        if (
            interaction.author.id == self.owner_id
            and self.bot.config["bidding"]["allow_own_bid"] != 1
        ):
            await interaction.response.send_message(
                f"{interaction.author.mention}, you can't bid on your own!",
                delete_after=5.0,
            )
        else:
            try:
    ...
    async def bid_edit(
        ...
        if interaction.author.id != self.owner_id:
            await interaction.response.send_message(
                f"{interaction.author.mention}, that's not your listing!",
                ephemeral=True,
            )
        else:
            try:
    ...
    @disnake.ui.button(
        label="🔴 Cancel", style=ButtonStyle.danger, custom_id="bidding_cancel"
    )
    async def bid_cancel(
        ...
        if interaction.author.id != self.owner_id:
            ...
            await interaction.response.send_message(
                f"{interaction.author.mention}, checking cancellation!", ephemeral=True
            )
            try:
                # add for confirmation
                view = ConfirmName(self.bot, interaction.author.id)
                ...
                if view.value is False:
                    await interaction.edit_original_message(
                        content=f"{EMOJI_INFORMATION} {interaction.author.mention}, the bid is not cancelled. Thank you!",
                        view=None,
                    )
                    return
                elif view.value is None:
                    ...
                    try:
                        ...
                        if cancelling is True:
                            ...
                            try:
                                ...
                                traceback.print_exc(file=sys.stdout)
                            await interaction.edit_original_message(
                                f"{interaction.author.mention}, successfully cancelled!\n{link_bid}",
                                view=None,
                            )
                            # DM refund
                            for i in refund_list:
                                try:
                                    ...
                                    if get_u is not None:
                                        await get_u.send(
                                            f"Bid __{str(self.message.id)}__ cancelled in guild {interaction.guild.name}/{interaction.guild.id}!"
                                            " You get a refund of "
    ...
    @disnake.ui.button(
        label="⚠️Report", style=ButtonStyle.gray, custom_id="bidding_report"
    )
    async def bid_report(
        ...
        try:
            await interaction.response.send_modal(
                modal=ReportBid(interaction, self.bot, self.message.id, self.owner_id)
            )
        except disnake.errors.NotFound:
            await interaction.response.send_message(
                f"{interaction.author.mention}, failed to retreive bidding information! Try again later!",
...
class Bidding(commands.Cog):
    def __init__(self, bot):
        ...
        self.bidding_cache = TTLCache(
            maxsize=2000, ttl=60.0
        )  # if previous value and new value the same, no need to edit
        self.wallet_api = WalletAPI(self.bot)
        self.utils = Utils(self.bot)
                    ...
                    try:
                        ...
                        try:
                            channel = self.bot.get_channel(
                                int(get_message["channel_id"])
                            )
                            if channel is None:
                                await logchanbot(
                                    "bidding_check: can not find channel ID: {}".format(
                                        each_bid["channel_id"]
                                    )
                                )
                                ...
                                if "fetched_msg" not in self.bot.other_data:
                                    ...
                                    if (
                                        each_bid["message_id"]
                                        in self.bot.other_data["fetched_msg"]
                                    ):
                                        time_left = each_bid["bid_open_time"] - int(
                                            time.time()
                                        )
                                        last_fetched = self.bot.other_data[
                                            "fetched_msg"
                                        ][each_bid["message_id"]]
                                        if (
                                            int(time.time()) - last_fetched < 90
                                            and time_left > 10 * 3600
                                        ):
                                            continue
                                _msg: disnake.Message = await channel.fetch_message(
                                    int(each_bid["message_id"])
                                )
                                embed = _msg.embeds[0]  # embeds is list, we take 0
                                embed.clear_fields()
                                embed.add_field(
                                ...
                                embed.add_field(
                                    name="Step amount",
                                    value=num_format_coin(step_amount)
                                    + " "
                                    + coin_name,
                                    inline=True,
                                )
                                ...
                                if each_bid["number_extension"] > 0:
                                    embed.add_field(
                                        name="Number of Extension",
                                        value="{}".format(each_bid["number_extension"]),
                                        inline=True,
                                    )
                                bid_note = self.bot.config["bidding"]["bid_note"]
                                if self.bot.config["bidding"]["bid_collecting_fee"] > 0:
                                ...
                                if each_bid["status"] == "CANCELLED":
                                    ...
                                    each_bid["winner_user_id"] is not None
                                    and each_bid["winner_instruction"] is None
                                ):
                                    status_msg = "Waiting for winner's information."
                                elif (
                                    each_bid["winner_user_id"] is not None
                                    and each_bid["owner_respond"] is None
                                ):
                                    status_msg = "Waiting for owner's update."
                                elif (
                                    each_bid["winner_user_id"] is not None
                                    and each_bid["winner_confirmation_date"] is None
                                ):
                                    status_msg = (
                                        "Waiting for winner's final confirmation."
                                    )
                                ...
                                embed.add_field(
                                    name="Note", value=bid_note, inline=False
                                )
                                if _msg is not None and len(_msg.components) == 0:
                                    print(
                                        "BID {} complete but not paid..".format(
                                            each_bid["message_id"]
                                        )
                                    )
                                    ...
                                    winner_btn = (
                                        False
                                        if each_bid["winner_confirmation_date"] is None
                                        else True
                                    )
                                    owner_btn = (
                                        False
                                    ...
                                    owner_req_btn = (
                                        False
                                        if each_bid["winner_instruction"] is not None
                                        else True
                                    )
                                    ...
                                    view = ClearButton(
                                        ...
                                        self.bot,
                                        channel.id,
                                        int(each_bid["user_id"]),
                                        int(attend_list[0]["user_id"]),
                                        attend_list[0]["bid_amount"],
                                        each_bid,
                                        winner_btn,
                                        owner_btn,
                                        complete_btn,
                                    ...
                                    _msg is not None
                                    and len(_msg.components) > 0
                                    and int(time.time())
                                    - int(_msg.edited_at.timestamp())
                                    > 2 * 60
                                ):
                    ...
                    try:
                        ...
                        try:
                            ...
                            if channel is None:
                                ...
                                link = (
                                    self.bot.config["bidding"]["web_path"]
                                    + each_bid["saved_name"]
                                )
                                embed.set_image(url=link)
                                embed.set_footer(
                                    text=f"Created by {owner_displayname} | /bid add | Time left: {time_left}"
                                )
                                if duration <= 0:
                                    embed.set_footer(
                                        text=f"Created by {owner_displayname} | /bid add | Ended!"
                                    )
                                if len(attend_list) > 0:
                                    for i in attend_list:
                                        if i["user_id"] not in list_joined_key:
                            ...
                            if each_bid["failed_check"] <= 3:
                                await self.utils.update_bid_failed(
                                    each_bid["message_id"], False
                                )
                            elif len(attend_list) == 0:
                                # nothing to refund
                                await self.utils.update_bid_failed(
                                    each_bid["message_id"], True
                                )
                            elif len(attend_list) > 0:
                                try:
                                    # refund
                        ...
                        if each_bid["bid_open_time"] < int(time.time()):
                            ...
                            try:
                                ...
                                if len(attend_list) == 0:
                                    ...
                                    await self.utils.update_bid_no_winning(
                                        each_bid["message_id"]
                                    )
                                    # notify owner, no winner
                                    msg_owner = (
                                        "One of your bidding is completed! "
                                    ...
                                    msg_winner = (
                                        "Congratulation! You won a bid __{}__ in guild {}/{}. "
                                ...
                                for i in refund_list:
                                    try:
                                        ...
                                        if get_u is not None:
                                            await get_u.send(
                                                f"You didn't win for bidding __{str(each_bid['message_id'])}__ in Guild "
                            ...
                            try:
                                ...
                                if (
                                    ...
                                    view = BidButton(
                                        ...
                                        self.bot.coin_list,
                                        self.bot,
                                        int(each_bid["channel_id"]),
                                        int(each_bid["user_id"]),
                                        each_bid["token_name"],
                                        each_bid["minimum_amount"],
                                        each_bid["step_amount"],
                                        each_bid["title"],
                                        each_bid["description"],
                                    )
                                    view.message = _msg
    ...
    @commands.slash_command(
        name="bid", dm_permission=False, description="Various bid's commands."
    )
    async def bid(self, ctx):
        if (
            self.bot.config["bidding"]["enable"] == 0
            and ctx.author.id != self.bot.config["discord"]["owner_id"]
        ):
            await ctx.response.send_message(
                content=f"{ctx.author.mention}, this command is currently disable!"
            )
            return
        if (
            self.bot.config["bidding"]["enable"] == 1
            and self.bot.config["bidding"]["is_private"] == 1
            and ctx.author.id not in self.bot.config["bidding"]["testers"]
        ):
            await ctx.response.send_message(
                content=f"{ctx.author.mention}, this command is still restricted! Try again later!"
            )
            return
    ...
    async def bidding_add_new(
        ...
        try:
            count_ongoing = await self.utils.discord_bid_ongoing(
                str(ctx.guild.id), "ONGOING"
            )
            # Check max if set in guild
            if (
                serverinfo
                and count_ongoing >= serverinfo["max_ongoing_bid"]
                and ctx.author.id != self.bot.config["discord"]["owner_id"]
            ):
                ...
                await ctx.edit_original_message(
                    content=msg, view=RowButtonRowCloseAnyMessage()
                )
                return
            elif (
                serverinfo is None
                ...
                await logchanbot(
                    f"[BIDDING] server {str(ctx.guild.id)} has no data in discord_server."
                )
                return
        except Exception:
            traceback.print_exc(file=sys.stdout)
        ...
        try:
            ...
            if (
                get_perms[self.bot.config["bidding"]["perm_bid_add"]] is False
            ...
            try:
                if not hasattr(self.bot.coin_list, coin_name):
                    ...
                    return
                bid_enable = getattr(
                    getattr(self.bot.coin_list, coin_name), "bid_enable"
                )
                if bid_enable != 1:
                ...
                if "$" in step_amount[-1] or "$" in step_amount[0]:  # last is $
                    ...
                    if price_with is None:
                        ...
                        if per_unit and per_unit["price"] and per_unit["price"] > 0:
                            per_unit = per_unit["price"]
                            step_amount = float(
                                Decimal(step_amount) / Decimal(per_unit)
                            )
                        else:
                            msg = (
                                f"{EMOJI_RED_NO} {ctx.author.mention}, I cannot fetch equivalent price. "
                ...
                if len(title) <= 3 or len(title) > 128:
                    msg = (
                        f"{EMOJI_INFORMATION} {ctx.author.mention}, the title is too short or too long. "
                            ...
                            async with session.get(
                                ...
                                if response.status == 200:
                                    ...
                                    if mime_type not in self.file_accept:
                                        ...
                                        with open(
                                            self.bid_storage + saved_name, "wb"
                                        ) as f:
    ...
    async def pbid_add_token_name_autocomp(
    ...
    async def bid_view(self, ctx, bid_id: str):
        msg = f"{EMOJI_INFORMATION} {ctx.author.mention}, loading..."
        ...
        if ctx.author.id != self.bot.config["discord"]["owner_id"]:
            await ctx.edit_original_message(
                content="Permission denied or currently unavailable!"
            )
            return
        ...
        try:
            ...
            if get_message is None:
                await ctx.edit_original_message(
                    content=f"I can't find bid message ID: {bid_id}"
                )
                return
            ...
            try:
                ...
                if channel is None:
                    ...
                    if get_message["number_extension"] > 0:
                        embed.add_field(
                            name="Number of Extension",
                            value="{}".format(get_message["number_extension"]),
                            inline=True,
                        )
                    bid_note = self.bot.config["bidding"]["bid_note"]
                    if self.bot.config["bidding"]["bid_collecting_fee"] > 0:
                    ...
                    if get_message["status"] == "CANCELLED":
                        ...
                        get_message["winner_user_id"] is not None
                        and get_message["winner_instruction"] is None
                    ):
                        status_msg = "Waiting for winner's information."
                    elif (
                        get_message["winner_user_id"] is not None
                        and get_message["owner_respond"] is None
                    ):
                        status_msg = "Waiting for owner's update."
                    elif (
                        get_message["winner_user_id"] is not None
                        and get_message["winner_confirmation_date"] is None
                    ):
                    ...
                    embed.add_field(
                        name="Status",
                        value=status_msg
                        + " and created <t:{}:f>".format(get_message["message_time"]),
                        inline=False,
                    )
                    embed.add_field(
                        name="Guild {}/{}".format(
                            get_message["guild_name"], get_message["guild_id"]
                        ),
    ...
    async def bid_clear_win_menu(self, ctx, msg_id: str):
        msg = f"{EMOJI_INFORMATION} {ctx.author.mention}, loading..."
        await ctx.response.send_message(msg, ephemeral=True)
        ...
        try:
            ...
            if get_message is None:
                await ctx.edit_original_message(
                    content=f"I can't find message ID: {msg_id}"
                )
                return
            elif get_message["status"] == "ONGOING":
                await ctx.edit_original_message(
                    content="That bidding is still ONGOING."
                )
                return
            elif get_message["status"] == "CANCELLED":
                await ctx.edit_original_message(
                    content="That bidding is already CANCELLED."
                )
                return
            elif (
                get_message["status"] == "COMPLETED"
                and get_message["owner_respond"] is not None
                and get_message["winner_confirmation_date"] is not None
            ):
                await ctx.edit_original_message(
                    content="That bidding is already SETTLE and paid."
                )
                return
            elif (
                get_message["status"] == "COMPLETED"
                ...
                if get_guild is None:
                    await ctx.edit_original_message(
                        content=f"I can't find guild {get_message['guild_id']}."
                    )
                    return
                else:
                    ...
                    if channel is None:
                        await ctx.edit_original_message(
                            content=f"I can't find guild {get_message['channel_id']}."
                        )
                    else:
                        _msg: disnake.Message = await channel.fetch_message(
                            int(get_message["message_id"])
                        ...
                        if _msg:
                            ...
                            await ctx.edit_original_message(
                                content=f"Done. Removed win menu fo {get_message['message_id']}."
                            )
                        else:
                            await ctx.edit_original_message(
                                content=f"Failed to fetch message {get_message['channel_id']}."
                            )
        except Exception:
            traceback.print_exc(file=sys.stdout)
    ...
    async def bid_myrecents(self, ctx, member_id: str = None):
        msg = f"{EMOJI_INFORMATION} {ctx.author.mention}, loading..."
        await ctx.response.send_message(msg, ephemeral=True)
        ...
        if (
            ...
            try:
                user_bids = await self.utils.bidding_joined_by_userid(
                    member_id, limit=25
                )
                ...
                if len(user_logs) > 0:
                    list_logs_items = []
                    for i in user_logs:
                        list_logs_items.append(
                            "⚆ <t:{}:f>\n{}/{}".format(
                                i["time"],
                                i["message_id"] if i["message_id"] else "N/A",
                                i["type"],
                            )
                        )
                    embed.add_field(
    ...
    async def bid_list(self, ctx, guild_id: str = None):
        msg = f"{EMOJI_INFORMATION} {ctx.author.mention}, loading..."
        await ctx.response.send_message(msg, ephemeral=True)
        if guild_id is None:
            guild_id = str(ctx.guild.id)
        ...
        try:
            if ctx.author.id != self.bot.config["discord"]["owner_id"]:
                await ctx.edit_original_message(
                    content=f"{ctx.author.mention}, permission denied!"
                )
                return
            ...
            if len(list_bids) > 0:
                ...
                for i in list_bids:
                    list_bid_items.append(
                        "⚆ <t:{}:f>-{}/{}\nMin. amount {} {}".format(
                            ...
                            i["message_id"],
                            i["status"],
                            num_format_coin(i["minimum_amount"]),
                            i["token_name"],
                        )
                    )
                ...
                await ctx.edit_original_message(
                    content=f"{ctx.author.mention}, there is not any listing!"
                )
        except Exception:
            traceback.print_exc(file=sys.stdout)
    ...
    async def bid_report_list(self, ctx, report_id: int = None):
        msg = f"{EMOJI_INFORMATION} {ctx.author.mention}, loading..."
        await ctx.response.send_message(msg, ephemeral=True)
        ...
        try:
            ...
            if report_id is None and len(list_reps) > 0:
                ...
                await ctx.edit_original_message(
                    ...
                    "Contact:\n{}\n-----------\nReporter: <@{}>\nGuild Name: {}/{}".format(
                        report["report_id"],
                        report["list_message_id"],
                        report["time"],
                        report["reported_content"],
                        report["how_to_contact"],
                        report["user_id"],
                        report["guild_name"],
                        report["guild_id"],
                    )
                ...
                await ctx.edit_original_message(
                    content=f"{ctx.author.mention}, there is no such report id!"
                )
            else:
                await ctx.edit_original_message(
                    content=f"{ctx.author.mention}, no report found!"
                )
        except Exception:
            traceback.print_exc(file=sys.stdout)
