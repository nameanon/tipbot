        ...
        try:
            ...
            async with store.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    """
                    await cur.execute(
                        sql, (user_id, directory, guild_id, type_vote, date_vote)
                    )
                    await conn.commit()
                    return True
        except Exception:
                    ...
                    if str(request.rel_url).startswith("/topgg_server_vote/"):
                        if "Authorization" in request.headers:
                            ...
                            try:
                                ...
                                check_last_vote = await self.check_last_guild_vote(
                                    user_vote, "topgg", guild_id
                                )
                                if (
                            ...
                            if get_guild_by_key and get_guild_by_key == key.strip():
                                ...
                                if guild and int(user_vote) not in [
                                    ...
                                    try:
                                        ...
                                        traceback.print_exc(file=sys.stdout)
                                    return web.Response(
                                        status=200, text="Thank you anyway!"
                                    )
                                elif guild and int(user_vote) in [
                                    m.id for m in guild.members
                                ]:
                                    get_guild = await self.guild_find_by_id(guild_id)
                                    if (
                                        get_guild["vote_reward_amount"]
                                        and get_guild["vote_reward_amount"] > 0
                                    ):
                                        ...
                                        if total_balance < amount + extra_amount:
                                            ...
                                            try:
                                                ...
                                                if price_with:
                                                    per_unit = (
                                                        await self.utils.get_coin_price(
                                                ...
                                                if member is not None:
                                                    ...
                                                    if extra_amount > 0:
                                                        extra_msg = (
                                                            " You have a guild's role that give you additional "
                                                            f"bonus **"
                                                            + num_format_coin(
                                                                extra_amount
                                                            )
                                                    ...
                                                    msg = (
                                                        f"Thank you for voting for guild `{guild.name}` at top.gg. "
                                ...
                                return web.Response(
                                    status=404,
                                    text="No such server by this key! Or not up to date!",
                                )
                        else:
                            return web.Response(
                                status=500, text="No Authorization! Or not up to date!"
                            )
                                                    ...
                                                    if total_balance <= amount:
                                                        ...
                                                        try:
                                                            ...
                                                            if member is not None:
                                                                ...
                                                                # end advert
                                                                msg = (
                                                                    f"Thank you for voting for our TipBot at <{self.bot.config['bot_vote_link']['topgg']}>. "
