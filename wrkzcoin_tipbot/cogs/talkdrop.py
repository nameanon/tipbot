...
class TalkDropButton(disnake.ui.View):
    ...
    @disnake.ui.button(
        label="Collect", style=ButtonStyle.green, custom_id="talkdrop_tipbot"
    )
...
class TalkDrop(commands.Cog):
    ...
    async def talkdrop_check(self):
        ...
        if (
            check_last_running and int(time.time()) - check_last_running["run_at"] < 10
        ):  # not running if less than 10s
            return
        try:
                ...
                for each_talkdrop in get_list_talkdrop:
                    ...
                    try:
                        attend_list = await store.get_talkdrop_collectors(
                            each_talkdrop["message_id"]
                        )
                        # Update view
                        owner_displayname = each_talkdrop["from_ownername"]
                        each_talkdrop["real_amount_usd_text"]
                        ...
                        time_passed = (
                            int(time.time()) - each_talkdrop["talked_from_when"]
                        )
                        # If talkdrop ends
                        if each_talkdrop["talkdrop_time"] < int(time.time()):
                            embed = disnake.Embed(
                                ...
                                description="You can collect only if you have chatted in channel <#{}> from {} ago.".format(
                                    each_talkdrop["talked_in_channel"],
                                    seconds_str_days(time_passed),
                                ),
                                timestamp=datetime.fromtimestamp(
                                    each_talkdrop["talkdrop_time"]
                                ),
                            )
                            if len(coin_emoji) > 0:
                                extension = ".png"
                            ...
                            embed.set_footer(
                                text=f"Contributed by {owner_displayname} | /talkdrop | Ended"
                            )
                            all_name_list = []
                            ...
                            indiv_amount = (
                                each_talkdrop["real_amount"] / len(all_name_list)
                                if len(all_name_list) > 0
                                else each_talkdrop["real_amount"]
                            )
                            amount_in_usd = (
                                indiv_amount * each_talkdrop["unit_price_usd"]
                                if each_talkdrop["unit_price_usd"]
                                and each_talkdrop["unit_price_usd"] > 0.0
                                else 0.0
                            )
                            ...
                            embed.add_field(
                                name="Total Amount",
                                value=coin_emoji
                                + num_format_coin(each_talkdrop["real_amount"])
                                + " "
                                + coin_name,
                                inline=True,
                            )
                            ...
                            try:
                                channel = self.bot.get_channel(
                                    int(each_talkdrop["channel_id"])
                                )
                                if channel:
                                    try:
                                        _msg: disnake.Message = (
                                            await channel.fetch_message(
                                                int(each_talkdrop["message_id"])
                                            )
                                        )
                                        await _msg.edit(
                                        ...
                                        if len(all_name_list) > 0:
                                            await store.sql_user_balance_mv_multiple(
                                                each_talkdrop["from_userid"],
                                                all_name_list,
                                                each_talkdrop["guild_id"],
                                                each_talkdrop["channel_id"],
                                                indiv_amount,
                                                coin_name,
                                                "TALKDROP",
                                                coin_decimal,
                                                SERVER_BOT,
                                                each_talkdrop["contract"],
                                                float(amount_in_usd),
                                                None,
                                            )
                                        await store.update_talkdrop_id(
                                            each_talkdrop["message_id"],
                                            "COMPLETED"
                                            if len(all_name_list) > 0
                                            else "NOCOLLECT",
                                        ...
                                        await logchanbot(
                                            "talkdrop_check: can not find message ID: {} in channel {}.".format(
                                                each_talkdrop["message_id"],
                                                each_talkdrop["channel_id"],
                                            )
                                        )
                                    ...
                                    await logchanbot(
                                        "talkdrop_check: can not find channel {} for message ID: {}".format(
                                            each_talkdrop["channel_id"],
                                            each_talkdrop["message_id"],
                                        )
                                    )
                            ...
                            time_left = (
                                seconds_str_days(
                                    each_talkdrop["talkdrop_time"] - int(time.time())
                                )
                                if int(time.time()) < each_talkdrop["talkdrop_time"]
                                else "00:00:00"
                            )
                            lap_div = int(
                                (each_talkdrop["talkdrop_time"] - int(time.time())) / 30
                            )
                            embed.set_footer(
                            ...
                            indiv_amount = (
                                each_talkdrop["real_amount"] / len(user_tos)
                                if len(user_tos) > 0
                                else each_talkdrop["real_amount"]
                            )
                            try:
                                key = each_talkdrop["message_id"]
                                if self.talkdrop_cache[key] == "{}_{}_{}".format(
                                    len(user_tos), each_talkdrop["real_amount"], lap_div
                                ):
                                    continue  # to next, no need to edit
                                else:
                                    self.talkdrop_cache[key] = "{}_{}_{}".format(
                                        len(user_tos),
                                        each_talkdrop["real_amount"],
                                        lap_div,
                                    )
                            except Exception:
                                pass
                                ...
                                if channel is None:
                                    ...
                                    try:
                                        ...
                                        if each_talkdrop["failed_check"] > 3:
                                            turn_off = True
                                        await store.update_talkdrop_failed(
                                            each_talkdrop["message_id"], turn_off
                                        )
                                        await logchanbot(
                                            "talkdrop_check: can not find message ID: {} in channel: {}".format(
                                                each_talkdrop["message_id"],
    ...
    async def async_talkdrop(
        ...
        await ctx.response.send_message(
            f"{ctx.author.mention}, /talkdrop preparation... "
        )
        ...
        try:
            ...
            if (
                ...
                await ctx.edit_original_message(content=msg)
                await logchanbot(
                    f"[TALKDROP] server {str(ctx.guild.id)} has no data in discord_server."
                )
                return
        except Exception:
            traceback.print_exc(file=sys.stdout)
        ...
        if amount > max_tip or amount < min_tip:
            ...
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, insufficient balance to do a drop of "
        ...
        if len(message_talker) <= 1:
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, there are not not enough active text in that channel "
        ...
        embed.add_field(name="Minimum Messages", value=minimum_message, inline=True)
        if len(coin_emoji) > 0:
            extension = ".png"
            if coin_emoji.startswith("<a:"):
                extension = ".gif"
    ...
    @commands.slash_command(
        ...
        options=[
            ...
            Option(
                "minimum_message", "minimum_message", OptionType.integer, required=True
            ),
        ],
    ...
    async def talkdrop(
        ...
        await self.async_talkdrop(
            ctx, amount, token, channel, from_when, end, minimum_message
        )

    @talkdrop.autocomplete("token")
    async def talkdrop_token_name_autocomp(
