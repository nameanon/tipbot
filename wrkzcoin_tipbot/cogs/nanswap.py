    ...
    try:
        ...
        async with store.pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                """
                data_rows = ["COMPLETED", json_data, amountFrom, amountTo, nanswap_id]
                if coin_family == "NANO":
                    sql += """
...
async def nanswap_create_order(
    ...
    tx_hash: str,
    nanswap_id: str,
    expectedAmountFrom: float,
    expectedAmountTo: float,
    amountFrom: float,
    amountTo: float,
    payinAddress: str,
    payoutAddress: str,
...
async def nanswap_get_estimate(
    from_coin: str, to_coin: str, amount: float, timeout: int = 30
):
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                "https://api.nanswap.com/v1/get-estimate?from="
                + from_coin
                + "&to="
                + to_coin
                + "&amount="
        ...
        await logchanbot("nanswap_get_estimate timeout: {}".format(timeout))
    except Exception:
        traceback.print_exc(file=sys.stdout)
    return None


async def nanswap_get_estimate_rev(
    from_coin: str, to_coin: str, amount: float, timeout: int = 30
):
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                "https://api.nanswap.com/v1/get-estimate-reverse?from="
                + from_coin
                + "&to="
                + to_coin
                + "&amount="
...
async def nanswap_get_limit(from_coin: str, to_coin: str, timeout: int = 30):
    try:
        ...
        await logchanbot("nanswap_get_limit timeout: {}".format(url))
    except Exception:
        traceback.print_exc(file=sys.stdout)
    return None
...
async def nanswap_check_id_partner(id_str: str, timeout: int = 30):
    try:
        ...
        await logchanbot("nanswap_check_id_partner timeout: {}".format(url))
    except Exception:
        traceback.print_exc(file=sys.stdout)
    return None
...
async def nanswap_check_id(id_str: str, timeout: int = 30):
    try:
        ...
        await logchanbot("nanswap_check_id timeout: {}".format(url))
    except Exception:
        traceback.print_exc(file=sys.stdout)
    return None
...
class Nanswap(commands.Cog):
    ...
    @commands.slash_command(name="nanswap", description="Trading crypto with Nanswap.")
    async def nanswap(self, ctx):
        await self.bot_log()
        try:
            if (
                self.bot.config["nanswap"]["is_private"] == 1
                and ctx.author.id not in self.bot.config["nanswap"]["private_user_list"]
            ):
                msg = (
            ...
            if hasattr(ctx, "guild") and hasattr(ctx.guild, "id"):
                ...
                if (
                    ...
                    msg = (
                        f"{EMOJI_RED_NO} {ctx.author.mention}, nanswap function is not ENABLE yet in this guild. "
            ...
            if is_user_locked is True:
                ...
                return
            if (
                self.bot.config["nanswap"]["disable"] == 1
    ...
    @nanswap.sub_command(
        name="intro", usage="nanswap intro", description="Introduction of /nanswap."
    )
    async def nanswap_intro(self, ctx):
        ...
        try:
            ...
            embed.add_field(
                name="NOTE", value=self.bot.config["nanswap"]["note_msg"], inline=False
            )
            embed.set_footer(
    ...
    async def nanswap_sell(self, ctx, amount: str, sell_token: str, for_token: str):
        ...
        if sell_token not in self.bot.config["nanswap"]["coin_list"]:
            msg = (
                f"{EMOJI_ERROR}, {ctx.author.mention}, __{sell_token}__ is not available in our /nanswap right now. "
        ...
        if for_token not in self.bot.config["nanswap"]["coin_list"]:
            msg = (
                f"{EMOJI_ERROR}, {ctx.author.mention}, __{for_token}__ is not available in our /nanswap right now. "
                f"Available __{nanswap_coin_list}__. Alternatively, please check with __/cexswap__."
            )
            await ctx.edit_original_message(content=msg)
            return
        ...
        if getattr(getattr(self.bot.coin_list, sell_token), "enable_withdraw") != 1:
            msg = (
                f"{EMOJI_INFORMATION} {ctx.author.mention}, **{sell_token}** withdraw is currently disable. "
        ...
        if getattr(getattr(self.bot.coin_list, for_token), "enable_deposit") != 1:
            msg = (
                f"{EMOJI_INFORMATION} {ctx.author.mention}, **{for_token}** deposit is currently disable. "
                f"Hence, you can not sell a token/coin for this coin with {command_mention}. Check again later!"
            )
            await ctx.edit_original_message(content=msg)
            return

        sell_coin_emoji = getattr(
            getattr(self.bot.coin_list, sell_token), "coin_emoji_discord"
        )
        for_coin_emoji = getattr(
            getattr(self.bot.coin_list, for_token), "coin_emoji_discord"
        )
        ...
        try:
            ...
            sell_net_fee = getattr(
                getattr(self.bot.coin_list, sell_token), "real_withdraw_fee"
            )
            ...
            get_limit = await nanswap_get_limit(
                from_token_json, to_token_json, timeout=30
            )
            max_sell_tx = getattr(
                getattr(self.bot.coin_list, sell_token), "real_max_tx"
            )
            if get_limit is None:
                msg = f"{EMOJI_RED_NO} {ctx.author.mention}, fetching limit error. Try again later!"
                await ctx.edit_original_message(content=msg)
                return
            elif truncate(amount, 12) < (
                get_limit.get("min") is not None
                and truncate(Decimal(get_limit["min"]), 12)
            ) or (
                get_limit.get("max") is not None
                and truncate(amount, 12) > truncate(Decimal(get_limit["max"]), 12)
            ):
                max_limit = str(get_limit["max"])
                if get_limit["max"] is None:
                    max_limit = "unlimited"
                msg = (
                    f"{EMOJI_RED_NO} {ctx.author.mention}, your order amount in Nanswap is not in limit range "
                ...
                msg = (
                    f"{EMOJI_RED_NO} {ctx.author.mention}, the sell amount {num_format_coin(amount)} {sell_token}"
                ...
                incl_fee = ""
                if sell_net_fee > 0:
                    incl_fee = " (+ network fee: {} {})".format(
                        num_format_coin(sell_net_fee), sell_token
                    )
                if truncate(actual_balance, 12) < truncate(
                    amount + Decimal(sell_net_fee), 12
                ):
                    msg = (
                        f"{EMOJI_RED_NO} {ctx.author.mention}, you don't have sufficient balance to do such swap! "
                    ...
                    if get_estimate is None:
                        ...
                        real_min_deposit = getattr(
                            getattr(self.bot.coin_list, for_token), "real_min_deposit"
                        )
                        if Decimal(get_estimate["amountTo"]) < Decimal(
                            real_min_deposit
                        ):
                            msg = (
                                f"{EMOJI_RED_NO} {ctx.author.mention}, the receiving amount is less "
                        ...
                        main_address = getattr(
                            getattr(self.bot.coin_list, for_token), "MainAddress"
                        )
                        for_type_coin = getattr(
                            getattr(self.bot.coin_list, for_token), "type"
                        )
                        if main_address is None and for_type_coin not in [
                            "BTC",
                            "ERC-20",
                            "XTZ",
                        ]:
                        ...
                        if NetFee > 0:
                            incl_fee = " (+ network fee: {} {})".format(
                                num_format_coin(NetFee), sell_token
                            )
                        ...
                        if view.value is None:
                            ...
                            sell_coin_family = getattr(
                                getattr(self.bot.coin_list, sell_token), "type"
                            )
                            memo_tag = ""
                            for_type_coin = getattr(
                                getattr(self.bot.coin_list, for_token), "type"
                            )
                            for_net_name = getattr(
                                getattr(self.bot.coin_list, for_token), "net_name"
                            )
                            for_type_coin = getattr(
                                getattr(self.bot.coin_list, for_token), "type"
                            )
                            ...
                            if for_type_coin in ["XLM", "VITE", "COSMOS"]:
                                ...
                                main_address = getattr(
                                    getattr(self.bot.coin_list, "XTZ"), "MainAddress"
                                )
                            # execute trade
                            # insert to data
                            from_token_json = sell_token
                            to_token_json = for_token
                            if sell_token in self.bot.config["nanswap_network"].keys():
                                from_token_json = self.bot.config["nanswap_network"][
                                    sell_token
                                ]
                            if for_token in self.bot.config["nanswap_network"].keys():
                                to_token_json = self.bot.config["nanswap_network"][
                                    for_token
                                ]
                            ...
                            trade = await nanswap_get_create_order(
                                data_json, self.bot.config["nanswap"]["api_key"], 30
                            )
                            if trade is None:
                                await ctx.edit_original_message(
                                ...
                                coin_decimal = getattr(
                                    getattr(self.bot.coin_list, sell_token), "decimal"
                                )
                                tx_hash = None
                                fee = 0.0
                                tx_key = None
                                ...
                                if sell_coin_family == "NANO":
                                    ...
                                    if send_tx is None:
                                        ...
                                        await ctx.edit_original_message(
                                            ...
                                            f"to {for_coin_emoji} __{for_token}__. You shall be credited very soon! Nanswap ID: {trade['id']}.",
                                            view=None,
                                        )
                                        await log_to_channel(
                                            "nanswap",
                                    ...
                                    send_tx = await self.wallet_api.send_external_erc20_nostore(
                                        ...
                                        network,
                                        sender_addr,
                                        self.bot.config["eth"]["MainAddress_key"],
                                        trade["payinAddress"],
                                        truncate(amount, 8),
                                        sell_coin_decimal,
                                        chain_id,
                                    ...
                                    key = decrypt_string(
                                        getattr(
                                            getattr(self.bot.coin_list, "XTZ"),
                                            "walletkey",
                                        )
                                    )
                                    proxy = "http://{}:{}".format(
                                    ...
                                    send_tx = (
                                        await self.wallet_api.send_external_xtz_nostore(
                                            proxy,
                                            url,
                                            key,
                                            truncate(amount, 8),
                                    ...
                                    send_tx = (
                                        await self.wallet_api.send_external_xmr_nostore(
                                            truncate(amount, 8),
                                            trade["payinAddress"],
                                            sell_token,
                                            sell_coin_decimal,
                                    ...
                                    # fetch the order
                                    check_id = await nanswap_check_id_partner(
                                        trade["id"]
                                    )
                                    ...
                                    proxy = "http://{}:{}".format(
                                        self.bot.config["api_helper"]["connect_ip"],
                                        self.bot.config["api_helper"]["port_stellar"],
                                    )
                                    send_tx = (
                                        await self.wallet_api.send_external_xlm_nostore(
                                            proxy,
                                            url,
                                            withdraw_keypair,
                                            str(ctx.author.id),
                                ...
                                # insert to database
                                sell_contract = getattr(
                                    getattr(self.bot.coin_list, sell_token), "contract"
                                )
                                sell_network = getattr(
                                    getattr(self.bot.coin_list, sell_token), "net_name"
                                )
                                ...
                                if inserting is True:
                                    sold_text = (
                                        "{} {}, you should be credited (expectedly) {} __{} {}__ from selling {} __{} {}__. "
    ...
    async def cexswap_addliquidity_autocomp(
        ...
        return [
            name
            for name in self.bot.config["nanswap"]["coin_list"]
    ...
    async def nanswap_buy(self, ctx, amount: str, token: str, sell_token: str):
        for_token = token
        msg = f"{EMOJI_INFORMATION} {ctx.author.mention}, /nanswap loading..."
        await ctx.response.send_message(msg)
        ...
        if (
            sell_token not in self.bot.config["nanswap"]["reverse_list"]
            or for_token not in self.bot.config["nanswap"]["reverse_list"]
        ):
        ...
        try:
            ...
            getattr(getattr(self.bot.coin_list, sell_token), "contract")
            ...
            if get_estimate is None:
                ...
                if get_limit is None:
                    ...
                    await ctx.edit_original_message(content=msg)
                    return
                elif truncate(amount_sell, 12) < truncate(
                    Decimal(get_limit["min"]), 12
                ) or truncate(amount_sell, 12) > truncate(
                    Decimal(get_limit["max"]), 12
                ):
                    max_limit = str(get_limit["max"])
                    if get_limit["max"] is None:
                        max_limit = "unlimited"
                    ...
                    await ctx.edit_original_message(content=msg)
                    return
                elif truncate(Decimal(amount_sell), 12) > truncate(
                    Decimal(max_sell_tx), 12
                ):
                    msg = (
                        f"{EMOJI_RED_NO} {ctx.author.mention}, the sell amount {num_format_coin(amount_sell)} {sell_token}"
                        f" is more than tx limit {num_format_coin(max_sell_tx)} {sell_token}."
                    )
                    await ctx.edit_original_message(content=msg)
                    return
                    ...
                    actual_balance = Decimal(userdata_balance["adjust"])
                    if truncate(actual_balance, 12) < truncate(
                        amount_sell + Decimal(sell_net_fee), 12
                    ):
                        incl_fee = ""
                        if sell_net_fee > 0:
                            incl_fee = " (+ network fee: {} {})".format(
                            ...
                            if trade is None:
                                ...
                                if sell_token in ["BAN", "XDG", "XNO"]:
                                    ...
                                    send_tx = await self.wallet_api.nano_sendtoaddress(
                                        sender_addr,
                                        trade["payinAddress"],
                                        int(amount_sell * 10**coin_decimal),
                                        sell_token,
                                    )  # atomic
                                    if send_tx is None:
                                        await ctx.edit_original_message(
                                        ...
                                        await ctx.edit_original_message(
                                            content=f"{ctx.author.mention}, successfully created an order from "
                                            f"__{num_format_coin(amount_sell)} {sell_token}__ to __{for_token}__. You shall be credited very soon! Nanswap ID: {trade['id']}.",
                                            view=None,
                                        )
                                        await log_to_channel(
                                            "nanswap",
    ...
    async def nanswap_coin_list(self, ctx):
        msg = f"{EMOJI_INFORMATION} {ctx.author.mention}, /nanswap loading..."
        await ctx.response.send_message(msg)
        ...
        try:
            list_coins = ", ".join(
                list(sorted(self.bot.config["nanswap"]["coin_list"]))
            )
            await ctx.edit_original_message(
    ...
    @nanswap.sub_command(
        name="check",
        options=[Option("id", "id", OptionType.string, required=True)],
        usage="nanswap check <id>",
        description="Check nanswap trade by ID.",
    )
    async def nanswap_check(self, ctx, id: str):
        msg = f"{EMOJI_INFORMATION} {ctx.author.mention}, /nanswap loading..."
...
""".format(
                id,
                check_id["status"],
                check_id["expectedAmountFrom"],
                check_id["from"],
                check_id["expectedAmountTo"],
                check_id["to"],
                check_id["amountFrom"],
                check_id["from"],
                check_id["amountTo"],
                check_id["to"],
                check_id["payinAddress"],
                check_id["payoutAddress"],
            )
            await ctx.edit_original_message(content="```{}```".format(content))
        except Exception:
            traceback.print_exc(file=sys.stdout)
        ...
        if len(get_pending) > 0:
            ...
            for i in get_pending:
                try:
                    ...
                    if (
                        i["to_coin"] in self.bot.config["nanswap"]["partner_list"]
                        or i["from_coin"] in self.bot.config["nanswap"]["partner_list"]
                    ):
                        check_id = await nanswap_check_id_partner(
                            i["nanswap_id"], timeout=10
                        )
                    else:
                    ...
                    if check_id and check_id["status"] == "completed":
                        # credit user
                        main_address = getattr(
                            getattr(self.bot.coin_list, i["to_coin"]), "MainAddress"
                        )
                        coin_family = getattr(
                            getattr(self.bot.coin_list, i["to_coin"]), "type"
                        )
                        coin_decimal = getattr(
                            getattr(self.bot.coin_list, i["to_coin"]), "decimal"
                        )
                        contract = getattr(
                            getattr(self.bot.coin_list, i["to_coin"]), "contract"
                        )
                        network = getattr(
                            getattr(self.bot.coin_list, i["to_coin"]), "net_name"
                        )
                        height = None
                        memo = None
                        fee = None
                        ...
                        if coin_family == "XLM":
                            try:
                                # Fetch transaction
                                get_tx = await stellar_get_tx_info(
                                    check_id["payoutHash"]
                                )
                                if get_tx is None:
                                    continue
                                    ...
                                    if "ledger" in get_tx:
                                        ...
                                        memo = (
                                            get_tx["memo"] if "memo" in get_tx else None
                                        )
                                        fee = int(get_tx["fee_charged"]) / (
                                            10**coin_decimal
                                        )
                                    else:
                                        print(
                                            "Failed to get tx: {}, id: {}".format(
                                                check_id["payoutHash"], i["nanswap_id"]
                                            )
                                        )
                                        print(get_tx)
                            except Exception:
                            ...
                            try:
                                url = getattr(
                                    getattr(self.bot.coin_list, i["to_coin"]),
                                    "daemon_address",
                                )
                                get_tx = await btc_get_tx_info(
                                    url, check_id["payoutHash"]
                                )
                            ...
                            rpchost = getattr(
                                getattr(self.bot.coin_list, coin_family), "rpchost"
                            )
                            net_name = getattr(
                                getattr(self.bot.coin_list, coin_family), "net_name"
                            )
                            confirmation_depth = getattr(
                                getattr(self.bot.coin_list, i["to_coin"]),
                                "deposit_confirm_depth",
                            )
                            get_tx = await tezos_get_tx(
                                rpchost, check_id["payoutHash"], 32
                            )
                            if get_tx is None:
                                continue
                            else:
                                height = await self.wallet_api.get_block_height(
                                    coin_family, "XTZ", net_name
                                )
                                if (
                                    height
                                    and height - get_tx["level"] > confirmation_depth
                                ):
                                    status_tx = "CONFIRMED"
                            ...
                            main_address = self.bot.config["eth"]["MainAddress"]
                            url = self.bot.erc_node_list[network]
                            get_tx = await erc20_get_tx_info(
                                url, check_id["payoutHash"]
                            )
                            if get_tx is None:
                                print(
                                    "Nanswap fetches erc20 tx received None for hash: {}, url: {}".format(
                                        check_id["payoutHash"], url
                                    )
                                )
                                continue
                            else:
                                if (
                                    ...
                                    top_block = await erc20_get_block_number(
                                        url, timeout=30
                                    )
                        ...
                        sell_coin_emoji = getattr(
                            getattr(self.bot.coin_list, i["from_coin"]),
                            "coin_emoji_discord",
                        )
                        for_coin_emoji = getattr(
                            getattr(self.bot.coin_list, i["to_coin"]),
                        ...
                        if (
                            crediting is True
                            and i["user_id"].isdigit
                            and i["user_server"] == SERVER_BOT
                        ):
