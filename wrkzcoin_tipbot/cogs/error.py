...
class Error(commands.Cog):
    ...
    async def on_slash_command_error(self, ctx: disnake.ext.commands.Context, error):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            embed = disnake.Embed(
                title="Error!",
                description="You appear to be missing a required argument!",
                color=disnake.Color.red(),
            )
            embed.add_field(
                name="Missing argument", value=f"`{error.args[0]}`", inline=False
            )
            embed.add_field(
                name="Command Usage", value=f"`{ctx.command.usage}`", inline=False
            )
            if ctx.command.aliases:
                aliases = "`" + "".join(f"!{c}, " for c in ctx.command.aliases) + "`"
                embed.add_field(
                    name="Command Aliases", value=f"{aliases}", inline=False
                )
            return await ctx.response.send_message(
                embed=embed, view=RowButtonRowCloseAnyMessage()
            )

        if isinstance(error, commands.BadArgument):
            embed = disnake.Embed(
                title="Error!",
                description="An argument you entered is invalid!",
                color=disnake.Color.red(),
            )
            embed.add_field(
                name="Bad Argument", value=f"`{error.args[0]}`", inline=False
            )
            embed.add_field(
                name="Command Usage", value=f"`{ctx.command.usage}`", inline=False
            )
        ...
        if isinstance(error, disnake.ext.commands.errors.ExtensionNotLoaded):
            embed = disnake.Embed(
                title="Error!", description="Cog not found!", color=disnake.Color.red()
            )
            embed.add_field(
                name="Bad Argument", value=f"`{error.args[0]}`", inline=False
            )
    ...
    async def on_command_error(self, ctx: disnake.ext.commands.Context, error):
        ...
        if isinstance(error, commands.NoPrivateMessage):
            await ctx.reply(
                f"{ctx.author.mention} This command cannot be used in private messages."
            )
