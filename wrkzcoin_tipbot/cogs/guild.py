...
class Guild(commands.Cog):
    ...
    async def get_faucet_claim_user_guild(
        self, userId: str, guild_id: str, user_server: str = "DISCORD"
    ):
        try:
            ...
            async with store.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    await cur.execute(
                        sql, (guild_id, userId, "GUILDFAUCET", user_server)
                    )
        ...
        try:
            ...
            if len(list_activedrop) > 0:
                for each_drop in list_activedrop:
                    ...
                    # check last drop
                    last_drop = await self.get_last_activedrop_guild(
                        each_drop["serverid"]
                    )
                    role = None
                    if last_drop is None or (
                        last_drop is not None
                        and int(time.time()) - last_drop["spread_time"]
                        >= each_drop["tiptalk_duration"]
                    ):
                        ...
                        message_talker = await store.sql_get_messages(
                            each_drop["serverid"],
                            each_drop["tiptalk_channel"],
                            each_drop["tiptalk_duration"] + additional_time,
                            None,
                        )
                        msg = ""
                        msg_no_embed = ""
                        ...
                        if len(message_talker) == 0:
                            ...
                            if each_drop["tiptalk_role_id"] is not None:
                                role = disnake.utils.get(
                                    get_guild.roles,
                                    id=int(each_drop["tiptalk_role_id"]),
                                )
                            ...
                            if actual_balance < float(each_drop["tiptalk_amount"]):
                                msg = (
                                    f"Guild {get_guild.name} runs out of {coin_name}'s balance. "
                                ...
                                for member_id in message_talker:
                                    try:
                                        ...
                                        if (
                                            member
                                            and member in get_guild.members
                                            and role
                                            and hasattr(member, "roles")
                                            ...
                                            try:
                                                ...
                                                print(
                                                    "Failed creating wallet for activedrop for userid: {}".format(
                                                        member_id
                                                    )
                                                )
                                    except Exception:
                                        traceback.print_exc(file=sys.stdout)
                                        await logchanbot(
                                            "guild " + str(traceback.format_exc())
                                        )
                                if len(list_receivers) == 0:
                                    ...
                                    equivalent_usd = ""
                                    amount_in_usd = 0.0
                                    amount = each_drop["tiptalk_amount"] / len(
                                        list_receivers
                                    )
                                    if price_with:
                                    ...
                                    try:
                                        # re-check last drop
                                        last_drop_recheck = (
                                            await self.get_last_activedrop_guild(
                                                each_drop["serverid"]
                                            )
                                        )
                                        if (
                                            last_drop_recheck is not None
                                            and int(time.time())
                                            - last_drop_recheck["spread_time"]
                                            < 60
                                        ):
                                            continue
                                        # add to DB
                                        await self.insert_new_activedrop_guild(
                                            ...
                                            / len(list_receivers),
                                            len(list_receivers),
                                            json.dumps(list_receivers),
                                            json.dumps(list_receiver_names),
                                            int(time.time()),
                                        )
                                        ...
                                        list_mentioned = [
                                            f"<@{each}>" for each in list_receivers
                                        ]
                                        msg = (
                                            ", ".join(list_mentioned)
                                            + f" active talker(s) in the last {lap_str}."
                                        )
                                        each_msg = "each"
                                        ...
                                        msg_no_embed = ", ".join(
                                            list_receiver_names
                                        ) + " got {} {} {}. Next drop in {}.".format(
                                            num_format_coin(
                                                each_drop["tiptalk_amount"]
                                                / len(list_receivers)
                                                if len(list_receivers) > 0
                                                else each_drop["tiptalk_amount"]
                                            ),
                                            coin_name,
                                            each_msg,
                                        ...
                                        if len(msg) > 999:
                                            ...
                                            msg_no_embed = (
                                                msg
                                                + " Each got {} {}. Next drop in {}. "
                                                "You can disable it by /tiptalker and set amount 0.".format(
                                                    num_format_coin(
                                                        each_drop["tiptalk_amount"]
                                                        / len(list_receivers)
                                                        if len(list_receivers) > 0
                        ...
                        if len(msg) > 0:
                            ...
                            embed = disnake.Embed(
                                ...
                                description="Keep on chatting in <#{}>".format(
                                    each_drop["tiptalk_channel"]
                                ),
                            ...
                            embed.add_field(
                                ...
                                value="{}{} {}".format(
                                    coin_emoji,
                                    num_format_coin(each_drop["tiptalk_amount"]),
                                    coin_name,
                                ),
                                inline=False,
                            )
                            ...
                            embed.add_field(
                                name="NEXT DROP",
                                value="<t:{}:f>".format(
                                    int(time.time()) + each_drop["tiptalk_duration"]
                                ),
                                inline=False,
                            )
                            if len(coin_emoji) > 0:
                                ...
                                link = (
                                    ...
                                    + str(split_id.replace(">", "")).strip()
                                    + extension
                                )
                                embed.set_thumbnail(url=link)
                            embed.set_footer(
                                text="You can disable it by /tiptalker and set amount 0."
                            )
        ...
        try:
            ...
            if get_all_active_raffle and len(get_all_active_raffle) > 0:
                for each_raffle in get_all_active_raffle:
                    key = "guild_raffle_{}_{}".format(
                        each_raffle["guild_id"], each_raffle["id"]
                    )
                    try:
                        if self.ttlcache[key] == key:
                            continue  # next
                    ...
                    try:
                        if each_raffle["status"] == "OPENED":
                            if each_raffle["ending_ts"] - to_close_fromopen < int(
                                time.time()
                            ):
                                # less than 3 participants, cancel
                                list_raffle_id = await self.raffle_get_from_by_id(
                                    each_raffle["id"], SERVER_BOT, None
                                )
                                if (
                                    list_raffle_id
                                    and list_raffle_id["entries"]
                                    and len(list_raffle_id["entries"]) < 3
                                ) or (
                                    ...
                                    serverinfo = self.bot.other_data["guild_list"].get(
                                        each_raffle["guild_id"]
                                    )
                                    if serverinfo["raffle_channel"]:
                                        raffle_chan = self.bot.get_channel(
                                            int(serverinfo["raffle_channel"])
                                        )
                                        if raffle_chan:
                                    ...
                                    if (
                                        each_raffle["id"]
                                        in self.raffle_opened_to_ongoing
                                    ):
                                        continue
                                    else:
                                        self.raffle_opened_to_ongoing.append(
                                            each_raffle["id"]
                                        )
                                else:
                                    ...
                                    if update_status:
                                        msg_raffle = "Changed raffle #{} status to **ONGOING** in guild {}/{}! ".format(
                                            each_raffle["id"],
                                            each_raffle["guild_name"],
                                            each_raffle["guild_id"],
                                        )
                                        msg_raffle += (
                                            "Raffle will start in **{}**".format(
                                                seconds_str(to_close_fromopen)
                                            )
                                        )
                                        serverinfo = self.bot.other_data[
                                            "guild_list"
                                        ].get(each_raffle["guild_id"])
                                        if serverinfo["raffle_channel"]:
                                            raffle_chan = self.bot.get_channel(
                                                int(serverinfo["raffle_channel"])
                                            )
                                            if raffle_chan:
                                                ...
                                                try:
                                                    # Ping users
                                                    list_ping = []
                                                    for each_user in list_raffle_id[
                                                        "entries"
                                                    ]:
                                                        list_ping.append(
                                                            "<@{}>".format(
                                        ...
                                        await logchanbot(
                                            f"Internal error to {msg_raffle}"
                                        )
                            ...
                            if each_raffle["ending_ts"] < int(time.time()):
                                ...
                                if (
                                    ...
                                    won_amounts.append(
                                        float(total_reward) * self.raffle_1st_winner
                                    )
                                    ...
                                    won_amounts.append(
                                        float(total_reward) * self.raffle_2nd_winner
                                    )
                                    ...
                                    won_amounts.append(
                                        float(total_reward) * self.raffle_3rd_winner
                                    )
                                    list_losers.remove(user_entries_id[winner_3])
                                    won_amounts.append(
                                        float(total_reward) * self.raffle_pot_fee
                                    )
                                    # channel_id = RAFFLE
                                    update_status = await self.raffle_update_id(
                                        each_raffle["id"],
                                    ...
                                    embed.set_footer(
                                        text="Raffle for {} by {}".format(
                                            each_raffle["guild_name"],
                                            each_raffle["created_username"],
                                        )
                                    )
    ...
    async def raffle_update_id(
        ...
        list_amounts=None,
        list_entries=None,
        amount_each: float = None,
        coin_decimal: int = None,
            ...
            async with store.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    if list_winner is None and list_amounts is None:
                        sql = (
                            """ UPDATE `guild_raffle` SET `status`=%s WHERE `id`=%s """
                        )
                        ...
                        if (
                            ...
                            for each_loser in list_entries:
                                update_status.append(
                                    ("LOST", amount_each, raffle_id, each_loser)
                                )
                            ...
                            for item in list_entries + [
                                ...
                                values_list.append(
                                    (
                                        ...
                                        currentTs,
                                        user_server,
                                        float(amount_each) * float(unit_price_usd),
                                        item,
                                        coin.upper(),
                                        user_server,
                                        -amount_each,
    ...
    async def raffle_get_from_by_id(
        self, idx: str, user_server: str = "DISCORD", user_check: str = None
    ):
        user_server = user_server.upper()
        try:
            await store.openConnection()
    ...
    async def raffle_get_from_guild(
        self, guild: str, last_play: bool = False, user_server: str = "DISCORD"
    ):
        user_server = user_server.upper()
        try:
            await store.openConnection()
                ...
                async with conn.cursor() as cur:
                    ...
                    if result and len(result) > 0:
                        for each_guild in result:
                            ...
                            actual_balance = float(userdata_balance["adjust"])
                            if actual_balance < 10 * float(
                                each_guild["vote_reward_amount"]
                            ):
                                amount = 10 * float(each_guild["vote_reward_amount"])
                                # Disable it
                                # Process, only guild owner can process
                                update_reward = await self.update_reward(
                                    each_guild["serverid"],
                                    actual_balance,
                                    coin_name,
                                    True,
                                ...
                                if update_reward > 0:
                                    try:
                                        guild_found = self.bot.get_guild(
                                            int(each_guild["serverid"])
                                        )
                                        user_found = self.bot.get_user(
                                            guild_found.owner.id
                                        )
                                    ...
                                    await self.vote_logchan(
                                        f"[{SERVER_BOT}] Disable vote reward for {guild_found.name} / "
    ...
    async def guild_update_featurerole(
        ...
        guild_id: str,
        role_id: str,
        faucet_multipled_by: float,
        guild_vote_multiplied_by: float,
        faucet_cut_time_percent: float,
        updated_by_uid: str,
        updated_by_uname: str,
    ):
        try:
            await store.openConnection()
            async with store.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    await cur.execute(
                        ...
                        (
                            ...
                            role_id,
                            faucet_multipled_by,
                            guild_vote_multiplied_by,
                            faucet_cut_time_percent,
                            updated_by_uid,
                            updated_by_uname,
                            int(time.time()),
                        ),
    ...
    async def vote_logchan(self, content: str):
        try:
            webhook = AsyncDiscordWebhook(
                url=self.bot.config["discord"]["vote_webhook"], content=content
            )
    ...
    async def guild_insert_key(
        self, guild_id: str, key: str, secret: str, update: bool = False
    ):
        try:
            await store.openConnection()
            async with store.pool.acquire() as conn:
            ...
            async with store.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    if disable is True:
                        ...
                        """
                        await cur.execute(
                            sql, (amount, coin_name.upper(), channel, guild_id)
                        )
                        await conn.commit()
                        return cur.rowcount
        except Exception:
            ...
            async with store.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    if disable is True:
                        ...
                        await cur.execute(
                            sql,
                            (amount, coin_name.upper(), channel, duration, guild_id),
                        )
                        await conn.commit()
                        return cur.rowcount
        except Exception:
            ...
            async with store.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    await cur.execute(
                        ...
                        (
                            amount,
                            coin_name.upper() if coin_name else None,
                            channel,
                            duration,
                            role_id,
                            guild_id,
    ...
    async def insert_new_activedrop_guild(
        ...
        each_amount: float,
        numb_receivers: int,
        list_receivers_id: str,
        list_receivers_name: str,
        spread_time: int,
    ):
        try:
            await store.openConnection()
    ...
    @commands.slash_command(
        dm_permission=False, description="Various guild's commands."
    )
    ...
    async def guild_create_raffle(self, ctx, amount: float, coin: str, duration: str):
        ...
        if serverinfo["raffle_channel"]:
            ...
            if not raffle_chan:
                await ctx.edit_original_message(
                    content=f"{EMOJI_RED_NO} {ctx.author.mention}, can not find raffle channel or invalid."
                )
                return
        else:
            await ctx.edit_original_message(
                content=f"{EMOJI_RED_NO} {ctx.author.mention}, there is no raffle channel yet."
            )
            return
        ...
        if enable_raffle != 1:
            await ctx.edit_original_message(
                content=f"{ctx.author.mention}, **{coin_name}** not available for raffle."
            )
            return
        ...
        try:
            num_online = len(
                [
                    ...
                    if member.bot == False and member.status != disnake.Status.offline
                ]
            )
            if num_online < self.raffle_min_useronline:
                msg = (
                    f"{EMOJI_RED_NO} {ctx.author.mention}, your guild needs to have at least: "
        ...
        if amount < min_tip or amount > max_tip:
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, amount has to be between "
        ...
        get_raffle = await self.raffle_get_from_guild(
            str(ctx.guild.id), False, SERVER_BOT
        )
        if get_raffle and get_raffle["status"] not in ["COMPLETED", "CANCELLED"]:
            ...
            try:
                if "D" in duration and "H" in duration:
                    ...
                    duration_in_s = (
                        int(duration.replace("D", "")) * 3600 * 24
                    )  # convert to second
                elif "H" in duration:
                    duration_in_s = (
                        int(duration.replace("H", "")) * 3600
                    )  # convert to second
            except ValueError:
            ...
            try:
                ...
                try:
                    ...
                    traceback.print_exc(file=sys.stdout)
                    await logchanbot(
                        f"Failed to message raffle creation in guild {ctx.guild.name} / {ctx.guild.id} "
                    )
            except Exception:
                traceback.print_exc(file=sys.stdout)

    @guild_create_raffle.autocomplete("coin")
    async def createraffle_token_name_autocomp(
    ...
    async def guild_raffle(self, ctx, subc: str = None):
        msg = f"{EMOJI_INFORMATION} {ctx.author.mention}, loading..."
        await ctx.response.send_message(msg)
        ...
        if serverinfo["raffle_channel"] is not None:
            ...
            if raffle_chan is None:
                ...
                return
            elif raffle_chan is not None and raffle_chan.id != int(
                serverinfo["raffle_channel"]
            ):
                await ctx.edit_original_message(
                    content=f"{EMOJI_RED_NO} {ctx.author.mention}, please execute in the assigned channel {raffle_chan.mention}."
                )
                return
        else:
            await ctx.edit_original_message(
        ...
        if get_raffle:
            list_raffle_id = await self.raffle_get_from_by_id(
                get_raffle["id"], SERVER_BOT, str(ctx.author.id)
            )
        subc_list = ["INFO", "LAST", "JOIN", "CHECK", "CANCEL"]
        if subc not in subc_list:
            await ctx.edit_original_message(
                content=f"{EMOJI_RED_NO} {ctx.author.mention}, invalid sub-command {subc}!"
            )
            return
        else:
            if get_raffle is None:
                await ctx.edit_original_message(
                    content=f"{EMOJI_RED_NO} {ctx.author.mention}, there is no information of current raffle yet!"
                )
                return
        try:
            if ctx.author.bot is True:
                await ctx.edit_original_message(
                    content=f"{EMOJI_RED_NO} {ctx.author.mention}, Bot is not allowed!"
                )
                return
        except Exception:
            pass
        ...
        if subc == "INFO":
            try:
                ...
                embed = disnake.Embed(
                    title="RAFFLE #{} / {}".format(get_raffle["id"], ctx.guild.name),
                    timestamp=ending_ts,
                )
                embed.add_field(
                    name="ENTRY FEE",
                    value="{} {}".format(
                        num_format_coin(get_raffle["amount"]), coin_name
                    ),
                    inline=True,
                )
                create_ts = datetime.fromtimestamp(
                    int(get_raffle["created_ts"])
                ).strftime("%Y-%m-%d %H:%M:%S")
                create_ts_ago = str(
                    timeago.format(create_ts, datetime.fromtimestamp(int(time.time())))
                )
                embed.add_field(name="CREATED", value=create_ts_ago, inline=True)
                if list_raffle_id and list_raffle_id["entries"]:
                    embed.add_field(
                        name="PARTICIPANTS",
                        value=len(list_raffle_id["entries"]),
                        inline=True,
                    )
                    if 0 < len(list_raffle_id["entries"]) < 20:
                        ...
                        embed.add_field(
                            ...
                            value=", ".join(list_ping),
                            inline=False,
                        )
                    embed.add_field(
                        name="RAFFLE JAR",
                        value=num_format_coin(
                            len(list_raffle_id["entries"]) * float(get_raffle["amount"])
                        )
                        + " "
                        + coin_name,
                        inline=True,
                ...
                if get_raffle["status"] in ["OPENED", "ONGOING"]:
                    if int(get_raffle["ending_ts"]) - int(time.time()) < 0:
                        embed.add_field(
                            name="WHEN", value="(ON QUEUE UPDATING)", inline=False
                        )
                    else:
                        embed.add_field(
                            name="WHEN",
                            value=seconds_str_days(
                                int(get_raffle["ending_ts"]) - int(time.time())
                            ),
                            inline=False,
                        )
                embed.set_footer(
                    text="Raffle for {} by {}".format(
                        ctx.guild.name, get_raffle["created_username"]
                    )
                )
            ...
            if get_raffle is None:
                ...
                if get_raffle["status"] != "OPENED":
                    ...
                    # Cancel game
                    await self.raffle_cancel_id(get_raffle["id"])
                    msg_raffle = "Cancelled raffle #{} in guild {}: Requested by {}#{}. User entry fee refund!".format(
                        get_raffle["id"],
                        get_raffle["guild_name"],
                        ctx.author.name,
                        ctx.author.discriminator,
                    )
                    serverinfo = self.bot.other_data["guild_list"].get(
                    ...
                    if get_raffle["status"] != "OPENED":
                        ...
                        if list_raffle_id and list_raffle_id["user_joined"]:
                            ...
                            if actual_balance < get_raffle["amount"]:
                                fee_str = (
                                    num_format_coin(get_raffle["amount"])
                                    + " "
                                    + coin_name
                                )
                                having_str = (
                                    num_format_coin(actual_balance) + " " + coin_name
                                )
                            ...
                            try:
                                ...
                                if inserting is True:
                                    ...
                                    msg = (
                                        f"{ctx.author.mention}, successfully registered your Entry for raffle "
                                        f"#**{raffle_id}** in {ctx.guild.name}! {note_entry}"
                                    )
                                    await ctx.edit_original_message(content=msg)
                                else:
                                    await ctx.edit_original_message(
                                        content=f"{EMOJI_RED_NO} {ctx.author.mention}, internal error. Please report!"
                                    )
                            except Exception:
                                traceback.print_exc(file=sys.stdout)
                            ## remove QUEUE: reply
    ...
    @guild.sub_command(
        name="balance", usage="guild balance", description="Show guild's balance"
    )
    async def balance(self, ctx):
                ...
                if total_balance > 0:
                    ...
                    try:
                        if (
                            ...
                            coin_emojis[coin_name] = getattr(
                                getattr(self.bot.coin_list, coin_name),
                                "coin_emoji_discord",
                            )
                            coin_emojis[coin_name] = (
                                coin_emojis[coin_name] + " "
                                if coin_emojis[coin_name]
                                else ""
                            )
                    except Exception:
                        traceback.print_exc(file=sys.stdout)
                    if price_with:
                        ...
                        if per_unit and per_unit["price"] and per_unit["price"] > 0:
                            per_unit = per_unit["price"]
                            coin_balance_usd[coin_name] = float(
                                Decimal(total_balance) * Decimal(per_unit)
                            )
                            total_all_balance_usd += coin_balance_usd[coin_name]
                            if coin_balance_usd[coin_name] >= 0.01:
                                coin_balance_equivalent_usd[
                                    coin_name
                                ] = " ~ {:,.2f}$".format(coin_balance_usd[coin_name])
                            elif coin_balance_usd[coin_name] >= 0.0001:
                                coin_balance_equivalent_usd[
                                    coin_name
                                ] = " ~ {:,.4f}$".format(coin_balance_usd[coin_name])
            except Exception:
                traceback.print_exc(file=sys.stdout)
        ...
        if has_none_balance is True:
            ...
            if total_all_balance_usd >= 0.01:
                total_all_balance_usd = "Having ~ {:,.2f}$".format(
                    total_all_balance_usd
                )
            elif total_all_balance_usd >= 0.0001:
                total_all_balance_usd = "Having ~ {:,.4f}$".format(
                    total_all_balance_usd
                )
            else:
            ...
            for k, v in coin_balance_list.items():
                ...
                page.add_field(
                    name="{}{}{}".format(
                        coin_emojis[k], k, coin_balance_equivalent_usd[k]
                    ),
                    value="{}".format(v),
                    inline=True,
                )
            ...
            if len(all_pages) == 1:
                await ctx.edit_original_message(
                    content=None, embed=all_pages[0], view=RowButtonCloseMessage()
                )
            else:
                view = None
                try:
                    ...
                    msg = (
                        f"{ctx.author.mention}, internal error when checking {self.bot.config['command_list']['guild_balance']}. "
                        "Try again later.  If problem still persists, contact TipBot dev."
                    )
                    await ctx.edit_original_message(content=msg)
                    traceback.print_exc(file=sys.stdout)
                    await logchanbot(
                        f"[ERROR] /guild balance with {ctx.guild.name} / {ctx.guild.id}"
                    )
    ...
    async def votereward(
        self, ctx, amount: str, coin: str, channel: disnake.TextChannel
    ):
        msg = f"{EMOJI_INFORMATION} {ctx.author.mention}, loading..."
        await ctx.response.send_message(msg)
        ...
        # We assume max reward by max_tip / 10
            ...
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, reward cannot be smaller than "
        ...
        # We assume at least guild need to have 100x of reward or depends on guild's population
            ...
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, your guild needs to have at least 100x "
            ...
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention} you need to have at least {str(population)}x "
            ...
            # Test message
            msg = (
                f"New vote reward set to {num_format_coin(amount)} {token_display}"
            ...
            try:
                ...
                traceback.print_exc(file=sys.stdout)
            update_reward = await self.update_reward(
                str(ctx.guild.id), float(amount), coin_name, False, channel_str
            )
            if update_reward > 0:
                msg = (
                    f"{ctx.author.mention} Successfully set reward for voting in guild {ctx.guild.name} to "
    ...
    async def votereward_token_name_autocomp(
        self, inter: disnake.CommandInteraction, string: str
    ):
        string = string.lower()
        return [name for name in self.bot.coin_name_list if string in name.lower()][:10]
    ...
    async def deposit(self, ctx, amount: str, coin: str):
        ...
        try:
            ...
            if amount > actual_balance:
                msg = (
                    f"{EMOJI_RED_NO} {ctx.author.mention}, insufficient balance to deposit "
                    f"{num_format_coin(amount)} {token_display}."
                )
                await ctx.edit_original_message(content=msg)
                return
                ...
                msg = (
                    f"{EMOJI_RED_NO} {ctx.author.mention}, transaction cannot be smaller than "
                    f"{num_format_coin(min_tip)} {token_display} "
            ...
            if str(ctx.author.id) in self.bot.tipping_in_progress:
                ...
                try:
                    ...
                    if tip:
                        try:
                            msg = (
                                f"{EMOJI_ARROW_RIGHTHOOK} {ctx.author.mention} transferred "
    ...
    async def guilddeposit_token_name_autocomp(
        self, inter: disnake.CommandInteraction, string: str
    ):
        string = string.lower()
        return [name for name in self.bot.coin_name_list if string in name.lower()][:10]
    ...
    async def topgg(self, ctx, resetkey: str = None):
        msg = f"{EMOJI_INFORMATION} {ctx.author.mention}, loading..."
        ...
        if get_guild_by_key is None:
            ...
            insert_key = await self.guild_insert_key(
                str(ctx.guild.id), random_string, secret, False
            )
            if insert_key:
                try:
                    await ctx.edit_original_message(
                ...
                try:
                    await ctx.edit_original_message(
                        content=f"Internal error! Please report!"
                    )
                except Exception:
                    traceback.print_exc(file=sys.stdout)
            ...
            random_string = str(uuid.uuid4())
            insert_key = await self.guild_insert_key(
                str(ctx.guild.id), random_string, secret, True
            )
            if insert_key:
                try:
                    await ctx.edit_original_message(
    ...
    async def async_mdeposit(self, ctx, token: str = None, plain: str = None):
        ...
        if token is None:
            ...
            if not hasattr(self.bot.coin_list, coin_name):
                ...
                if (
                    getattr(getattr(self.bot.coin_list, coin_name), "enable_deposit")
                    == 0
                ):
                    await ctx.edit_original_message(
        ...
        try:
            ...
            if (
                getattr(getattr(self.bot.coin_list, coin_name), "deposit_note")
                and len(getattr(getattr(self.bot.coin_list, coin_name), "deposit_note"))
                > 0
            ):
                description = getattr(
                    getattr(self.bot.coin_list, coin_name), "deposit_note"
                )
            if (
                getattr(getattr(self.bot.coin_list, coin_name), "real_deposit_fee")
                and getattr(getattr(self.bot.coin_list, coin_name), "real_deposit_fee")
                > 0
            ):
            ...
            try:
                await self.wallet_api.generate_qr_address(wallet_address)
                embed.set_thumbnail(
                    url=self.bot.config["storage"]["deposit_url"]
                    + wallet_address
                    + ".png"
                )
            except Exception:
                traceback.print_exc(file=sys.stdout)
            plain_msg = "Guild {} deposit address: ```{}```".format(
                ctx.guild.name, wallet_address
            )
            embed.add_field(
                name="Guild {}".format(ctx.guild.name),
                value="__{}__".format(wallet_address),
                inline=False,
            )
            if (
            ...
            try:
                if plain and plain.lower() == "plain" or plain.lower() == "text":
                    await ctx.edit_original_message(
                        content=plain_msg, view=RowButtonRowCloseAnyMessage()
                    )
                else:
                    await ctx.edit_original_message(
    ...
    async def mdeposit(self, ctx, token: str, plain: str = "embed"):
    ...
    async def faucetclaim(
        self, ctx, amount: str, coin: str, duration: str, channel: disnake.TextChannel
    ):
        msg = f"{EMOJI_INFORMATION} {ctx.author.mention}, loading..."
        await ctx.response.send_message(msg)
        ...
        if duration not in ["4H", "8H", "12H", "24H"]:
            await ctx.edit_original_message(
                content=f"{ctx.author.mention}, accepted duration 4H, 8H, 12H, 24H."
            )
            return
        elif duration == "4H":
            duration_s = 4 * 3600
        ...
        # We assume max reward by max_tip / 10
            ...
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, faucet cannot be smaller than "
                f"{num_format_coin(min_tip)} {token_display} "
                f"or bigger than {num_format_coin(max_tip / 10)} {token_display}."
            )
            await ctx.edit_original_message(content=msg)
        ...
        # We assume at least guild need to have 100x of reward or depends on guild's population
            ...
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, your guild needs to have at least 100x reward balance. "
            ...
            population = len(ctx.guild.members)
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, you need to have at least {str(population)}x reward balance. "
            ...
            # Test message
            msg = (
                f"New guild /faucet set to {num_format_coin(amount)} {token_display} "
                f"by {ctx.author.name}#{ctx.author.discriminator} and message here."
            )
            try:
                await get_channel.send(msg)
            ...
            update_faucet = await self.update_faucet(
                str(ctx.guild.id),
                float(amount),
                coin_name,
                duration_s,
            ...
            if update_faucet > 0:
                msg = (
                    f"{ctx.author.mention}, successfully faucet in guild {ctx.guild.name} to "
    ...
    async def activedrop_token_name_autocomp(
        self, inter: disnake.CommandInteraction, string: str
    ):
        string = string.lower()
        return [name for name in self.bot.coin_name_list if string in name.lower()][:10]
    ...
    async def tiptalker_token_name_autocomp(
        self, inter: disnake.CommandInteraction, string: str
    ):
        string = string.lower()
        return [name for name in self.bot.coin_name_list if string in name.lower()][:10]

    async def async_activedrop(
        self,
        ctx,
        amount: str,
        coin: str,
        ...
        if duration not in ["0.5H", "1H", "2H", "3H", "4H", "5H", "6H", "12H", "24H"]:
            await ctx.edit_original_message(
                content=f"{ctx.author.mention}, accepted duration 0.5H to 6H."
            )
            return
        ...
        if amount.isdigit() and float(amount) <= 0 and serverinfo["tiptalk_amount"] > 0:
            update_tiptalk = await self.update_activedrop(
                str(ctx.guild.id), 0.0, None, None, None, None
            )
            ...
            amount.isdigit()
            and float(amount) <= 0
            and serverinfo["tiptalk_amount"] == 0
        ):
        ...
        # We assume max reward by max_tip / 10
            ...
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, activedrop/tiptalker amount cannot be smaller than "
        ...
        # We assume at least guild need to have 100x of reward or depends on guild's population
            ...
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, your guild needs to have at least 100x `active drop amount`"
            ...
            # Test message
            msg = (
                f"New guild's active drop set to __{num_format_coin(amount)} "
            ...
            if update_tiptalk > 0:
                msg = (
                    f"{ctx.author.mention}, successfully set activedrop/tiptalker in guild {ctx.guild.name} to"
                    f" {num_format_coin(amount)} {token_display} for every {duration}."
                )
                try:
                    if (
                        serverinfo["tiptalk_channel"]
                        and channel_str != serverinfo["tiptalk_channel"]
                    ):
                        msg += " You guild's previous /tiptalk set in channel <#{}> is deleted.".format(
                            serverinfo["tiptalk_channel"]
                        )
                except Exception:
                    traceback.print_exc(file=sys.stdout)
    ...
    async def async_guild_info(self, ctx, private: bool):
        ...
        try:
            ...
            num_bot = len(
                [member for member in ctx.guild.members if member.bot == True]
            )
        ...
        if serverinfo["enable_faucet"] == "YES" and serverinfo["faucet_channel"]:
            embed.add_field(
                name="Faucet {} {}".format(
                    serverinfo["faucet_amount"], serverinfo["faucet_coin"]
                ),
        ...
        if (
            serverinfo["tiptalk_channel"]
            and serverinfo["tiptalk_amount"] > 0
            and serverinfo["tiptallk_coin"]
        ):
            embed.add_field(
                name="TipTalk",
                value="`{} {} @ {}`".format(
                    serverinfo["tiptalk_amount"],
                    serverinfo["tiptallk_coin"],
                    seconds_str(serverinfo["tiptalk_duration"]),
                ),
                inline=True,
            )
        ...
        if serverinfo["mute_tip"]:
            embed.add_field(name="Mute Tip", value=serverinfo["mute_tip"], inline=True)
        if private is True:
        ...
        if (
            ...
            and serverinfo["vote_reward_coin"]
            and serverinfo["vote_reward_channel"]
        ):
            getattr(
                getattr(self.bot.coin_list, serverinfo["vote_reward_coin"]), "decimal"
            )
            vote_links = []
            vote_links.append("https://top.gg/servers/{}/vote".format(ctx.guild.id))
            embed.add_field(
                name="Vote Reward {} {}".format(
                    num_format_coin(serverinfo["vote_reward_amount"]),
                    serverinfo["vote_reward_coin"],
                ),
                value="<#{}> | {}".format(
                    serverinfo["vote_reward_channel"], "\n".join(vote_links)
                ),
                inline=False,
            )
        try:
            if (
                serverinfo["feature_roles"]
                and len(serverinfo["feature_roles"].keys()) > 0
            ):
                list_featureroles = []
    ...
    @commands.bot_has_permissions(send_messages=True)
    @guild.sub_command(usage="guild info", description="Get information about a guild.")
    ...
    async def featurerole(self, ctx):
        pass
    ...
    async def slash_featurerole_list(self, ctx):
        await self.bot_log()
        ...
        try:
            ...
            if serverinfo and serverinfo["enable_featurerole"] != 1:
                ...
                serverinfo
                and serverinfo["feature_roles"] is not None
                and len(serverinfo["feature_roles"].keys()) > 0
            ):
                embed = disnake.Embed(
                    title="Guild {} / {}".format(ctx.guild.name, ctx.guild.id),
    ...
    @featurerole.sub_command(
        ...
        options=[
            ...
            Option(
                "guild_vote_multiplied",
                "Multiplied reward for each guild vote",
                OptionType.string,
                required=True,
                choices=[
                    OptionChoice("1.0x", "1.0"),
            ...
            Option(
                "faucet_cut_time_percent",
                "/faucet cutting time in percentage",
                OptionType.string,
                required=True,
                choices=[
                    OptionChoice("0%", "0"),
    ...
    async def slash_featurerole_add(
        ...
        try:
            ...
            if (
                serverinfo
                and serverinfo["feature_roles"] is not None
                and str(role.id) in serverinfo["feature_roles"]
            ):
                prev_faucet = "{:,.2f}".format(
                    serverinfo["feature_roles"][str(role.id)]["faucet_multipled_by"]
                )
                prev_vote = "{:,.2f}".format(
                    serverinfo["feature_roles"][str(role.id)][
                        "guild_vote_multiplied_by"
                    ]
                )
                prev_cut = "{:,.2f}{}".format(
                    serverinfo["feature_roles"][str(role.id)]["faucet_cut_time_percent"]
                    * 100,
                    "%",
                )
            if serverinfo and serverinfo["enable_featurerole"] != 1:
                ...
                await ctx.edit_original_message(content=msg)
                await logchanbot(
                    f"/featurerole User `{str(ctx.author.id)}` commanded by Guild `{str(ctx.guild.id)}` is not enable."
                )
                return
            # Check if reach max
            if (
                serverinfo
                and serverinfo["feature_roles"]
                and len(serverinfo["feature_roles"].keys()) >= self.max_featurerole
            ):
            ...
            try:
                ...
                if adjust_feature_role is True:
                    msg = (
                        f"{ctx.author.mention}, featurerole `{role.name}` updated.\n"
    ...
    @featurerole.sub_command(
        ...
        options=[Option("role", "role", OptionType.role, required=True)],
        description="Delete a feature role.. (Not deleting role)",
    )
    async def slash_featurerole_delete(self, ctx, role: disnake.Role):
        await self.bot_log()
        msg = f"{ctx.author.mention}, checking your guild's info..."
        await ctx.response.send_message(msg)
        ...
        try:
            check_exist = await self.guild_exist_featurerole(
                str(ctx.guild.id), str(role.id)
            )
            if check_exist is True:
                delete_feature = await self.guild_delete_featurerole(
                    str(ctx.guild.id), str(role.id)
                )
    ...
    @commands.guild_only()
    @commands.slash_command(
        dm_permission=False, usage="faucet", description="Claim guild's faucet."
    )
    async def faucet(self, ctx):
        try:
        ...
        try:
            ...
            if (
                serverinfo
                and serverinfo["faucet_channel"]
                and ctx.channel.id != int(serverinfo["faucet_channel"])
            ):
                try:
        ...
        try:
            if (
                serverinfo["faucet_coin"]
                and serverinfo["faucet_amount"] > 0
                and serverinfo["faucet_duration"] is not None
            ):
                        ...
                        if member.roles and len(member.roles) > 0:
                            for r in member.roles:
                                if str(r.id) in serverinfo["feature_roles"]:
                                    extra_amount = (
                                        amount
                                        * serverinfo["feature_roles"][str(r.id)][
                                            "faucet_multipled_by"
                                        ]
                                        - amount
                                    )
                ...
                if not hasattr(self.bot.coin_list, coin_name):
                    msg = (
                        f"{ctx.author.mention}, **{coin_name}** does not exist with us."
                    )
                    await ctx.edit_original_message(content=msg)
                    return

                get_last_claim = await self.get_faucet_claim_user_guild(
                    str(ctx.author.id), str(ctx.guild.id), SERVER_BOT
                )
                extra_msg = ""
                ...
                if (
                    get_last_claim is not None
                    and int(time.time()) - get_last_claim["date"]
                    < duration - cutting_duration
                ):
                    # last_duration = seconds_str(int(time.time()) - get_last_claim['date'])
                    last_duration = disnake.utils.format_dt(
                        get_last_claim["date"], style="f"
                    )
                    # waiting_time = seconds_str(duration - cutting_duration - int(time.time()) + get_last_claim['date'])
                    waiting_time = disnake.utils.format_dt(
                        duration - cutting_duration + get_last_claim["date"], style="R"
                    )
                    msg = (
                        f"{EMOJI_RED_NO} {ctx.author.mention}, you claimed in this guild "
                    ...
                    try:
                        ...
                        traceback.print_exc(file=sys.stdout)
                    min_tip = getattr(
                        getattr(self.bot.coin_list, coin_name), "real_min_tip"
                    )
                    max_tip = getattr(
                        getattr(self.bot.coin_list, coin_name), "real_max_tip"
                    )
                    price_with = getattr(
                        getattr(self.bot.coin_list, coin_name), "price_with"
                    )
                    ...
                    if (
                        str(ctx.guild.id) in self.bot.tipping_in_progress
                        and int(time.time())
                        - self.bot.tipping_in_progress[str(ctx.guild.id)]
                        < 150
                    ):
                    ...
                    if amount + extra_amount > actual_balance:
                        msg = (
                            f"{EMOJI_RED_NO} {ctx.author.mention}, guild has insufficient balance for "
                            f"{num_format_coin(amount + extra_amount)} {token_display}."
                        )
                        await ctx.edit_original_message(content=msg)
                        return
                    elif (
                        amount + extra_amount < min_tip
                        or amount + extra_amount > max_tip
                    ):
                        msg = (
                            f"{EMOJI_RED_NO} {ctx.author.mention}, transaction cannot be smaller than "
                    ...
                    if price_with:
                        ...
                        if per_unit and per_unit["price"] and per_unit["price"] > 0:
                            per_unit = per_unit["price"]
                            amount_in_usd = float(
                                Decimal(per_unit) * Decimal(amount + extra_amount)
                            )
                            if amount_in_usd > 0.0001:
                                equivalent_usd = " ~ {:,.4f} USD".format(amount_in_usd)
                    if str(ctx.guild.id) not in self.bot.tipping_in_progress:
                        self.bot.tipping_in_progress[str(ctx.guild.id)] = int(
                            time.time()
                        )
                        try:
                            tip = await store.sql_user_balance_mv_single(
                            ...
                            if tip:
                                ...
                                if extra_amount > 0:
                                    extra_msg = (
                                        f" You have a guild's role that give you additional bonus {coin_emoji}**"
                                ...
                                msg = (
                                    f"{EMOJI_ARROW_RIGHTHOOK} {ctx.author.mention} got a faucet of "
                ...
                msg = (
                    f"{EMOJI_RED_NO} {ctx.author.mention}, this guild __{ctx.guild.name}__ has no guild's faucet. "
    ...
    @commands.guild_only()
    @commands.has_permissions(manage_channels=True)
    @commands.slash_command(dm_permission=False, description="Guild setting commands.")
    ...
    @setting.sub_command(
        ...
        options=[Option("coin_list", "coin_list", OptionType.string, required=True)],
    ...
    async def tiponly(self, ctx, coin_list: str):
        ...
        if coin_list in ["ALLCOIN", "*", "ALL", "TIPALL", "ANY"]:
            await store.sql_changeinfo_by_server(
                str(ctx.guild.id), "tiponly", "ALLCOIN"
            )
            if self.enable_logchan:
                await self.botLogChan.send(
            ...
            if contained and len(contained) >= 2:
                ...
                await ctx.edit_original_message(content=msg)
                await store.sql_changeinfo_by_server(
                    str(ctx.guild.id), "tiponly", tiponly_value.upper()
                )
                # re-load guild list
                await self.utils.bot_reload_guilds()
            else:
            ...
            if coin_list not in self.bot.coin_name_list:
                msg = (
                    f"{ctx.author.mention} {coin_list} is not in any known coin we set."
                )
                await ctx.edit_original_message(content=msg)
            else:
                # coin_list is single coin set_coin
                await store.sql_changeinfo_by_server(
                    str(ctx.guild.id), "tiponly", coin_list
                )
                if self.enable_logchan:
                    await self.botLogChan.send(
    ...
    @commands.has_permissions(manage_channels=True)
    @setting.sub_command(
        usage="setting trade", description="Toggle trade enable ON/OFF in your guild"
    )
    async def trade(
        self,
        ctx,
        ...
        if serverinfo and serverinfo["enable_trade"] == "YES":
            await store.sql_changeinfo_by_server(
                str(ctx.guild.id), "enable_trade", "NO"
            )
            if self.enable_logchan:
                await self.botLogChan.send(
            ...
            await store.sql_changeinfo_by_server(
                str(ctx.guild.id), "enable_trade", "YES"
            )
            if self.enable_logchan:
                await self.botLogChan.send(
            ...
            msg = (
                f"{ctx.author.mention} ENABLE TRADE feature in this guild {ctx.guild.name}. "
    ...
    async def memepls(
        ...
        if serverinfo and serverinfo["enable_memepls"] == "YES":
            await store.sql_changeinfo_by_server(
                str(ctx.guild.id), "enable_memepls", "NO"
            )
            if self.enable_logchan:
                await self.botLogChan.send(
            ...
            await store.sql_changeinfo_by_server(
                str(ctx.guild.id), "enable_memepls", "YES"
            )
            if self.enable_logchan:
                await self.botLogChan.send(
    ...
    @commands.has_permissions(manage_channels=True)
    @setting.sub_command(
        usage="setting nsfw", description="Toggle nsfw ON/OFF in your guild"
    )
    async def nsfw(
        self,
        ctx,
        ...
        if serverinfo and serverinfo["enable_nsfw"] == "YES":
            ...
            await store.sql_changeinfo_by_server(
                str(ctx.guild.id), "enable_nsfw", "YES"
            )
            if self.enable_logchan:
                await self.botLogChan.send(
    ...
    @commands.has_permissions(manage_channels=True)
    @setting.sub_command(
        usage="setting game", description="Toggle game ON/OFF in your guild"
    )
    async def game(
        self,
        ctx,
        ...
        if serverinfo and serverinfo["enable_game"] == "YES":
            ...
            await store.sql_changeinfo_by_server(
                str(ctx.guild.id), "enable_game", "YES"
            )
            if self.enable_logchan:
                await self.botLogChan.send(
    ...
    @commands.has_permissions(manage_channels=True)
    @setting.sub_command(
        usage="setting botchan", description="Set bot channel to the commanded channel"
    )
    async def botchan(
        self,
        ctx,
        ...
        if serverinfo["botchan"]:
            try:
                if ctx.channel.id == int(serverinfo["botchan"]):
                    ...
                    # change channel info
                    await store.sql_changeinfo_by_server(
                        str(ctx.guild.id), "botchan", str(ctx.channel.id)
                    )
        ...
        if serverinfo["economy_channel"]:
            try:
                if ctx.channel.id == int(serverinfo["economy_channel"]):
                    ...
                    # change channel info
                    await store.sql_changeinfo_by_server(
                        str(ctx.guild.id), "economy_channel", str(ctx.channel.id)
                    )
                    msg = f"Economy game channel of guild {ctx.guild.name} has set to {ctx.channel.mention}."
                    await ctx.edit_original_message(content=msg)
                    if self.enable_logchan:
                        await self.botLogChan.send(
                            f"{ctx.author.name} / {ctx.author.id} change economy game channel {ctx.guild.name} / {ctx.guild.id} to #{ctx.channel.name}."
                        )
                    # re-load guild list
                    await self.utils.bot_reload_guilds()
            except Exception:
        ...
        if serverinfo["trade_channel"]:
            try:
                if ctx.channel.id == int(serverinfo["trade_channel"]):
                    ...
                    # change channel info
                    update = await store.sql_changeinfo_by_server(
                        str(ctx.guild.id), "trade_channel", str(ctx.channel.id)
                    )
                    ...
                    if update is True:
                        ...
                        try:
                            await self.utils.async_set_cache_kv(
                                "market_guild", str(ctx.guild.id), ctx.channel.id
                            )
                        except Exception:
                            traceback.print_exc(file=sys.stdout)
                    if self.enable_logchan:
    ...
    async def setfaucet(
        ...
        if serverinfo and serverinfo["enable_faucet"] == "YES":
            await store.sql_changeinfo_by_server(
                str(ctx.guild.id), "enable_faucet", "NO"
            )
            if self.enable_logchan:
                await self.botLogChan.send(
                    f"{ctx.author.name} / {ctx.author.id} DISABLE faucet (take) command in their guild {ctx.guild.name} / {ctx.guild.id}"
                )
            ...
            await store.sql_changeinfo_by_server(
                str(ctx.guild.id), "enable_faucet", "YES"
            )
            if self.enable_logchan:
                await self.botLogChan.send(
                    f"{ctx.author.name} / {ctx.author.id} ENABLE faucet (take) command in their guild {ctx.guild.name} / {ctx.guild.id}"
                )
    ...
    async def async_set_gamechan(self, ctx, game):
        ...
        if game is None:
            msg = (
                f"{EMOJI_RED_NO} {ctx.channel.mention} please mention a game name to set game channel for it. Game list: "
            ...
            if game not in self.bot.config["game"]["game_list"]:
                msg = (
                    f"{EMOJI_RED_NO} {ctx.channel.mention} please mention a game name within this list"
                    f": {', '.join(self.bot.config['game']['game_list'])}."
                )
                await ctx.edit_original_message(content=msg)
                return
                ...
                if serverinfo[index_game]:
                    try:
                        if ctx.channel.id == int(serverinfo[index_game]):
                            ...
                            # change channel info
                            await store.sql_changeinfo_by_server(
                                str(ctx.guild.id), index_game, str(ctx.channel.id)
                            )
    ...
    async def gamechan(self, ctx, game: str):
    ...
    async def rolecount(self, ctx):
        ...
        embed.set_footer(
            text=f"Requested by {ctx.author.name}#{ctx.author.discriminator} | /rolecount"
        )
        total_role = 0
