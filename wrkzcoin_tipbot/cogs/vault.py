...
import store
from cogs.utils import Utils, gen_password, num_format_coin
from cogs.wallet import WalletAPI
...
async def http_wallet_getbalance(
    ...
    if contract is None:
        ...
        try:
            ...
            print(
                "http_wallet_getbalance disconnected from url: {} for contract {}".format(
                    url, contract
                )
            )
        except asyncio.TimeoutError:
...
async def estimate_gas_amount_send_all(url: str, from_address: str, to_address: str):
    # should return gas and amount
    try:
        # get balance
        get_balance = await http_wallet_getbalance(url, from_address, None, 5)
...
def send_erc_token(
    ...
    try:
        ...
        return {
            "success": True,
            "msg": "You sent transaction to network tx hash: {}".format(
        ...
        traceback.print_exc(file=sys.stdout)
    return {"success": False, "msg": "Internal error. Please report", "tx": None}
        ...
        if coin_name in ["WRKZ", "DEGO"]:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.delete(
                    wallet_api_url + method, headers=headers, timeout=timeout
                ) as response:
                    if response.status == 200 or response.status == 201:
                        try:
                            ...
                            async with session.put(
                                ...
                                if save_resp.status != 200:
                                    print(
                                        f"internal error during save wallet {coin_name}"
                                    )
                        except Exception:
                            traceback.print_exc(file=sys.stdout)
                        return True
                        ...
                        print(
                            f"internal error during sending transaction of wallet {coin_name}"
                        )
    except Exception:
        traceback.print_exc(file=sys.stdout)
    return False
        ...
        if coin_name in ["WRKZ", "DEGO"]:
            ...
            try:
                if send_all is False:
                    ...
                    payload = {
                        "destinations": [
                            {"amount": int(atomic_amount), "address": ex_address}
                        ],
                ...
                async with aiohttp.ClientSession(
                    ...
                    async with session.post(
                        wallet_api_url + "/json_rpc", json=full_payload, timeout=timeout
                    ) as response:
...
async def bcn_close_wallet(
    ...
    try:
        headers = {"Content-Type": "application/json"}
        json_data_save = {"jsonrpc": "2.0", "id": "0", "method": "store"}
        json_data_close = {"jsonrpc": "2.0", "id": "0", "method": "close_wallet"}
        # save first
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
...
async def bcn_get_height(
    ...
    try:
        if coin_name in ["WRKZ", "DEGO"]:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    wallet_api_url + "/status", headers=headers, timeout=timeout
                ) as response:
                    if response.status == 200:
                        json_resp = await response.json()
                        return json_resp
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            print(
                                f"internal error during get height wallet {coin_name}"
                            )
                            return None
            except Exception:
                traceback.print_exc(file=sys.stdout)
...
async def bcn_get_address_bool(
    ...
    try:
        if coin_name in ["WRKZ", "DEGO"]:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    wallet_api_url + "/addresses", headers=headers, timeout=timeout
                ) as response:
                    if response.status == 200:
                        json_resp = await response.json()
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 201:
                        ...
                        # call save wallet
                        async with session.put(
                            wallet_api_url + "/save", headers=headers, timeout=timeout
                        ) as save_resp:
                            if save_resp.status == 200:
                                return json_resp
                            else:
...
async def vault_confirm_backup(user_id: str, user_server: str, coin_name: str):
    try:
        await store.openConnection()
        async with store.pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                """
                await cur.execute(
                    sql, (int(time.time()), coin_name, user_id, user_server)
                )
                await conn.commit()
                return True
    except Exception:
...
class ConfirmBackup(disnake.ui.View):
    ...
    @disnake.ui.button(
        label="Yes, already backup!", emoji="✅", style=disnake.ButtonStyle.green
    )
    async def confirm(
    ...
    @disnake.ui.button(
        label="No, will do later!", emoji="❌", style=disnake.ButtonStyle.grey
    )
    async def cancel(
        self, button: disnake.ui.Button, inter: disnake.MessageInteraction
    ):
        await inter.response.send_message(
            f"{inter.author.mention}, you need to backup before you can withdraw.",
            delete_after=10.0,
        )
        self.value = False
        self.stop()
...
class DropdownVaultCoin(disnake.ui.StringSelect):
    def __init__(self, ctx, owner_id, bot, embed, list_coins, selected_coin):
        ...
        options = [
            disnake.SelectOption(
                label=each,
                description="Select {}".format(each.upper()),
    ...
    async def callback(self, inter: disnake.MessageInteraction):
        if inter.author.id != self.owner_id:
            await inter.response.send_message(
                f"{inter.author.mention}, that is not your menu!", delete_after=3.0
            )
            return
        else:
            await inter.response.send_message(
                f"{inter.author.mention}, checking wallet ...", ephemeral=True
            )
            # check if user has that coin
            get_a_vault = None
            if self.values[0] is not None:
                get_a_vault = await get_a_user_vault_coin(
                    str(self.owner_id), self.values[0], SERVER_BOT
                )
            ...
            if get_a_vault is None:
                ...
                disable_viewkey = True
                disable_archive = True
                if self.values[0] in ["ETH", "MATIC", "BNB"] + ["WOW", "XMR"] + [
                    "WRKZ",
                    "DEGO",
                ]:
            ...
            self.embed.add_field(
                name="Selected coins", value=self.values[0], inline=False
            )
            coin_setting = None
            if self.values[0] is not None:
            ...
            if get_a_vault is not None:
                self.embed.add_field(
                    name="Address", value=get_a_vault["address"], inline=False
                )
                try:
                    if coin_setting is not None:
                        if self.values[0] in ["WRKZ", "DEGO"]:
                            ...
                            if get_balance is not None:
                                ...
                                self.embed.add_field(
                                    name="Balance", value="N/A", inline=False
                                )
                                disable_withdraw = True
                            ...
                            if get_balance is not None:
                                ...
                                if (
                                    get_balance / 10 ** coin_setting["coin_decimal"]
                                    <= coin_setting["min_withdraw_btn"]
                                ):
                                    if coin_setting["min_withdraw_btn"] > 0:
                                        self.embed.add_field(
                            ...
                            if get_slot is None:
                                ...
                                view = VaultMenu(
                                    ...
                                    inter.author.id,
                                    self.embed,
                                    self.bot.config["vault"]["enable_vault"],
                                    self.values[0],
                                    disable_update,
                                    disable_withdraw,
                                    disable_viewkey,
                            ...
                            if get_balance is not None:
                                self.embed.add_field(
                                    ...
                                    value="Balance: {}\nUnlocked: {}".format(
                                        ...
                                        num_format_coin(
                                            get_balance["result"]["unlocked_balance"]
                                            / 10 ** coin_setting["coin_decimal"]
                                        ),
                                    ),
                                    inline=False,
                                ...
                                try:
                                    wallet_height = await bcn_get_height(
                                        self.values[0], wallet_api_url, None, 30
                                    )
                                    type_coin = "XMR"
                                    netheight = ""
                                    height = await self.wallet_api.get_block_height(
                                        type_coin, self.values[0], None
                                    )
            ...
            if coin_setting and coin_setting["wallet_apps"]:
                self.embed.add_field(
                    name="Wallet App", value=coin_setting["wallet_apps"], inline=False
                )
            view = VaultMenu(
                self.bot,
                self.ctx,
...
class VaultMenu(disnake.ui.View):
    def __init__(
        ...
        disable_create_update: bool = True,
        disable_withdraw: bool = True,
        disable_viewkey: bool = True,
        disable_archive: bool = True,
    ):
    ...
    @disnake.ui.button(
        label="Create/Open", style=ButtonStyle.green, custom_id="vault_update"
    )
    async def btn_vault_update(
        ...
        if interaction.author.id != self.owner_id:
            ...
            await interaction.response.send_message(
                f"{interaction.author.mention}, creating/opening...", ephemeral=True
            )
            inserting = False
            address = ""
            if self.selected_coin in ["WOW", "XMR"]:
                ...
                await self.ctx.edit_original_message(
                    content=None, embed=self.embed, view=disable_view
                )
                ...
                get_a_vault = await get_a_user_vault_coin(
                    str(self.owner_id), self.selected_coin, SERVER_BOT
                )
                if get_a_vault is None:
                    # get coin height
                    height = await self.wallet_api.get_block_height(
                        type_coin, self.selected_coin, None
                    )
                    if height is None:
                        await interaction.edit_original_message(
                    ...
                    path = Path(
                        self.bot.config["vault"][
                            self.selected_coin.lower() + "_wallet_dir"
                        ]
                        + wallet_filename
                    )
                    ...
                    create_wallet = await bcn_get_new_address(
                        coin_name=self.selected_coin,
                        wallet_api_url=wallet_url,
                    ...
                    if create_wallet is not None and type(create_wallet) == dict:
                        inserting = await vault_insert(
                            ...
                            view_key=None,
                            private_key=None,
                            seed=encrypt_string(create_wallet["seed"]),
                            dump=encrypt_string(json.dumps(create_wallet)),
                            height=height,
                            password=encrypt_string(wallet_password),
                        )
                    ...
                    if get_slot is None:
                        ...
                        opening = await bcn_open_wallet(
                            ...
                            SERVER_BOT,
                            wallet_filename,
                            decrypt_string(get_a_vault["password"]),
                            30,
                        )
                        if opening is False:
                            await interaction.edit_original_message(
                    ...
                    self.embed.clear_fields()
                    self.embed.add_field(
                        name="Selected coins", value=self.selected_coin, inline=False
                    )
                    self.embed.add_field(
                        name="Address", value=get_a_vault["address"], inline=False
                    )
                    ...
                    try:
                        ...
                        if get_balance is not None:
                            ...
                            try:
                                wallet_height = await bcn_get_height(
                                    self.selected_coin, wallet_url, None, 30
                                )
                                type_coin = "XMR"
                                netheight = ""
                                height = await self.wallet_api.get_block_height(
                ...
                if coin_setting is None:
                    ...
                    if coin_setting["is_maintenance"] == 1:
                        ...
                        await interaction.edit_original_message(
                            ...
                            "We will increase the capacity and you can check again later!",
                            ephemeral=True,
                        )
                        return
                create_wallet = await bcn_get_new_address(
                    coin_name=self.selected_coin,
                    wallet_api_url=coin_setting["wallet_address"],
                    header=coin_setting["header"],
                    timeout=30,
                )
                if create_wallet is not None:
                    inserting = await vault_insert(
                        ...
                        type_coin,
                        create_wallet["address"],
                        spend_key=encrypt_string(create_wallet["privateSpendKey"]),
                        view_key=coin_setting["view_key"],
                        private_key=None,
                        seed=None,
                        dump=encrypt_string(json.dumps(create_wallet)),
                        height=height,
            ...
            if inserting is True:
                ...
                self.embed.add_field(name="Address", value=address, inline=False)
                self.embed.add_field(
                    name="NOTE",
                    value=self.bot.config["vault"]["note_msg"],
                    inline=False,
                ...
                await interaction.edit_original_message(
                    f"{interaction.author.mention}, successfully created your {self.selected_coin} address! Please View Seed/Key and backup it now!"
                )
                await self.ctx.edit_original_message(
                    content=None, embed=self.embed, view=view
                )
                ...
                await interaction.edit_original_message(
                    f"{interaction.author.mention}, internal error when creating {self.selected_coin} address!"
                )
                await self.ctx.edit_original_message(
                    content=None, embed=self.embed, view=view
                )
    ...
    @disnake.ui.button(
        label="Withdraw", style=ButtonStyle.primary, custom_id="vault_withdraw"
    )
            ...
            try:
                ...
                if self.selected_coin in ["ETH", "MATIC", "BNB"]:
                    ...
                    if get_slot is None:
                        ...
                        check_open = await bcn_get_address_bool(
                            self.selected_coin,
                            wallet_url,
                            None,
                            get_a_vault["address"],
                ...
                modal_inter: disnake.ModalInteraction = await self.bot.wait_for(
                    "modal_submit",
                    check=lambda i: i.custom_id == "modal_vault_withdraw"
    ...
    @disnake.ui.button(
        label="View Key/Seed", style=ButtonStyle.gray, custom_id="vault_viewkey"
    )
    async def btn_vault_viewkey(
        ...
        if interaction.author.id != self.owner_id:
            ...
            await interaction.response.send_message(
                f"{interaction.author.mention}, loading data...", ephemeral=True
            )
            get_a_vault = None
            ...
            if get_a_vault is None:
                await interaction.edit_original_message(
                    f"{interaction.author.mention}, internal error when loading your {self.selected_coin} data!"
                )
            else:
                await get_coin_vault_setting(self.selected_coin)
                data = ""
                if self.selected_coin in ["ETH", "MATIC", "BNB"]:
                    ...
                    data = "Address: {}\n".format(get_a_vault["address"])
                    data += "View key: {}\n".format(
                        decrypt_string(get_a_vault["view_key"])
                    )
                    data += "Spend key: {}\n".format(
                        decrypt_string(get_a_vault["spend_key"])
                    )
                elif self.selected_coin in ["WOW", "XMR"]:
                    if self.bot.config["vault"]["disable_monero_coins"] == 1:
                        await interaction.edit_original_message(
                ...
                view = ConfirmBackup()
                msg = (
                    f"{interaction.author.mention}, your {self.selected_coin} data! "
                ...
                if view.value is False:
                    ...
                    if update_backup is True:
                        ...
                        await interaction.edit_original_message(
                            content=f"{EMOJI_INFORMATION} {interaction.author.mention}, thank you for backup! You can run `/vault view` again for update.",
                            view=None,
                        )
                        await log_to_channel(
                            "vault",
                        ...
                        await interaction.edit_original_message(
                            content=f"{EMOJI_INFORMATION} {interaction.author.mention}, internal error. Please report!",
                            view=None,
                        )
                        await self.ctx.delete_original_message()

    @disnake.ui.button(
        label="❗ Archive", style=ButtonStyle.red, custom_id="vault_archive"
    )
    async def btn_vault_archive(
        ...
        if interaction.author.id != self.owner_id:
            ...
            await interaction.response.send_message(
                f"{interaction.author.mention}, checking ...", ephemeral=True
            )
            # Get user wallet
            get_a_vault = None
            if self.selected_coin is not None:
            ...
            if view.value is False:
                ...
                if self.selected_coin in ["ETH", "MATIC", "BNB"]:
                    ...
                    deleting = await vault_archive(
                        ...
                        get_a_vault["type"],
                        get_a_vault["address"],
                        get_a_vault["spend_key"],
                        get_a_vault["view_key"],
                        get_a_vault["private_key"],
                        get_a_vault["seed"],
                        get_a_vault["dump"],
                        get_a_vault["address_ts"],
                        get_a_vault["height"],
                        get_a_vault["confirmed_backup"],
                    ...
                    try:
                        os.rename(
                            ...
                            + wallet_filename,
                            self.bot.config["vault"]["archive_wallet_dir"]
                            + wallet_filename
                            + "_"
                            + str(int(time.time())),
                        )
                ...
                if deleting is True:
                    ...
                    await self.ctx.edit_original_message(
                        content="Wallet deleted. Please execute `/vault` command again.",
                        embed=None,
                        view=None,
                    )
                    await log_to_channel(
...
class Withdraw(disnake.ui.Modal):
    def __init__(
        ...
        super().__init__(
            title=f"Withdraw {self.coin_name}",
            custom_id="modal_vault_withdraw",
    ...
    async def callback(self, interaction: disnake.ModalInteraction) -> None:
        ...
        await interaction.response.send_message(
            content=f"{interaction.author.mention}, checking withdraw ...",
            ephemeral=True,
        )
        ...
        if address == "":
            await interaction.edit_original_message(
                f"{interaction.author.mention}, address can't be empty!"
            )
            return
        ...
        try:
            ...
            if amount.upper() == "ALL" and self.coin_name in ["ETH", "MATIC", "BNB"]:
                ...
                if get_est_amount is None:
                    ...
                    amount = float(
                        get_est_amount["remaining"] / (10**self.coin_decimal)
                    )
                    ...
                    if atomic_amount <= 0:
                        ...
                        return
                    gas = {
                        "gasPrice": get_est_amount["gas_price"],
                        "estimateGas": get_est_amount["estimate_gas"],
                    }
            ...
            if coin_name in ["WRKZ", "DEGO"]:
                ...
                if sending is not None:
                    if send_all is False:
                        ...
                        send_amount = (
                            sum(sending["amount_list"]) / 10**self.coin_decimal
                        )
                    await interaction.edit_original_message(
                ...
                transaction = functools.partial(
                    ...
                    self.seed,
                    address,
                    get_balance / (10**self.coin_decimal),
                    amount,
                    self.coin_decimal,
                    gas,
                )
...
class Vault(commands.Cog):
    ...
    async def vault(self, ctx):
        try:
            await ctx.response.send_message(
                f"{ctx.author.mention}, vault loading ...", ephemeral=True
            )
            ...
            if get_user is None or (get_user and get_user["vault_enable"] == 0):
                msg = (
                    f"{EMOJI_RED_NO} {ctx.author.mention}, you don't have have vault access yet! "
                    f"If you want to enable or test this, please contact TipBot dev or join our supported guild."
                )
                await ctx.edit_original_message(content=msg)
                return
            if (
                self.bot.config["vault"]["disable"] == 1
    ...
    @vault.sub_command(
        name="intro", usage="vault intro", description="Introduction of /vault."
    )
    async def vault_intro(self, ctx):
        try:
        ...
        try:
            ...
            embed.add_field(
                name="BRIEF", value=self.bot.config["vault"]["brief_msg"], inline=False
            )
            embed.add_field(
                name="NOTE", value=self.bot.config["vault"]["note_msg"], inline=False
            )
            embed.set_footer(
    ...
    @vault.sub_command(name="view", usage="vault view", description="View your /vault.")
    async def vault_view(self, ctx):
        try:
            self.bot.commandings.append(
                (
                    str(ctx.guild.id)
        ...
        try:
            ...
            get_user_vaults = await get_a_user_vault_list(
                str(ctx.author.id), SERVER_BOT
            )
            if len(get_user_vaults) == 0:
                embed.add_field(name="Your vault", value="Empty", inline=False)
            else:
                coin_list = [i["coin_name"] for i in get_user_vaults]
                embed.add_field(
                    name="Your vault", value=", ".join(coin_list), inline=False
                )
            embed.add_field(
                name="NOTE", value=self.bot.config["vault"]["note_msg"], inline=False
            )
    ...
    async def check_vault_xmr_occupied(self):
        ...
        try:
            ...
            get_active_proc = await vault_xmr_find_occupied_slot(
                self.bot.config["vault"]["max_duration_occupied"]
            )
                ...
                for i in get_active_proc:
                    try:
                        ...
                        try:
                            ...
                            traceback.print_exc(file=sys.stdout)
                        user_msg = (
                            f"Bot closed your opened wallet for {i['coin_name']} to save resource. "
