...
class GShop(commands.Cog):
        ...
        if hasattr(self, "guild") and hasattr(self.guild, "id"):
            ...
            if len(get_guild_items) > 0:
                return [
                    ...
                    if string.lower() in each["item_id"].lower()
                ]
            else:
            ...
            if len(get_active_orders) > 0:
                for each_order in get_active_orders:
                    ...
                    try:
                        ...
                        if guild is None:
                            await logchanbot(
                                f"[GSHOP] can not find Guild {str(each_order['guild_id'])}."
                            )
                            continue
                        ...
                        if bot_user is None:
                            await logchanbot(
                                f"[GSHOP] can not fetch TipBot's user in Guild {str(each_order['guild_id'])}."
                            )
                        ...
                        if bot_role_dict["manage_roles"] is False:
                            await logchanbot(
                                f"[GSHOP] TipBot doesn't has permission `manage_roles` in Guild `{str(each_order['guild_id'])}`."
                            )
                            continue
                        role = disnake.utils.get(
                            guild.roles, id=int(each_order["role_id"])
                        )
                        if role is None:
                            await logchanbot(
                                f"[GSHOP] can not find role id {str(each_order['role_id'])} in "
                                f"Guild {str(each_order['guild_id'])} / {guild.name}. Set that order to expired!"
                            )
                            expiring = await self.set_expired_role_item(
                                each_order["id"]
                            )
                            continue
                        ...
                        if int(each_order["expired_date"]) < int(time.time()):
                            ...
                            if not role.is_assignable():
                                await logchanbot(
                                    f"[GSHOP] TipBot has no permission to assign role `{role.name}` for order `{each_order['item_id']}` in Guild `{each_order['guild_id']}`."
                                )
                                expiring = await self.set_expired_role_item(
                                    each_order["id"]
                                )
                            ...
                            if member is None:
                                await logchanbot(
                                    f"[GSHOP] can not find member id {str(each_order['ordered_by_uid'])} in Guild {str(each_order['guild_id'])}. Set him/her to expired!"
                                )
                                expiring = await self.set_expired_role_item(
                                    each_order["id"]
                                )
                            ...
                            if member.roles and role not in member.roles:
                                ...
                                await logchanbot(
                                    f"[GSHOP] ERROR to turn expired on for id {str(each_order['id'])}."
                                )
                                continue
                    except Exception:
                        traceback.print_exc(file=sys.stdout)
                ...
                async with conn.cursor() as cur:
                    ...
                    if result:
                        delete_arg = [
                            ...
                            result["created_date"],
                            result["created_by_uid"],
                            result["created_by_uname"],
                            int(time.time()),
                            deleted_by_uid,
                        ]
                        sql = (
        ...
        try:
            ...
            async with store.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    """
                    await cur.execute(
                        sql, (item_id, guild_id, ordered_by_uid, is_expired)
                    )
                    result = await cur.fetchone()
                    if result:
                        return True
    ...
    async def guild_role_ordered(
        ...
        guild_id: str,
        role_id: str,
        acc_real_amount: float,
        token_name: str,
        token_decimal: int,
        ordered_by_uid: str,
        ordered_by_uname: str,
        renewed_date: int,
        expired_date: int,
        is_expired: int,
        buyer: str = None,
    ...
    @commands.slash_command(
        dm_permission=False, name="gshop", description="Guild shop's commands."
    )
    ...
    async def slash_delete(
        self, ctx, item_id: str = commands.Param(autocomplete=autocomplete_item_idx)
    ):
        ...
        try:
            ...
            if serverinfo and serverinfo["enable_role_shop"] == 0:
                msg = (
                    f"{EMOJI_INFORMATION} {ctx.author.mention}, role shop is disable in this Guild. "
        ...
        item_info = await self.get_guild_role_shop_by_item_id(
            item_id, str(ctx.guild.id)
        )
        if item_info is None:
            msg = f"{ctx.author.mention}, item_id `{item_id}` not exist!"
            await ctx.edit_original_message(content=msg)
        else:
            delete_item = await self.delete_guild_role_shop(
                item_id, str(ctx.guild.id), str(ctx.author.id)
            )
            if delete_item is True:
                ...
                await ctx.edit_original_message(content=msg)
                await logchanbot(
                    f"[GSHOP] server {str(ctx.guild.id)}, user `{str(ctx.author.id)}` failed to remove item `{item_id}`."
                )
    ...
    async def slash_buyrole(
        ...
        try:
            ...
            if item_info is None:
                ...
                try:
                    role = disnake.utils.get(
                        ctx.guild.roles, id=int(item_info["role_id"])
                    )
                    if role is None:
                        msg = (
                            f"{EMOJI_RED_NO} {ctx.author.mention}, I can not find role `{role_name}`, "
                        ...
                        msg = (
                            f"{EMOJI_RED_NO} {ctx.author.mention}, role `{role.name}` is no assignable "
                        ...
                        msg = (
                            f"{EMOJI_RED_NO} {ctx.author.mention}, {member.mention} have role `{role.name}` already. "
                    ...
                    if (
                        role_dict["kick_members"] is True
                        or role_dict["ban_members"] is True
                        or role_dict["manage_channels"] is True
                    ):
                ...
                check_item = await self.check_exist_role_ordered(
                    item_id, str(ctx.guild.id), str(member.id), 0
                )
                if check_item is True:
                    # check if user role removed for some reason.
                    if role in member.roles:
                        msg = (
                            f"{member.mention}, item_id `{item_id}` is not expired yet!"
                        )
                        await ctx.edit_original_message(content=msg)
                    else:
                        await member.add_roles(role)
                        msg = (
                            f"{member.mention} still owns item_id `{item_id}` and not expired yet "
                ...
                if amount > actual_balance:
                    msg = (
                        f"{EMOJI_RED_NO} {ctx.author.mention}, insufficient balance to do purchase "
    ...
    async def slash_addrole(
        ...
        if bot_role_dict["manage_roles"] is False:
            msg = (
                f"{ctx.author.mention}, TipBot doesn't has permission `manage_roles`! "
        ...
        try:
            ...
            if not role.is_assignable():
                msg = (
                    f"{EMOJI_RED_NO} {ctx.author.mention}, role `{role_name.name}` is no assignable "
        ...
        try:
            ...
            if (
                serverinfo
                and len(get_guild_items) >= serverinfo["max_role_shop_items"]
                and ctx.author.id != self.bot.config["discord"]["owner_id"]
            ):
                msg = (
                    f"{EMOJI_INFORMATION} {ctx.author.mention}, there are maximum number of "
                ...
                serverinfo is None
                and len(get_guild_items) >= self.max_default_guild_item
                and ctx.author.id != self.bot.config["discord"]["owner_id"]
            ):
                msg = (
                    f"{EMOJI_INFORMATION} {ctx.author.mention}, there are maximum number of role "
                    "items listed already!"
                )
                await ctx.edit_original_message(content=msg)
                await logchanbot(
                    f"[GSHOP] server {str(ctx.guild.id)} has no data in discord_server."
                )
                return
        except Exception:
            traceback.print_exc(file=sys.stdout)
        ...
        if check_exist is True:
            msg = (
                f"{EMOJI_INFORMATION} {ctx.author.mention}, duplicated data `{item_id}`. "
        ...
        if add_listing is True:
            ...
            await ctx.edit_original_message(content=msg)
            await logchanbot(
                f"[GSHOP] new item added `{item_id}` in Guild `{str(ctx.guild.id)} / {ctx.guild.name}`."
            )
            if ctx.author.id != ctx.guild.owner.id:
                await ctx.guild.owner.send(
    ...
    async def slash_rolelist(self, ctx):
        msg = f"{EMOJI_INFORMATION} {ctx.author.mention}, /gshop loading..."
        await ctx.response.send_message(msg)
        try:
            self.bot.commandings.append(
        ...
        if len(get_guild_items) == 0:
            msg = (
                f"{EMOJI_RED_NO} {ctx.author.mention}, there is no role to buy yet! "
                f"Check again later or request Guild's admin."
            )
            await ctx.edit_original_message(content=msg)
        else:
            embed = disnake.Embed(
                title=f"Role Shop List in {ctx.guild.name}", timestamp=datetime.now()
            )
            embed.set_footer(
                text=f"Requested by {ctx.author.name}#{ctx.author.discriminator} | /gshop"
            )
            for each in get_guild_items:
                int(each["duration"] / 3600 / 24)
                coin_emoji = ""
                try:
                    if (
