...
class Quickdrop_Verify(disnake.ui.Modal):
    def __init__(self, ctx, bot, question, answer, from_user_id, msg_id) -> None:
        ...
        super().__init__(
            title=f"Quickdrop Verification! {question}",
            custom_id="modal_quickdrop_verify",
        ...
        try:
            if int(answer) != self.answer:
                ...
                if get_message["collected_by_userid"] is not None:
                    await interaction.edit_original_message(
                        f"{interaction.author.mention}, too late! Already collected!"
                    )
                    return
                # correct
                ...
                if quick:
                    tip = await store.sql_user_balance_mv_single(
                        ...
                        get_message["real_amount"],
                        get_message["token_name"],
                        "QUICKDROP",
                        get_message["token_decimal"],
                        SERVER_BOT,
                        get_message["contract"],
                        get_message["real_amount_usd"],
                        None,
                    )
                    if tip:
                        ...
                        if interaction.author.id not in notifyList:
                            try:
                                ...
                                await interaction.author.send(
                                    "🎉🎉🎉 Congratulation! You collected {} {} in guild `{}`.".format(
                                        num_format_coin(
                                            get_message["real_amount"],
                                            get_message["token_name"],
                                            get_message["token_decimal"],
...
class Events(commands.Cog):
        ...
        try:
            ...
            async with store.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    await cur.execute(
                        sql, ("YES", int(time.time()), message_id, owner_id)
                    )
                    await conn.commit()
                    return True
        except Exception:
    ...
    async def exec_message_tip(self, amount: str, ticker: str, message):
        try:
            ...
            if not hasattr(self.bot.coin_list, coin_name):
                msg = (
                    f"{message.author.mention}, **{coin_name}** does not exist with us."
                )
            ...
            if (
                str(message.author.id) in self.bot.tipping_in_progress
                and int(time.time())
                - self.bot.tipping_in_progress[str(message.author.id)]
                < 150
            ):
            ...
            if amount < min_tip or amount > max_tip:
                msg = (
                    f"{EMOJI_RED_NO} {message.author.mention}, transactions cannot be smaller than **"
                ...
                msg = (
                    f"{EMOJI_RED_NO} {message.author.mention}, insufficient balance to send tip of "
            ...
            if len(user_mentions) == 0 and len(role_mentions) == 0:
                await message.reply(
                    f"{EMOJI_RED_NO} {message.author.mention}, there is no one to tip to."
                )
                return
            ...
            if len(list_users) == 0:
                ...
                if total_amount > max_tip:
                    msg = (
                        f"{EMOJI_RED_NO} {message.author.mention}, total transaction cannot be bigger than "
                    ...
                    msg = (
                        f"{EMOJI_RED_NO} {message.author.mention}, total transaction cannot be smaller than "
                        f"**{num_format_coin(min_tip, coin_name, coin_decimal, False)} {token_display}**."
                    )
                    await message.reply(msg)
                    return
                    ...
                    msg = (
                        f"{EMOJI_RED_NO} {message.author.mention}, insufficient balance to send total "
                ...
                if (
                    ...
                    self.bot.tipping_in_progress[str(message.author.id)] = int(
                        time.time()
                    )
                ...
                if tip is not None:
                    ...
                    try:
                        msg = (
                            f"{EMOJI_ARROW_RIGHTHOOK} tip of **{num_format_coin(total_amount, coin_name, coin_decimal, False)}"
    ...
    async def process_saving_message(self):
        ...
        if len(self.bot.message_list) > 0:
            ...
            try:
                saving = await self.insert_discord_message(
                    list(set(self.bot.message_list))
                )
                if saving > 0:
    ...
    async def update_discord_stats(self):
        ...
        try:
            ...
            uptime_seconds = round(
                (datetime.datetime.now() - self.bot.start_time).total_seconds()
            )
            ...
            async with store.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if result and len(result) > 0:
                        alias_names = {}
                        for each_item in result:
                            alias_names[each_item["alt_name"].upper()] = each_item[
                                "coin_name"
                            ]
    ...
    async def on_message(self, message):
        ...
        try:
            ...
            # If bot's message
                ...
                hasattr(message, "channel")
                and hasattr(message.channel, "id")
                and message.author.bot is True
                and self.bot.user.mentioned_in(message)
                and self.bot.config["discord"]["enable_bot_message_tip"] == 1
            ):
                try:
    ...
    async def on_message_delete(self, message):
        ...
        if (
            ...
            self.is_saving_message = True
            try:
                await self.delete_discord_message(
                    str(message.id), str(message.author.id)
                )
            except Exception:
                traceback.print_exc(file=sys.stdout)
            self.is_saving_message = False
    ...
    async def on_button_click(self, inter):
        ...
        if inter.message.author == self.bot.user and isinstance(
            inter.channel, disnake.DMChannel
        ):
            try:
                ...
                # traceback.print_exc(file=sys.stdout)
                try:
                    _msg: disnake.Message = await inter.channel.fetch_message(
                        inter.message.id
                    )
                    await _msg.delete()
            ...
            inter.message.author == self.bot.user
            and inter.component.custom_id == "close_any_message"
        ):
            try:
                await inter.message.delete()
            except Exception:
            ...
            inter.message.author == self.bot.user
            and inter.component.custom_id == "close_message"
        ):
            get_message = await self.get_discord_bot_message(
                str(inter.message.id), "NO"
            )
            if get_message and get_message["owner_id"] == str(inter.author.id):
                try:
                    await inter.message.delete()
                    await self.delete_discord_bot_message(
            ...
            inter.message.author == self.bot.user
            and inter.component.custom_id == "quickdrop_tipbot"
        ):
            try:
                ...
                if get_message["need_verify"] != 1:
                    await inter.response.send_message(
                        content=f"QuickDrop ID {str(inter.message.id)}: checking...",
                        ephemeral=True,
                    )
                ...
                if get_message and int(get_message["from_userid"]) == inter.author.id:
                    if get_message["need_verify"] != 1:
                        await inter.edit_original_message(
                            content=f"QuickDrop ID {str(inter.message.id)}: You are the owner of the drop!"
                        )
                    else:
                        await inter.response.send_message(
                ...
                if get_message and get_message["collected_by_userid"]:
                    ...
                    if get_message["need_verify"] == 1:
                        try:
                            ...
                            quickdrop_owner = self.bot.get_user(
                        ...
                        try:
                            if str(inter.message.id) not in self.quickdrop_cache:
                                self.quickdrop_cache[
                                    str(inter.message.id)
                                ] = inter.author.id
                            else:
                                await inter.edit_original_message(
            ...
            inter.message.author == self.bot.user
            and inter.component.custom_id == "talkdrop_tipbot"
        ):
            try:
                await inter.response.send_message(
                    content=f"Talkdrop ID {str(inter.message.id)}: checking...",
                    ephemeral=True,
                )
                msg_id = inter.message.id
                ...
                if get_message is None:
                    await inter.edit_original_message(
                        content=f"Talkdrop ID {str(inter.message.id)}: Failed to collect!"
                    )
                    await logchanbot(
                    ...
                    try:
                        ...
                        if key not in self.talkdrop_cache:
                            ...
                            await inter.edit_original_message(
                                content=f"Talkdrop ID {str(inter.message.id)}: too fast or try again later!"
                            )
                            return
                    except Exception:
                        pass
                    ...
                    if get_message["talkdrop_time"] < int(time.time()):
                        await inter.edit_original_message(
                            content=f"Talkdrop ID {str(inter.message.id)}: time passed already, it's ending soon!"
                        )
                        return
                    get_message["channel_id"]
                    if (
                        get_message
                        and int(get_message["from_userid"]) == inter.author.id
                    ):
                        await inter.edit_original_message(
                            content=f"Talkdrop ID {str(inter.message.id)}: You are the Owner of this talkdrop!"
                        )
                        return
                    # Check if he already in
                    checkin = await store.checkin_talkdrop_collector(
                        str(msg_id), str(inter.author.id)
                    )
                    if checkin is True:
                        await inter.edit_original_message(
                            content=f"Talkdrop ID {str(inter.message.id)}: You are already in the list!"
                        )
                        return
                    # If user is not in talk list
                    num_message = await store.talkdrop_check_user(
                        get_message["guild_id"],
                        get_message["talked_in_channel"],
                        str(inter.author.id),
                        get_message["talked_from_when"],
                    )
                    if num_message < get_message["minimum_message"]:
                        ...
                        if added is True:
                            ...
                            time_passed = (
                                int(time.time()) - get_message["talked_from_when"]
                            )
                            owner_displayname = get_message["from_ownername"]
                            embed = disnake.Embed(
                                title="✍️ Talk Drop ✍️",
                                description="You can collect only if you have chatted in channel <#{}> from {} ago.".format(
                                    get_message["talked_in_channel"],
                                    seconds_str_days(time_passed),
                                ),
                                timestamp=datetime.datetime.fromtimestamp(
                                    get_message["talkdrop_time"]
                                ),
                            )

                            time_left = (
                                seconds_str_days(
                                    get_message["talkdrop_time"] - int(time.time())
                                )
                                if int(time.time()) < get_message["talkdrop_time"]
                                else "00:00:00"
                            )
                            embed.set_footer(
                                text=f"Contributed by {owner_displayname} | /talkdrop | Time left: {time_left}"
                            )
                            name_list = []
                            user_tos = []
                            attend_list = await store.get_talkdrop_collectors(
                                str(msg_id)
                            )
                            ...
                            indiv_amount = (
                                get_message["real_amount"] / len(user_tos)
                                if len(user_tos) > 0
                                else get_message["real_amount"]
                            )
                            indiv_amount_str = num_format_coin(
                                indiv_amount, coin_name, coin_decimal, False
                            )
                            embed.add_field(
                            ...
                            try:
                                ...
                                if channel is None:
                                    await logchanbot(
                                        "talkdrop_check: can not find channel ID: {}".format(
                                            get_message["channel_id"]
                                        )
                                    )
                                ...
                                if _msg is None:
                                    await logchanbot(
                                        "talkdrop_check: can not find message ID: {}".format(
                                            get_message["message_id"]
                                        )
                                    )
                                    await asyncio.sleep(5.0)
                            ...
                            await inter.edit_original_message(
                                content=f"Talkdrop ID {str(inter.message.id)}: Internal error!"
                            )
                            await logchanbot(
            ...
            inter.message.author == self.bot.user
            and inter.component.custom_id.startswith("partydrop_tipbot")
        ):
            try:
                await inter.response.send_message(
                    content=f"Party ID {str(inter.message.id)}: checking...",
                    ephemeral=True,
                )
                msg_id = inter.message.id
                ...
                if get_message is None:
                    await inter.edit_original_message(
                        content=f"Party ID {str(inter.message.id)}: Failed to join party!"
                    )
                    await logchanbot(
                    ...
                    if actual_balance < new_amount:
                        await inter.edit_original_message(
                            content=f"Party ID {str(inter.message.id)}: not sufficient balance!"
                        )
                        return

                    owner_displayname = get_message["from_ownername"]
                    get_message["init_amount"]
                    get_message["real_init_amount_usd_text"]
                    # Get list attendant
                    ...
                    if (
                        ...
                        if str(inter.author.id) in self.bot.tipping_in_progress:
                            ...
                            self.bot.tipping_in_progress[str(inter.author.id)] = int(
                                time.time()
                            )
                        increase = await store.update_party_id_amount(
                            str(inter.message.id), new_amount
                        )
                        try:
                        ...
                        if increase is True:
                            await inter.edit_original_message(
                                content=f"Party ID {str(inter.message.id)}: Sucessfully increased amount!"
                            )
                            # Update view
                            embed = disnake.Embed(
                                title=f"🎉 Party Drop 🎉",
                                description="Each click will deduct from your TipBot's balance. Minimum entrance cost: `{} {}`. Party Pot will be distributed equally to all attendees after completion.".format(
                                    num_format_coin(
                            ...
                            time_left = (
                                seconds_str_days(
                                    get_message["partydrop_time"] - int(time.time())
                                )
                                if int(time.time()) < get_message["partydrop_time"]
                                else "00:00:00"
                            )
                            embed.set_footer(
                                text=f"Initiated by {owner_displayname} | /partydrop | Time left: {time_left}"
                            )
                            total_amount = get_message["init_amount"] + new_amount
                            attend_list = await store.get_party_attendant(
                                str(inter.message.id)
                            )
                            if len(attend_list) > 0:
                                name_list = []
                                ...
                                for each_att in attend_list:
                                    name_list.append(
                                        "<@{}> : {} {}".format(
                                            each_att["attendant_id"],
                                            num_format_coin(
                                                each_att["joined_amount"],
                                                coin_name,
                                                coin_decimal,
                                                False,
                                            ),
                            ...
                            embed.add_field(
                                name="Party Pot",
                                value=num_format_coin(
                                    total_amount, coin_name, coin_decimal, False
                                )
                                + " "
                                + coin_name,
                                inline=True,
                    ...
                    if int(time.time()) > get_message["partydrop_time"]:
                        await inter.edit_original_message(
                            content=f"Party ID: {str(inter.message.id)} passed already!"
                        )
            ...
            hasattr(inter, "message")
            and inter.message.author == self.bot.user
            ...
            try:
                ...
                try:
                    msg_id = inter.message.id
                    get_message = await store.get_discord_triviatip_by_msgid(
                        str(msg_id)
                    )
                except Exception:
                    traceback.print_exc(file=sys.stdout)
                ...
                if get_message is None:
                    await inter.response.send_message(
                        content="Failed for Trivia Button Click!"
                    )
                    await logchanbot(
                ...
                if check_if_in:
                    ...
                    if int(time.time()) > get_message["trivia_endtime"]:
                        ...
                        result = "WRONG"
                        if (
                            inter.component.label
                            == get_message["button_correct_answer"]
                        ):
                            result = "RIGHT"
                        ...
                        msg = "You answered to trivia id: {}".format(
                            str(inter.message.id)
                        )
                        await inter.response.defer()
            ...
            try:
                ...
                try:
                    ...
                    traceback.print_exc(file=sys.stdout)
                    original_message = await inter.original_message()
                    get_message = await store.get_discord_mathtip_by_msgid(
                        str(original_message.id)
                    )

                if get_message is None:
                    await inter.response.send_message(
                        content="Failed for Math Tip Button Click!"
                    )
                    await logchanbot(
                ...
                if check_if_in:
                    ...
                    if int(time.time()) > get_message["math_endtime"]:
                        ...
                        # Check if buttun is wrong or right
                        result = "WRONG"
                        if float(inter.component.label) == float(
                            get_message["eval_answer"]
                        ):
                            result = "RIGHT"
            ...
            inter.message.author == self.bot.user
            and inter.component.custom_id.startswith(
                "economy_{}_".format(inter.author.id)
            )
        ):
            if inter.component.custom_id.startswith(
                "economy_{}_eat_".format(inter.author.id)
            ):
                # Not to duplicate
                key = (
                ...
                try:
                    await inter.response.send_message(
                        f"{inter.author.mention}, checking food..."
                    )
                except Exception:
                    return
                name = inter.component.custom_id.replace(
                    "economy_{}_eat_".format(inter.author.id), ""
                )
                db = database_economy(self.bot)
                get_foodlist_guild = await db.economy_get_guild_foodlist(
                    str(inter.guild.id), False
                )
                all_food_in_guild = {}
            ...
            elif inter.component.custom_id.startswith(
                "economy_{}_work_".format(inter.author.id)
            ):
                # Not to duplicate
                key = (
                    inter.component.custom_id
                ...
                try:
                    await inter.response.send_message(
                        f"{inter.author.mention}, checking your work..."
                    )
                except Exception:
                    return
                db = database_economy(self.bot)
                self.bot.queue_game_economy[str(inter.author.id)] = int(time.time())
                get_last_act = await db.economy_get_last_activities(
                    str(inter.author.id), False
                )
                if get_last_act is not None:
                    ...
                    if remaining > 0:
                        msg = (
                            f"{EMOJI_ERROR} {inter.author.mention}, sorry, you are still busy with other activity. "
                ...
                all_work_in_guild = {}
                get_worklist_guild = await db.economy_get_guild_worklist(
                    str(inter.guild.id), False
                )
                if get_worklist_guild and len(get_worklist_guild) > 0:
                    ...
                    for each_work in get_worklist_guild:
                        all_work_in_guild[each_work["work_emoji"]] = each_work[
                            "work_id"
                        ]
                    ...
                    if (
                        get_userinfo["energy_current"] + get_work_id["energy_loss"]
                        > get_userinfo["energy_total"]
                        and get_work_id["energy_loss"] > 0
                    ):
                        add_energy = (
                            get_userinfo["energy_total"]
                            - get_userinfo["energy_current"]
                    ...
                    insert_activity = await db.economy_insert_activity(
                        ...
                        get_work_id["duration_in_second"],
                        coin_name,
                        get_work_id["reward_expense_amount"],
                        get_work_id["reward_expense_amount"] * get_work_id["fee_ratio"],
                        coin_decimal,
                        add_energy,
                        get_work_id["health_loss"],
                    ...
                    if insert_activity:
                        ...
                        task_name = "{} {}".format(
                            get_work_id["work_name"], get_work_id["work_emoji"]
                        )
                        await inter.edit_original_message(
                            content=f"{EMOJI_INFORMATION} {inter.author.mention}, "
                ...
                "economy_{}_item_".format(inter.author.id)
            ):
                # Not to duplicate
                key = (
                    inter.component.custom_id
                ...
                try:
                    await inter.response.send_message(
                        f"{inter.author.mention}, checking your items..."
                    )
                except Exception:
                    return
                ...
                all_item_backpack = {}
                db = database_economy(self.bot)
                get_user_inventory = await db.economy_get_user_inventory(
                    str(inter.author.id)
                )
                nos_items = sum(
                    each_item["numbers"]
                ...
                update_userinfo = await db.economy_item_update_used(
                    str(inter.author.id),
                    all_item_backpack[name],
                    add_energy,
                    add_health,
                )
                using_item = "{} {}".format(
                    get_item_id["item_name"], get_item_id["item_emoji"]
                )
                if update_userinfo:
                    await inter.edit_original_message(
    ...
    async def on_guild_join(self, guild):
        ...
        try:
            ...
            try:
                if len(self.bot.guilds) > 0 and (
                    len(self.bot.guilds) % 10 == 0 or len(self.bot.guilds) % 1111 == 0
                ):
                    ...
                    try:
                        ...
                        botdetails.add_field(
                            name="Bots", value="{:,.0f}".format(num_bots), inline=True
                        )
                        botdetails.add_field(
                            name="Tips",
        ...
        await store.sql_addinfo_by_server(
            str(guild.id),
            guild.name,
            self.bot.config["discord"]["prefixCmd"],
            "WRKZ",
            True,
        )
