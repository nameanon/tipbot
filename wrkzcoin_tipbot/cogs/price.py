...
class Price(commands.Cog):
    ...
    @commands.slash_command(
        name="price",
        usage="price <token>",
        options=[Option("token", "token", OptionType.string, required=True)],
        description="Display Token price.",
    )
    async def cmd_price(self, ctx, token: str):
        await ctx.response.send_message(
            f"{EMOJI_HOURGLASS_NOT_DONE} {ctx.author.mention}, checking price.."
        )
        ...
        if token is None:
            await ctx.edit_original_message(
                content=f"{EMOJI_INFORMATION} {ctx.author.mention}, missing coin/token name."
            )
            return
        ...
        if len(get_gecko) == 0:
            ...
            embed.add_field(
                name="Price (USD)",
                value="{}\n{}".format(
                    unit_price, "<t:{}:f>".format(coin_info["price_time"])
                ),
                inline=True,
            )
            ...
            try:
                get_marketdata = await self.utils.gecko_fetch_marketdata_coin(
                    coin_info["id"]
                )
                get_marketchart = await self.utils.gecko_fetch_marketchart_coin(
                    coin_info["id"]
                )
                if get_marketdata is not None and get_marketchart is not None:
                    ...
                    if checkin:
                        ...
                        if (
                            len(get_marketchart["prices"]) > 30
                            and len(get_marketchart["total_volumes"]) > 30
                        ):
                            # make chart
                            delta = timedelta(minutes=5)
                            chart_name = (
                                round_dt(datetime.now(), delta).strftime(
                                    "%Y-%m-%d-%H_%M"
                                )
                                + "_coingecko_"
                                + coin_info["id"].lower()
                            ...
                            saved_path = (
                                self.bot.config["cg_marketchart"]["static_image_path"]
                                + chart_name
                            )
                            has_chart = False
                            if os.path.exists(saved_path):
                                ...
                                create_chart = functools.partial(
                                    makechart, get_marketchart, saved_path
                                )
                                making_chart = await self.bot.loop.run_in_executor(
                                    None, create_chart
                                )
                                if making_chart is not None and os.path.exists(
                                    saved_path
                                ):
                                    has_chart = True
                            if has_chart is True:
                                embed.set_image(
                                    url=self.bot.config["cg_marketchart"][
                                        "static_image_link"
                                    ]
                                    + chart_name
                                )
                    ...
                    if (
                        ...
                        try:
                            ...
                            embed.add_field(
                                ...
                                value="```{}```👉 <{}>".format(
                                    self.bot.advert_list[0]["content"],
                                    self.bot.advert_list[0]["link"],
                                ),
                                inline=False,
                            )
