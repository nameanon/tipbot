...
from cogs.wallet import WalletAPI, sql_check_minimum_deposit_erc20
...
class Admin(commands.Cog):
            ...
            try:
                if self.pool_local_db_extra is None:
                    self.pool_local_db_extra = await aiomysql.create_pool(
                        host=self.local_db_extra["dbhost"],
                        port=3306,
                        minsize=1,
                        maxsize=2,
        ...
        try:
            ...
            async with store.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    """
                    await cur.execute(
                        sql, (user_id, user_server, int(time.time()), reason)
                    )
                    await conn.commit()
                    return True
        except Exception:
        ...
        try:
            ...
            async with store.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if result:
                        self.local_db_extra = json.loads(
                            decrypt_string(result["value"])
                        )
        except Exception:
            traceback.print_exc(file=sys.stdout)
            await logchanbot(f"admin {str(traceback.format_exc())}")
                ...
                async with conn.cursor() as cur:
                    ...
                    if result and len(result) > 0:
                        ...
                        for each in result:
                            if (
                                each["message_time"]
                                < int(time.time()) - self.old_message_data_age
                            ):
                                data_rows.append(
                                    (
                                        each["id"],
    ...
    async def user_balance_multi(self, user_id: str, user_server: str, coinlist: list):
        try:
            await store.openConnection()
            async with store.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if result_balance:
                        ...
                        if result_airdrop:
                            for i in result_airdrop:
                                ...
                                user_balance_coin[i["token_name"]] -= Decimal(
                                    i["airdrop"]
                                )
                        ...
                        if result_raffle:
                            for i in result_raffle:
                                ...
                                user_balance_coin[i["coin_name"]] -= Decimal(
                                    i["raffle"]
                                )
        ...
        try:
            ...
            async with store.pool.acquire() as conn:
                ...
                try:
                    balance["adjust"] = 0
                    balance["mv_balance"] = (
                        float("%.12f" % mv_balance) if mv_balance else 0
                    )
        ...
        try:
            ...
            TronClient = AsyncTron(
                provider=AsyncHTTPProvider(
                    self.bot.erc_node_list["TRX"], client=_http_client
                )
            )
        ...
        try:
            if ctx.author.id != self.bot.config["discord"]["owner"]:
                await ctx.response.send_message(
                    "You have no permission!", ephemeral=True
                )
            else:
                # let bot post some message (testing) etc.
                await logchanbot(
                    f"[TIPBOT SAY] {ctx.author.id} / {ctx.author.name}#{ctx.author.discriminator} asked to say:\n{text}"
                )
    ...
    @commands.dm_only()
    @commands.group(
        usage="admin <subcommand>", hidden=True, description="Various admin commands."
    )
    async def admin(self, ctx):
        if ctx.invoked_subcommand is None:
            await ctx.reply(f"{ctx.author.mention}, invalid admin command")
    ...
    @commands.is_owner()
    @admin.command(
        hidden=True, usage="admin importaddr", description="Import addresses"
    )
        ...
        try:
            if type_coin == "BTC":
                ...
                if len(get_addresses) > 0:
                    daemon_addresses = await self.wallet_api.call_doge(
                        "listreceivedbyaddress", coin_name, payload="0, true"
                    )
                    ...
                    for i in get_addresses:
                        if i["balance_wallet_address"] in daemon_addresses:
                            print(
                                f"Skip importing {coin_name} - {i['balance_wallet_address']}"
                            )
                            continue
                        try:
                            payload = f""""{decrypt_string(i['privateKey'])}", "{i['user_id']}", true"""
                            importing = await self.wallet_api.call_doge(
                                "importprivkey", coin_name, payload=payload
                            )
                            number += 1
                            print(
                            ...
                            traceback.print_exc(file=sys.stdout)
                    await ctx.reply(
                        f"{ctx.author.mention}, completed importing {str(number)} address(es) for {coin_name}."
                    )
                else:
                    await ctx.reply(
                        f"{ctx.author.mention}, there's no address to import."
                    )
            else:
    ...
    @commands.is_owner()
    @admin.command(
        hidden=True, usage="admin dumpthread", description="Dump all threads"
    )
    ...
    @admin.command(
        hidden=True,
        usage="admin enableuser <user id> <user_server>",
    ...
    async def enableuser(self, ctx, user_id: str, user_server: str = "DISCORD"):
        try:
            ...
            if user_server not in ["DISCORD", "TELEGRAM"]:
                await ctx.reply(
                    f"{ctx.author.mention}, invalid user_server `{user_server}`."
                )
                return
            ...
            if (
                ...
                if deleting is True:
                    ...
                    await ctx.reply(
                        f"{ctx.author.mention}, 🔴 error deleting a blocked user `{user_id}@{user_server}`."
                    )
            return
        except ValueError:
        ...
        try:
            ...
            if (
                ...
                if adding is True:
                    ...
                    await ctx.reply(
                        f"{ctx.author.mention}, 🔴 error adding user `{user_id}@{user_server}`."
                    )
            return
        except ValueError:
            msg = f"{ctx.author.mention}, invalid given user ID."
    ...
    async def updatebalance(self, ctx, coin: str):
        ...
        if not hasattr(self.bot.coin_list, coin_name):
            ...
            msg = await ctx.reply(
                f"{ctx.author.mention}, loading check for **{coin_name}**.."
            )
            try:
                getattr(getattr(self.bot.coin_list, coin_name), "real_min_deposit")
                real_deposit_fee = getattr(
                    getattr(self.bot.coin_list, coin_name), "real_deposit_fee"
                )
                ...
                min_move_deposit = getattr(
                    getattr(self.bot.coin_list, coin_name), "min_move_deposit"
                )
                min_gas_tx = getattr(
                    getattr(self.bot.coin_list, coin_name), "min_gas_tx"
                )
                gas_ticker = getattr(
                    getattr(self.bot.coin_list, coin_name), "gas_ticker"
                )
                move_gas_amount = getattr(
                    getattr(self.bot.coin_list, coin_name), "move_gas_amount"
                )
                erc20_approve_spend = getattr(
                    getattr(self.bot.coin_list, coin_name), "erc20_approve_spend"
                )
    ...
    @commands.is_owner()
    @admin.command(
        hidden=True, usage="admin status <text>", description="set bot's status."
    )
    ...
    @commands.is_owner()
    @admin.command(
        hidden=True, usage="admin ada <action> [param]", description="ADA's action"
    )
    async def ada(self, ctx, action: str, param: str = None):
        action = action.upper()
        if action == "CREATE":

            async def call_ada_wallet(
                url: str, wallet_name: str, seeds: str, number: int, timeout=60
            ):
                try:
                    headers = {"Content-Type": "application/json"}
                    data_json = {
                        "mnemonic_sentence": seeds,
                        "passphrase": self.bot.config["ada"]["default_passphrase"],
                        "name": wallet_name,
                        "address_pool_gap": number,
                    }
                    async with aiohttp.ClientSession() as session:
                        async with session.post(
                            url, headers=headers, json=data_json, timeout=timeout
                        ) as response:
            ...
            if param is None:
                ...
                try:
                    ...
                    seeds = words.split()
                    create = await call_ada_wallet(
                        self.bot.config["ada"]["default_wallet_url"] + "v2/wallets",
                        wallet_name,
                        seeds,
                        number,
                        300,
            ...
            "MOVE",
            "MV",
        }:  # no param, move from all deposit balance to withdraw
            ...
            async def estimate_fee_with_asset(
                url: str, to_address: str, assets, amount_atomic: int, timeout: int = 90
            ):
            ...
            async def send_tx(
                ...
                try:
                    headers = {"Content-Type": "application/json"}
                    data_json = {
                        "passphrase": decrypt_string(passphrase),
                        "payments": [
                            {
                                "address": to_address,
                                "amount": {
                    ...
                    async with conn.cursor() as cur:
                        ...
                        if result and len(result) > 0:
                            ...
                            for each_w in result:
                                if (
                                    each_w["wallet_name"] == "withdraw_ada"
                                    and each_w["is_for_withdraw"] == 1
                                ):
                            ...
                            for each_w in result:
                                if (
                                    each_w["is_for_withdraw"] != 1
                                    and each_w["used_address"] > 0
                                    and withdraw_address is not None
                                ):
                                    ...
                                    try:
                                        ...
                                        if (
                                            ...
                                            assets = []
                                            if (
                                                "available" in fetch_wallet["assets"]
                                                and len(
                                                    fetch_wallet["assets"]["available"]
                                                )
                                                > 0
                                            ):
                                                for each_asset in fetch_wallet[
                                                    "assets"
                                                ]["available"]:
                                                    # Check if they are in TipBot
                                                    for (
                                                        each_coin
                                                    ) in self.bot.coin_name_list:
                                                        if (
                                                            getattr(
                                                                getattr(
                                                                    self.bot.coin_list,
                                            ...
                                            if (
                                                estimate_tx
                                                and "minimum_coins" in estimate_tx
                                                and "estimated_min" in estimate_tx
                                            ):
                            ...
                            msg = (
                                f"{ctx.author.mention}, doesnot have any wallet in DB!"
                            )
                            await ctx.reply(msg)
                            return
            except Exception:
                ...
                try:
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.delete(
                            url_wallet_id, headers=headers, timeout=timeout
                        ) as response:
                    ...
                    async with conn.cursor() as cur:
                        ...
                        if result:
                            ...
                            delete = await call_ada_delete_wallet(
                                self.bot.config["ada"]["default_wallet_url"]
                                + "v2/wallets/"
                                + wallet_id,
                                300,
                            )
                    ...
                    async with conn.cursor() as cur:
                        ...
                        if result and len(result) > 0:
                            ...
                            for each_w in result:
                                list_wallets.append(
                                    "Name: {}, ID: {}".format(
                                        each_w["wallet_name"], each_w["wallet_id"]
                                    )
                                )
                            list_wallets_j = "\n".join(list_wallets)
                            data_file = disnake.File(
    ...
    async def guildlist(self, ctx):
        try:
            list_g = [
                f"{str(g.id)}, {len(g.members)}, {g.name}" for g in self.bot.guilds
            ]
    ...
    async def clearbutton(self, ctx, channel_id: str, msg_id: str):
        try:
            _channel: disnake.TextChannel = await self.bot.fetch_channel(
                int(channel_id)
            )
    ...
    @commands.is_owner()
    @admin.command(
        hidden=True, usage="checkshare", description="run check pool share of CEXSwap"
    )
            ...
            if len(get_poolshares["shares"]) > 0 and len(get_poolshares["pools"]) > 0:
                ...
                for sh in get_poolshares["shares"]:
                    ...
                    if sh["pairs"] not in pools:
                        pools[sh["pairs"]] = {
                            sh["ticker_1_name"]: Decimal(0),
                            sh["ticker_2_name"]: Decimal(0),
                        }
                ...
                for p in get_poolshares["pools"]:
                    if p["pairs"] not in pools:
                        ...
                        diff_1 = (
                            pools[p["pairs"]][p["ticker_1_name"]] - p["amount_ticker_1"]
                        )
                        diff_2 = (
                            pools[p["pairs"]][p["ticker_2_name"]] - p["amount_ticker_2"]
                        )
                        if truncate(abs(diff_1), 6) != Decimal(0) or truncate(
                            abs(diff_2), 6
                        ) != Decimal(0):
                            # check amount
                            checked_lines.append(
                        ...
                        if truncate(abs(diff_1), 6) != Decimal(0) or truncate(
                            ...
                            updating = await self.audit_update_poolshare(
                                diff_1, diff_2, p["pool_id"]
                            )
                            if updating is True:
                                checked_lines.append(
    ...
    @commands.is_owner()
    @admin.command(
        hidden=True, usage="auditshare", description="Audit share of CEXSwap"
    )
    ...
    async def auditlp(self, ctx):
        if ctx.author.id != self.bot.config["discord"]["owner_id"]:
            await logchanbot(
                "⚠️⚠️⚠️⚠️ {}#{} / {} is trying auditlp!".format(
                ...
                if len(get_lp["cexswap_pools"]) > 0:
                    for each in get_lp["cexswap_pools"]:
                        ...
                        if each["ticker_1_name"] not in cexswap_pools:
                            cexswap_pools[each["ticker_1_name"]] = each[
                                "amount_ticker_1"
                            ]
                        else:
                            cexswap_pools[each["ticker_1_name"]] += each[
                        ...
                        if each["ticker_2_name"] not in cexswap_pools:
                            cexswap_pools[each["ticker_2_name"]] = each[
                                "amount_ticker_2"
                            ]
                        else:
                            cexswap_pools[each["ticker_2_name"]] += each[
                ...
                if len(get_lp["cexswap_pools_share"]) > 0:
                    for each in get_lp["cexswap_pools_share"]:
                        ...
                        if each["ticker_1_name"] not in cexswap_pools_share:
                            cexswap_pools_share[each["ticker_1_name"]] = each[
                                "amount_ticker_1"
                            ]
                        else:
                            cexswap_pools_share[each["ticker_1_name"]] += each[
                        ...
                        if each["ticker_2_name"] not in cexswap_pools_share:
                            cexswap_pools_share[each["ticker_2_name"]] = each[
                                "amount_ticker_2"
                            ]
                        else:
                            cexswap_pools_share[each["ticker_2_name"]] += each[
                ...
                if len(get_lp["cexswap_distributing_fee"]) > 0:
                    for each in get_lp["cexswap_distributing_fee"]:
                        ...
                        if each["got_ticker"] not in cexswap_distributing_fee:
                            cexswap_distributing_fee[each["got_ticker"]] = each[
                                "amount"
                            ]
                        else:
                            cexswap_distributing_fee[each["got_ticker"]] += each[
                ...
                for each in coin_list:
                    if each in cexswap_pools:
                        ...
                        result.append(
                            "\nPOOL vs SHARE FOR {}\n   {} - {} = {} {}".format(
                                each,
                                float(truncate(cexswap_pools[each], 4)),
                    ...
                    if cexswap_pools.get(each) is not None:
                        result.append(
                            "DETAIL {}:\n   POOL: {}\n   -(ADD: {} - REMOVE: {} - SOLD {} - FEE {} - FEE LP {})\n   = {} ({:,.2f}{}) GOT: {} {}\n".format(
                                each,
                                pool_amount,
                                truncate(float(cexswap_add.get(each)), 5)
                                if cexswap_add.get(each)
                                else 0,
                                truncate(float(cexswap_remove.get(each)), 5)
                                if cexswap_remove.get(each)
                                else 0,
                                truncate(float(cexswap_sell_logs.get(each)), 5)
                                if cexswap_sell_logs.get(each)
                                else 0,
                                truncate(float(cexswap_distributing_fee.get(each)), 4)
                                if cexswap_distributing_fee.get(each)
                                else 0,
                                truncate(float(cexswaplp.get(each)), 5)
                                if cexswaplp.get(each)
                                else 0,
                                truncate(add_remove, 4),
                                add_remove / float(cexswap_pools.get(each)) * 100,
                                "%",
                                truncate(
                                    float(cexswap_pools.get(each)) - add_remove, 5
                                ),
                                "✅"
                                if truncate(
                                    float(cexswap_pools.get(each)) - add_remove, 5
    ...
    @commands.is_owner()
    @admin.command(
        hidden=True, usage="auditcoin <coin name>", description="Audit coin's balance"
    )
    async def auditcoin(self, ctx, coin: str):
        if ctx.author.id != self.bot.config["discord"]["owner_id"]:
            await logchanbot(
                "⚠️⚠️⚠️⚠️ {}#{} / {} is trying auditcoin!".format(
                    ctx.author.name, ctx.author.discriminator, ctx.author.mention
                )
            )
            return
        ...
        if not hasattr(self.bot.coin_list, coin_name):
            ...
            try:
                ...
                if len(all_user_id) > 0:
                    msg = (
                        f"{ctx.author.mention}, {EMOJI_INFORMATION} **{coin_name}** there are "
                    ...
                    for each_user_id in all_user_id:
                        ...
                        try:
                            if (
                                each_user_id["user_id"].isdigit()
                                and each_user_id["user_server"] == SERVER_BOT
                                and each_user_id["is_discord_guild"] == 0
                            ):
                                ...
                                each_user_id["user_id"].isdigit()
                                and each_user_id["user_server"] == SERVER_BOT
                                and each_user_id["is_discord_guild"] == 1
                            ):
                    ...
                    msg_checkcoin += (
                        "Total balance: "
                        + num_format_coin(sum_balance)
                        + " "
                        + coin_name
                        + "\n"
                    )
                    msg_checkcoin += (
                        "Total user/guild not found (discord): "
                        + str(sum_unfound_user)
                        + "\n"
                    )
                    msg_checkcoin += (
                        "Total balance not found (discord): "
                        + num_format_coin(sum_unfound_balance)
                        + " "
                        + coin_name
                        + "\n"
                    )
                    if len(negative_users) > 0:
                        msg_checkcoin += (
                            "Negative balance: " + str(len(negative_users)) + "\n"
                        )
                    ...
                    # sort by balance
                    list_user_balances = sorted(
                        list_user_balances, key=lambda d: d["balance"], reverse=True
                    )
                    new_list = []
    ...
    @commands.is_owner()
    @admin.command(
        hidden=True, usage="printbal <user> <coin>", description="print user's balance."
    )
    async def printbal(
        self, ctx, member_id: str, coin: str, user_server: str = "DISCORD"
    ):
        coin_name = coin.upper()
    ...
    async def creditlist(self, ctx, *, credit_msg: str = None):
        if ctx.author.id != self.bot.config["discord"]["owner_id"]:
            await logchanbot(
                "⚠️⚠️⚠️⚠️ {}#{} / {} is trying creditlist!".format(
                    ctx.author.name, ctx.author.discriminator, ctx.author.mention
                )
            )
            return
        ...
        if len(ctx.message.attachments) == 0:
            ...
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        str(ctx.message.attachments[0]), timeout=32
                    ) as response:
                        if response.status == 200:
                            res_data = await response.read()
                            res_data = res_data.decode("utf-8")
                            ...
                            for k, v in user_list_msg.items():
                                try:
                                    if k.isdigit():
                                        ...
                                        if member is not None:
                                            await member.send(
                                                "You received tip(s)```{}```from TipBot Admin/OP. Extra message: {}".format(
                                                    "\n".join(v), credit_msg
                                                )
                                            )
                                except Exception:
    ...
    async def credit(
        self, ctx, member_id: str, amount: str, coin: str, user_server: str = "DISCORD"
    ):
        user_server = user_server.upper()
        if ctx.author.id != self.bot.config["discord"]["owner_id"]:
            await logchanbot(
                "⚠️⚠️⚠️⚠️ {}#{} / {} is trying credit!".format(
                    ctx.author.name, ctx.author.discriminator, ctx.author.mention
                )
            )
            return
        ...
        try:
            ...
            if amount > max_tip or amount < min_tip:
                msg = (
                    f"{EMOJI_RED_NO} {ctx.author.mention}, credit cannot be bigger than **"
            ...
            if get_deposit is None:
                ...
                try:
                    ...
                    if tip:
                        msg = (
                            f"[CREDITING] to user {member_id} server {user_server} with amount "
    ...
    async def auto_purge_old_message(self):
        ...
        try:
            ...
            if purged_items > 50:
                print(
                    f"{datetime.now():%Y-%m-%d-%H-%M-%S} auto_purge_old_message: {str(purged_items)}"
                )
        except Exception:
            traceback.print_exc(file=sys.stdout)
    ...
    @commands.is_owner()
    @admin.command(
        hidden=True, usage="baluser <user>", description="Check a user's balances"
    )
    async def baluser(self, ctx, member_id: str, user_server: str = "DISCORD"):
        ...
        try:
            ...
            start = int(time.time())
            userdata_balances = await self.user_balance_multi(
                member_id, user_server, all_names
            )
    ...
    async def pending(self, ctx):
        ...
        embed.add_field(
            name="Pending Tx", value=str(len(self.bot.tipping_in_progress)), inline=True
        )
        ...
        embed.set_footer(
            text=f"Pending requested by {ctx.author.name}#{ctx.author.discriminator}"
        )
        try:
    ...
    async def withdraw(self, ctx, coin: str):
        ...
        if not hasattr(self.bot.coin_list, coin_name):
            ...
            enable_withdraw = getattr(
                getattr(self.bot.coin_list, coin_name), "enable_withdraw"
            )
            new_value = 1
            new_text = "enable"
    ...
    async def partydrop(self, ctx, coin: str):
        ...
        if not hasattr(self.bot.coin_list, coin_name):
            ...
            enable_partydrop = getattr(
                getattr(self.bot.coin_list, coin_name), "enable_partydrop"
            )
            new_value = 1
            new_text = "enable"
    ...
    async def quickdrop(self, ctx, coin: str):
        ...
        if not hasattr(self.bot.coin_list, coin_name):
            ...
            enable_quickdrop = getattr(
                getattr(self.bot.coin_list, coin_name), "enable_quickdrop"
            )
            new_value = 1
            new_text = "enable"
    ...
    async def talkdrop(self, ctx, coin: str):
        ...
        if not hasattr(self.bot.coin_list, coin_name):
            ...
            enable_talkdrop = getattr(
                getattr(self.bot.coin_list, coin_name), "enable_talkdrop"
            )
            new_value = 1
            new_text = "enable"
    ...
    async def rewardtask(self, ctx, coin: str):
        ...
        if not hasattr(self.bot.coin_list, coin_name):
            ...
            enabe_reward_task = getattr(
                getattr(self.bot.coin_list, coin_name), "enabe_reward_task"
            )
            new_value = 1
            new_text = "enable"
        ...
        if hasattr(self.bot.coin_list, coin_name):
            ...
            try:
                ...
                if coin_name not in coin_list:
                    ...
                    new_value = 1
                    toggle = await self.enable_disable_coin(
                        coin_name, command, new_value
                    )
                    if toggle > 0:
    ...
    async def deposit(self, ctx, coin: str):
        ...
        if not hasattr(self.bot.coin_list, coin_name):
            ...
            enable_deposit = getattr(
                getattr(self.bot.coin_list, coin_name), "enable_deposit"
            )
            new_value = 1
            new_text = "enable"
            if enable_deposit == 1:
    ...
    async def create(self, ctx, token: str):
        if token.upper() not in [
            ...
            try:
                proxy = "http://{}:{}".format(
                    self.bot.config["api_helper"]["connect_ip"],
                    self.bot.config["api_helper"]["port_solana"],
                )
                create_addr = await self.utils.solana_create_address(
                    proxy + "/create_address", timeout=32
                )
                await ctx.reply(
                    f"{ctx.author.mention}, ```{str(create_addr)}```",
            ...
            try:
                proxy = "http://{}:{}".format(
                    self.bot.config["api_helper"]["connect_ip"],
                    self.bot.config["api_helper"]["port_tezos"],
                )
                create_addr = await self.utils.tezos_create_address(proxy)
                await ctx.reply(
    ...
    @commands.is_owner()
    @admin.command(usage="encrypt <expression>", description="Encrypt text.")
    async def encrypt(self, ctx, *, text):
        encrypt = encrypt_string(text)
        if encrypt:
            return await ctx.reply(
                f"```{encrypt}```", view=RowButtonRowCloseAnyMessage()
            )

    @commands.is_owner()
    @admin.command(usage="decrypt <expression>", description="Decrypt text.")
    async def decrypt(self, ctx, *, text):
        decrypt = decrypt_string(text)
        if decrypt:
            return await ctx.reply(
                f"```{decrypt}```", view=RowButtonRowCloseAnyMessage()
            )
