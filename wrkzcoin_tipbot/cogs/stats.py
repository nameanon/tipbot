...
import disnake
import store
from Bot import EMOJI_INFORMATION, SERVER_BOT, logchanbot
...
class Stats(commands.Cog):
    ...
    async def async_stats(self, ctx, coin: str = None):
        def simple_number(amount):
            ...
            if amount is None:
                return "0.0"
            amount_test = "{:,f}".format(
                float(("%f" % (amount)).rstrip("0").rstrip("."))
            )
            ...
            return (
                amount_str.rstrip("0").rstrip(".") if "." in amount_str else amount_str
            )
        ...
        try:
            ...
            await ctx.response.send_message(
                f"{EMOJI_INFORMATION} {ctx.author.mention}, failed to execute /stats message...",
                ephemeral=True,
            )
            return
        if coin:
            ...
            if not hasattr(self.bot.coin_list, coin_name):
                ...
                try:
                    ...
                    if get_tip_stats is not None:
                        embed.add_field(
                            name="Tip/DB Records: {:,.0f}".format(
                                get_tip_stats["numb_tip"]
                            ),
                        ...
                        embed.add_field(
                            ...
                            value=disnake.utils.format_dt(
                                get_tip_stats["last_tip"], style="R"
                            ),
                            inline=True,
                        )
                ...
                try:
                    ...
                    if height:
                        embed.add_field(
                            name="blockNumber",
                            value="`{:,.0f}` | [Explorer Link]({})".format(
                                height, explorer_link
                            ),
                            inline=False,
                        )
                except Exception:
            ...
            embed.add_field(
                name="Creator's Discord Name:", value="pluton#8888", inline=False
            )
            embed.add_field(
                name="Online",
                value="{:,.0f}".format(
                    sum(
                        ...
                        if m.status == disnake.Status.online
                    )
                ),
                inline=True,
            )
            embed.add_field(
                name="Users",
                value="{:,.0f}".format(
                    sum(1 for m in self.bot.get_all_members() if m.bot == False)
                ),
                inline=True,
            )
            embed.add_field(
                name="Bots",
                value="{:,.0f}".format(
                    sum(1 for m in self.bot.get_all_members() if m.bot == True)
                ),
                inline=True,
            )
            embed.add_field(
                name="Note:",
                value="Use `/stats coin` to check each coin's stats.",
                inline=False,
            )
            embed.set_footer(
                text="Made in Python!",
    ...
    @commands.slash_command(
        ...
        options=[
            Option(
                "token", "Enter a coin/ticker name", OptionType.string, required=False
            )
        ],
        description="Get some statistic and information.",
    )
    async def stats(self, inter: disnake.AppCmdInter, token: str = None) -> None:
        try:
