        ...
        try:
            ...
            async with store.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    VALUES (%s, %s, %s, %s, %s, %s) """
                    await cur.execute(
                        sql, (user_id, voter, directory, bot_id, type_vote, date_vote)
                    )
                    await conn.commit()
                    return True
        except Exception:
        ...
        async def handler_post(request):
            ...
            try:
                if request.body_exists:
                    ...
                    type_vote = "upvote"
                    voter = "{}#{}".format(
                        full_payload["username"], full_payload["discriminator"]
                    )
                    ...
                    if str(request.rel_url).startswith("/bot_vote/"):
                        ...
                        if (
                            ...
                            == self.bot.config["discordbotlist"]["auth"]
                        ):
                            try:
                                ...
                                check_last_vote = await self.check_last_bot_vote(
                                    user_vote, "discordbotlist", bot_id
                                )
                                if (
                            ...
                            if insert_vote:
                                ...
                                try:
                                    ...
                                    get_user_coin = await faucet.get_user_faucet_coin(
                                    ...
                                    if get_user_coin is not None:
                                        ...
                                        if coin_name is not None:
                                            ...
                                            height = (
                                                await self.wallet_api.get_block_height(
                        ...
                        return web.Response(
                            text="Thank you but not discordbotlist.com!"
                        )
            except KeyError:
