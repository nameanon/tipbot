...
class ModTagGuildAdd(disnake.ui.Modal):
    def __init__(self) -> None:
        ...
        super().__init__(
            title="Add a new tag to guild",
            custom_id="modal_add_tag",
        ...
        if tag_name == "":
            ...
            if len(list_tag) > 0:
                ...
                if tag_name.upper() in d:
                    await inter.response.send_message(
                        f"{inter.author.mention}, tag `{tag_name}` already exists here."
                    )
                    return
            # Let's add
            ...
            if addTag is None:
                await inter.response.send_message(
                    f"{inter.author.mention}, failed to add tag `{tag_name}`."
                )
            if addTag.upper() == tag_name.upper():
                await inter.response.send_message(
                    f"{inter.author.mention}, successfully added tag `{tag_name}`."
                )
            else:
                await inter.response.send_message(
                    f"{inter.author.mention}, failed to add tag `{tag_name}`."
            ...
            await inter.response.send_message(
                f"{inter.author.mention}, tag `{tag_name}` is not valid."
            )
...
class Tag(commands.Cog):
    ...
    @commands.slash_command(
        dm_permission=False, description="Manage or display tag(s)."
    )
    ...
    @tag.sub_command(usage="tag add", description="Add a tag to the guild.")
    ...
    @tag.sub_command(
        ...
        options=[Option("tag_name", "tag_name", OptionType.string, required=True)],
        description="Remove a tag from the guild.",
    )
    async def delete(self, ctx, tag_name: str):
        if re.match("^[a-zA-Z0-9]+(-[a-zA-Z0-9]+)*$", tag_name):
            tag = tag_name.upper()
            delTag = await self.tag_db.sql_tag_by_server_del(
                str(ctx.guild.id), tag.strip()
            )
            if delTag is None:
                await ctx.response.send_message(
                    f"{ctx.author.mention}, failed to delete tag `{tag_name}`."
                )
            elif delTag.upper() == tag.upper():
                await ctx.response.send_message(
                    f"{ctx.author.mention}, successfully deleted tag `{tag_name}`."
                )
            else:
                await ctx.response.send_message(
                    f"{ctx.author.mention}, failed to delete tag `{tag_name}`."
            ...
            await ctx.response.send_message(
                f"{ctx.author.mention}, tag `{tag_name}` is not valid."
            )
        try:
    ...
    async def delete_tag_name_autocomp(
    ...
    @tag.sub_command(
        ...
        options=[Option("tag_name", "tag_name", OptionType.string, required=False)],
        description="Show a saved tag.",
    )
    async def show(self, ctx, tag_name: str = None):
        if tag_name is None:
            ListTag = await self.tag_db.sql_tag_by_server(str(ctx.guild.id), None)
            if len(ListTag) > 0:
                tags = (", ".join([w["tag_id"] for w in ListTag])).lower()
                await ctx.response.send_message(
                    f"{ctx.author.mention}, available tag:```{tags}```"
                )
            else:
                await ctx.response.send_message(
                    f"{ctx.author.mention}, there is not any tag in this server."
                )
        else:
            TagIt = await self.tag_db.sql_tag_by_server(
                str(ctx.guild.id), tag_name.upper()
            )
            if TagIt:
                tagDesc = TagIt["tag_desc"].replace("\n", "\r\n")
                try:
                    await ctx.response.send_message(
                        f"{ctx.author.mention}, `{tag_name}`:\n{tagDesc}"
                    )
                except Exception:
                    traceback.print_exc(file=sys.stdout)
            else:
                await ctx.response.send_message(
                    f"{ctx.author.mention}, there is no tag `{tag_name}` in this server."
                )
        try:
            self.bot.commandings.append(
                (
    ...
    async def show_tag_name_autocomp(
        self, inter: disnake.CommandInteraction, string: str
    ):
        string = string.lower()
        list_tag = await self.tag_db.sql_tag_by_server(str(inter.guild.id), None)
