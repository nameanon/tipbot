                    ...
                    if len(decoded_data) > 0:
                        ...
                        for each_item in decoded_data:
                            ...
                            try:
                                if (
                                    len(each_item["id"]) > 0
                                    and len(each_item["symbol"]) > 0
                                    and len(each_item["name"]) > 0
                                    and each_item["id"].lower().strip()
                                    not in id_list_inserting
                                ):
                                    insert_list.append(
                                        (
                                            each_item["id"].lower().strip(),
                                            each_item["symbol"],
                                            each_item["name"],
                                        )
                                    )
                                    id_list_inserting.append(
                                        each_item["id"].lower().strip()
                                    )
                            except Exception:
                                traceback.print_exc(file=sys.stdout)
