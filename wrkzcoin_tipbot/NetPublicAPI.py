        ...
        try:
            ...
            async with self.pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if result:
                        result["sell_24h"] = (
                            result["sell_24h"] if result["sell_24h"] else 0
                        )
                        result["get_24h"] = (
                            result["get_24h"] if result["get_24h"] else 0
                        )
                        result["sell_7d"] = (
                            result["sell_7d"] if result["sell_7d"] else 0
                        )
                        result["get_7d"] = result["get_7d"] if result["get_7d"] else 0
                        result["sell_30d"] = (
                            result["sell_30d"] if result["sell_30d"] else 0
                        )
                        result["get_30d"] = (
                            result["get_30d"] if result["get_30d"] else 0
                        )
                        return {
                            "trade_24h": result["sell_24h"] + result["get_24h"],
...
async def show_all_ordered_books(market_pair: str):
    ...
    if len(market_pair) != 2 or market_pair is None:
        return {"error": "Invalid market pair!", "time": int(time.time())}
...
async def list_all_available_markets():
        ...
        if each_market_coin and len(each_market_coin) > 0:
            for each_item in each_market_coin:
                ...
                if pair_items not in pairs:
                    ...
                    get_market_buy_list = (
                        await app.db.sql_get_open_order_by_alluser_by_coins(
                            sell_coin, buy_coin, "OPEN", "ASC", 1000
                        )
                    )
                    get_market_sell_list = (
                        await app.db.sql_get_open_order_by_alluser_by_coins(
                            buy_coin, sell_coin, "OPEN", "DESC", 1000
                        )
                    )
                    ...
                    if get_market_buy_list and len(get_market_buy_list) > 0:
                        for each_buy in get_market_buy_list:
                            rate = "{:.8f}".format(
                                each_buy["amount_sell"] / (each_buy["amount_get"])
                            )
                            amount = "{:.8f}".format(each_buy["amount_sell"])
                            buy_list.append(
                                {
                    ...
                    if get_market_sell_list and len(get_market_sell_list) > 0:
                        for each_sell in get_market_sell_list:
                            rate = "{:.8f}".format(
                                each_sell["amount_sell"] / each_sell["amount_get"]
                            )
                            amount = "{:.8f}".format(each_sell["amount_get"])
                            sell_list.append(
                                {
    ...
    return {"success": True, "market_list": market_list, "time": int(time.time())}
...
async def check_information_by_ticker(coin_name: str):
    ...
    market_list = (
        [f"{each_item['coin_sell']}-{each_item['coin_get']}" for each_item in markets]
...
async def list_all_coin_information():
    ...
    if len(get_all_coins) <= 0:
        return {"error": "No data!", "time": int(time.time())}
    all_coins = []
    ...
    return {"data": all_coins, "time": int(time.time())}
