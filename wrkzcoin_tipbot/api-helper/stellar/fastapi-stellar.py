...
async def get_xlm_transactions(endpoint: str, account_addr: str):
    async with ServerAsync(horizon_url=endpoint, client=AiohttpClient()) as server:
        # get a list of transactions submitted by a particular account
        transactions = (
            await server.transactions()
            .for_account(account_addr)
            .order(desc=True)
            .limit(50)
            .call()
        )
...
async def check_xlm_asset(url: str, asset_name: str, issuer: str, to_address: str):
    found = False
    try:
        ...
        try:
            ...
            if memo is not None:
                transaction = (
                    ...
                    .add_text_memo(memo)
                    .append_payment_op(
                        to_address, asset_sending, str(truncate(amount, 6))
                    )
                    .set_timeout(30)
                    .build()
                )
...
app = FastAPI(title="TipBotv2 FastAPI Stellar", version="0.1", docs_url="/dokument")
app.config = config
...
async def verify_asset(item: VerifyAsset):
    try:
        checking = await check_xlm_asset(
            item.endpoint, item.asset_name, item.issuer, item.address
        )
        return {"result": checking, "timestamp": int(time.time())}
    except Exception:
        traceback.print_exc(file=sys.stdout)
    return {"error": "Error!", "timestamp": int(time.time())}


@app.post("/send_transaction")
async def send_transaction(item: SendToken):
    try:
        ...
        if sending:
            return {"result": sending, "timestamp": int(time.time())}
        else:
            return {
...
async def get_balance_xlm(item: BalanceXLMData):
    balance = 0.0
    try:
        async with ServerAsync(
            ...
            if "balances" in account and len(account["balances"]) > 0:
                for each_balance in account["balances"]:
                    if (
                        item.coin_name == "XLM"
                        and each_balance["asset_type"] == "native"
                    ):
                        ...
                        "asset_code" in each_balance
                        and "asset_issuer" in each_balance
                        and each_balance["asset_code"] == asset_code
                        and issuer == each_balance["asset_issuer"]
                    ):
                        balance = float(each_balance["balance"])
                        break
        ...
        traceback.print_exc(file=sys.stdout)
    return {"balance": balance, "timestamp": int(time.time())}


@app.post("/get_transactions")
async def get_transactions(item: AddressData):
    try:
        list_tx = await get_xlm_transactions(item.endpoint, item.address)
        return {"result": list_tx, "timestamp": int(time.time())}
    except Exception:
        traceback.print_exc(file=sys.stdout)
    return {"error": "Error!", "timestamp": int(time.time())}
...
@app.post("/get_status")
async def get_status(item: EndpointData):
    try:
        headers = {"Content-Type": "application/json"}
        async with aiohttp.ClientSession() as session:
            async with session.get(
                item.endpoint, headers=headers, timeout=item.timeout
            ) as response:
...
async def parse_transaction(item: ParseTxData):
    try:
        with ThreadPoolExecutor(max_workers=1) as executor:
            ...
            if hasattr(transaction_envelope, "transaction"):
                ...
                for Payment in transaction_envelope.transaction.operations:
                    try:
                        ...
                        if asset_type == "native":
                            coin_name = "XLM"
                        elif hasattr(Payment.asset, "code") and hasattr(
                            ...
                            for each_coin in item.coin_name_list:
                                ...
                                if (
                                    asset_code == item.coin_list[each_coin]["header"]
                                    and asset_issuer
                                    == item.coin_list[each_coin]["contract"]
                                ):
                ...
                if coin_name:
                    result = {
                        "coin_name": coin_name,
                        "fee": float(transaction_envelope.transaction.fee)
                        / 10000000,  # atomic,
                        "amount": amount,
                    }
                    return {"result": result, "timestamp": int(time.time())}
    except Exception:
        traceback.print_exc(file=sys.stdout)
    return {"error": "Error!", "timestamp": int(time.time())}
