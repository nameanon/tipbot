        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                if rejoin:
                    ...
                    """
                    await cur.execute(
                        sql, (server_id, servername[:28], prefix, default_coin)
                    )
                await conn.commit()
    except Exception:
        traceback.print_exc(file=sys.stdout)
...
async def sql_user_balance_single(
    ...
    20 * 60 if top_block is None else top_block - confirmed_depth
    ...
    try:
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                VALUES (%s, %s, %s, %s) """
                await cur.execute(
                    sql, (message_id, guild_id, owner_id, int(time.time()))
                )
                await conn.commit()
                return True
    except Exception:
    ...
    try:
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                WHERE `is_maintenance`=0 AND `enable`=1 """
                    + sql_coin_type
                )
                await cur.execute(
                    sql,
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                if result and len(result) > 0:
                    return {
                        "txHash_unique": [
                            item["contract_blockNumber_Tx_from_to_uniq"]
                            for item in result
                        ]
                    }
    except Exception:
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                if net_name == "TRX":
                    ...
                    LIMIT 1 """
                    await cur.execute(
                        sql, (new_height, net_name, new_height, coin_name)
                    )
                else:
                    ...
                    if response.status == 200:
                        ...
                        if decoded_data and "result" in decoded_data:
                            return (
                                ...
                                else int(decoded_data["result"], 16)
                            )
        except aiohttp.client_exceptions.ServerDisconnectedError:
            print(
    ...
    try:
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                """
                await cur.execute(
                    sql, (user_id, address, contract, user_server, network)
                )
                result = await cur.fetchone()
                if result:
                    return True
...
async def sql_check_pending_move_deposit_erc20(
    url: str, net_name: str, deposit_confirm_depth: int, block_timeout: int = 64
):
    global pool
...
async def trx_check_minimum_deposit(
    ...
    contract: str,
    coin_decimal: int,
    min_move_deposit: float,
    min_gas_tx: float,
    fee_limit_trx: float,
    gas_ticker: str,
    move_gas_amount: float,
    chainId: str,
    real_deposit_fee: float,
    time_lap: int = 0,
):
            ...
            try:
                ...
                async with pool.acquire() as conn:
                    async with conn.cursor() as cur:
                        ...
                        """
                        await cur.execute(
                            sql, (each_address["balance_wallet_address"], lap)
                        )
                        result = await cur.fetchone()
                        if (
                            result is not None
                            and "failed" in result
                            and result["failed"] >= num_failed_limit
                        ):
                ...
                if token_name == "TRX":
                    ...
                    try:
                        ...
                        TronClient = AsyncTron(
                            provider=AsyncHTTPProvider(url, client=_http_client)
                        )
                        txb = (
                        ...
                        txn = await txb.build()
                        priv_key = PrivateKey(
                            bytes.fromhex(decrypt_string(each_address["private_key"]))
                        )
                        ...
                        print(
                            "httpx.RemoteProtocolError: url {} for token {}".format(
                                url, coin
                            )
                        )
                    except httpx.ReadTimeout:
                        print(
                            "httpx.ReadTimeout: url {} for token {}".format(url, coin)
                        )
                    except httpx.ConnectTimeout:
                        print(
                            "httpx.ConnectTimeout: url {} for token {}".format(
                                url, coin
                            )
                        )
                    except Exception:
                    ...
                    if type_coin == "TRC-20":
                        try:
                            ...
                            try:
                                ...
                                if gas_balance < min_gas_tx:
                                    txb_gas = TronClient.trx.transfer(
                                        config["trc"]["MainAddress"],
                                        each_address["balance_wallet_address"],
                                        int(move_gas_amount * 10**6),
                                    ).fee_limit(int(fee_limit_trx * 10**6))
                                    txn_gas = await txb_gas.build()
                                    priv_key_gas = PrivateKey(
                                        bytes.fromhex(config["trc"]["MainAddress_key"])
                                    )
                                    await txn_gas.sign(priv_key_gas).broadcast()
                                    await asyncio.sleep(0.5)
                            except Exception:
                                traceback.print_exc(file=sys.stdout)
                            ...
                            priv_key = PrivateKey(
                                bytes.fromhex(
                                    decrypt_string(each_address["private_key"])
                                )
                            )
                            txn_ret = await txn.sign(priv_key).broadcast()
                            in_block = None
                            ...
                            print(
                                "httpx.ReadTimeout: url {} for token {}".format(
                                    url, coin
                                )
                            )
                        except httpx.ConnectTimeout:
                        ...
                        try:
                            ...
                            traceback.print_exc(file=sys.stdout)
        msg_deposit += (
            "TOKEN {}: Total deposit address: {}: Below min.: {} Above min. {}".format(
                token_name,
            ...
            if type_coin == "TRC-20":
                try:
                    ...
                    if token_name.upper() == SYM.upper():
                        precision = await cntr.functions.decimals()
                        balance = (
                            await cntr.functions.balanceOf(address) / 10**precision
                        )
                    else:
                        await logchanbot(
                            "Mis-match SYM vs TOKEN NAME: {} vs {}".format(
                                SYM, token_name
                            )
                        )
                ...
                try:
                    ...
                    balance = (
                        await TronClient.get_account_asset_balance(
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            async with conn.cursor() as cur:
                if called_Update == 0:
                    sql = (
                        """ SELECT `user_id`, `user_id_trc20`, `balance_wallet_address`, 
                    `hex_address`, `private_key`, `user_server` FROM trc20_user 
                    """
                        + extra_str
                        + """ ) OR `is_discord_guild`=1 """
                    )
                    ...
                    """
                        + extra_str
                        + """ AND `called_Update`>%s) OR `is_discord_guild`=1 """
                    )
                    await cur.execute(sql, (lap))
                    result = await cur.fetchall()
                    if result:
    ...
    try:
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                result = await cur.fetchall()
                if result and len(result) > 0:
                    return [each["addresses"] for each in result]
    except Exception:
        traceback.print_exc(file=sys.stdout)
    return []
...
async def sql_user_balance_mv_multple_amount(
    user_dict_tip, tiptype: str, user_server: str
):
            ...
            async with conn.cursor() as cur:
                ...
                if result:
                    ...
                    if res_used_address and len(res_used_address) > 0:
                        used_addresses = [
                            each["balance_wallet_address"] for each in res_used_address
                        ]
                    if len(used_addresses) == 0:
                        return {
                            "wallet_name": result["wallet_name"],
                            "addresses": wallet_name_address,
                            "address_pool_gap": result["address_pool_gap"],
                            "remaining": len(wallet_name_address),
                        }
                    else:
                        remaining = []
            ...
            async with conn.cursor() as cur:
                ...
                await cur.execute(
                    ...
                    (
                        ...
                        collector_id,
                        collector_name,
                        "{}-{}-{}".format(message_id, from_userid, collector_id),
                        int(time.time()),
                    ),
                )
                await conn.commit()
...
# Trade
async def sql_count_open_order_by_sellerid(
    user_id: str, user_server: str, status: str = None
):
    global pool
    user_server = user_server.upper()
    if status is None:
...
async def sql_store_openorder(
    coin_sell: str,
    coin_decimal_sell: str,
    amount_sell: float,
    amount_sell_after_fee: float,
    userid_sell: str,
    coin_get: str,
    coin_decimal_buy: str,
    amount_get: float,
    amount_get_after_fee: float,
    sell_div_get: float,
    ...
    if (
        amount_sell == 0
        or amount_sell_after_fee == 0
        or amount_get == 0
        or amount_get_after_fee == 0
        or sell_div_get == 0
    ):
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                try:
                    ...
                    sql = (
                        """ SELECT * FROM `open_order` WHERE `order_id` = %s LIMIT 1 """
                    )
                    ...
                    if result is not None:
                        fee_user = "TRADE"
                        fee_sell = (
                            result["amount_sell"] - result["amount_sell_after_fee"]
                        )
                        ...
                        list_tx.append(
                            (
                                ...
                                "TRADE",
                                "TRADE",
                                result["amount_sell_after_fee"],
                                0.0,
                                result["coin_sell_decimal"],
                                "TRADE",
                                currentTs,
                                sell_user_server,
                                result["userid_sell"],
                        ...
                        list_tx.append(
                            (
                                ...
                                "TRADE",
                                "TRADE",
                                result["amount_get_after_fee"],
                                0.0,
                                result["coin_get_decimal"],
                                "TRADE",
                                currentTs,
                                sell_user_server,
                                result["userid_get"],
...
async def sql_get_open_order_by_sellerid(
    userid_sell: str, coin: str, status: str = "OPEN"
):
    global pool
    coin_name = coin.upper()
    try:
...
async def sql_faucet_checkuser(
    user_id: str, user_server: str = "DISCORD", type_a: str = "TAKE"
):
    global pool
    user_server = user_server.upper()
    if user_server not in ["DISCORD", "TELEGRAM", "REDDIT"]:
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                if list_roach:
                    ...
                    sql = (
                        """
                    SELECT * FROM discord_faucet WHERE claimed_user IN """
                        + roach_sql
                        + """ 
...
async def erc20_if_approved(
    url: str, contract: str, sender_address: str, operator_address: str
):
    try:
        ...
        _spender = w3.toChecksumAddress(operator_address)
        spendable_amount = cnt.functions.allowance(
            w3.toChecksumAddress(sender_address), _spender
        ).call()
...
async def erc20_transfer_token_to_operator(
    ...
    try:
        ...
        allowance = unicorns.functions.allowance(
            w3.toChecksumAddress(sender_address), w3.toChecksumAddress(operator_address)
        ).call()
        if allowance and allowance < atomic_amount:
            print(
                "Contract: {} operator: {} allowance for address: {} is less than amount {} < {}.".format(
                    contract, operator_address, sender_address, allowance, atomic_amount
                )
            )
            return None
        ...
        acct = Account.from_mnemonic(mnemonic=operator_seed)
...
async def insert_partydrop_create(
    ...
    init_amount: float,
    real_init_amount_usd: float,
    real_init_amount_usd_text: str,
    unit_price_usd: float,
    token_decimal: int,
    partydrop_time: int,
    status: str = "ONGOING",
            ...
            async with conn.cursor() as cur:
                ...
                await cur.execute(
                    ...
                    (
                        ...
                        minimum_amount,
                        init_amount,
                        real_init_amount_usd,
                        real_init_amount_usd_text,
                        unit_price_usd,
                        token_decimal,
                        int(time.time()),
                        partydrop_time,
    ...
    try:
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                SET `status`=%s WHERE `message_id`=%s;
                """
                await cur.execute(sql, (to_status, message_id, to_status, message_id))
                await conn.commit()
                return True
    except Exception:
        traceback.print_exc(file=sys.stdout)
    ...
    try:
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute(
                    sql,
                    (status, collected_id, collected_name, collected_date, message_id),
                )
                await conn.commit()
                return True
    except Exception:
...
async def insert_talkdrop_create(
    ...
    guild_id: str,
    channel_id: str,
    talked_in_channel: str,
    talked_from_when: int,
    minimum_message: int,
    real_amount: float,
    real_amount_usd: float,
    real_amount_usd_text: str,
...
async def add_talkdrop(
    message_id: str, from_userid: str, collector_id: str, collector_name: float
):
    global pool
    try:
        await openConnection()
            ...
            async with conn.cursor() as cur:
                if what.lower() == "withdraw":
                    ...
                    if coin_family in ["TRTL-API", "TRTL-SERVICE", "BCN", "XMR"]:
                        ...
                        ORDER BY b.time_insert DESC LIMIT """ + str(
                            limit
                        )
                        await cur.execute(sql, (user_id, user_server, coin_name))
                        result = await cur.fetchall()
                        ...
                        ORDER BY `time_insert` DESC LIMIT """ + str(
                            limit
                        )
                        await cur.execute(
                            sql, (user_id, user_server, coin_name, "CONFIRMED")
                        )
                        result = await cur.fetchall()
                        if result:
                            return result
