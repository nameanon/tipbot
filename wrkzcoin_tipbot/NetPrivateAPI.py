...
async def handle_get_all(request):
    ...
    if (
        "authorization-user" in request.headers
        and "authorization-key" in request.headers
    ):
    ...
    if uri.startswith("/get_balance/"):
        ...
        if wallet is None:
            if COIN_NAME in ENABLE_COIN_ERC:
                w = create_eth_wallet()
                await store.sql_register_user(userid, COIN_NAME, "DISCORD", 0, w)
            elif COIN_NAME in ENABLE_COIN_TRC:
                result = await store.create_address_trx()
                await store.sql_register_user(userid, COIN_NAME, "DISCORD", 0, result)
            else:
                await store.sql_register_user(userid, COIN_NAME, "DISCORD", 0)
            wallet = await store.sql_get_userwallet(userid, COIN_NAME)
        ...
        if COIN_NAME in ENABLE_COIN_DOGE + ENABLE_COIN_ERC + ENABLE_COIN_TRC:
            ...
            actual_balance = round(
                actual_balance / get_decimal(COIN_NAME), 6
        ...
        # add to api call
        try:
            await store.api_trade_store(userid, uri)
        except Exception:
            traceback.print_exc(file=sys.stdout)
        ...
        if is_maintenance_coin(COIN_NAME):
            result = {
                "success": False,
                COIN_NAME: {"maintenance": bool(is_maintenance_coin(COIN_NAME))},
            }
        ...
        if coin_family in ["TRTL", "BCN"]:
            ...
            if wallet is None:
                ...
                wallet = await store.sql_register_user(
                    userid, COIN_NAME, "DISCORD", 0, w
                )
                wallet = await store.sql_get_userwallet(userid, COIN_NAME)
            ...
            if wallet is None:
                result = await store.create_address_trx()
                wallet = await store.sql_register_user(
                    userid, COIN_NAME, "DISCORD", 0, result
                )
                wallet = await store.sql_get_userwallet(userid, COIN_NAME)
        else:
...
async def handle_post_all(request):
    ...
    if check_auth is None:
        try:
            print(
                "Denied client ip: {} / {} / requested {}".format(
                    client_ip,
                    request.headers.get("cf-ipcountry", None),
                    request.rel_url,
                )
            )
        except Exception:
    ...
    if CF:
        try:
            print(
                "Client ip: {} / {} / Browser: {} requested {}".format(
                    client_ip,
                    request.headers.get("cf-ipcountry", None),
    ...
    # add to api call
    try:
        await store.api_trade_store(userid, uri, json.dumps(data))
    except Exception:
        traceback.print_exc(file=sys.stdout)
    ...
    if uri.startswith("/sell"):
        try:
            await logchanbot(
                "{}: user: {} called /sell:```{}```".format(
                    SERVER_BOT, userid, json.dumps(data) if data else ""
                )
            )
        except Exception:
            traceback.print_exc(file=sys.stdout)
        ...
        if (
            "coin_sell" in data
            and "coin_get" in data
            and "amount_sell" in data
            and "amount_get" in data
        ):
            if (
                len(data["coin_sell"]) == 0
                or len(data["coin_get"]) == 0
                or len(data["amount_sell"]) == 0
                or len(data["amount_get"]) == 0
            ):
                result = {
                    "success": False,
                    "result": {"error": "Input data must not be empty."},
                }
                return web.Response(
                    text=json.dumps(result).replace("\\", ""), status=500
                )
            if (
                data["coin_sell"].isalnum() == False
                or data["coin_get"].isalnum() == False
            ):
                result = {
                    "success": False,
                    "result": {"error": "Invalid coin name for sell or buy."},
                }
                return web.Response(
                    text=json.dumps(result).replace("\\", ""), status=500
                )
            if (
                check_float(data["amount_sell"].replace(",", "")) == False
                and check_float(data["amount_get"].replace(",", "")) == False
            ):
                result = {
                    "success": False,
                    "result": {"error": "Invalid amount for sell or buy."},
                }
                return web.Response(
                    text=json.dumps(result).replace("\\", ""), status=500
                )
            ...
            if sell_ticker not in ENABLE_TRADE_COIN:
                result = {
                    "success": False,
                    "result": {
                        "error": "{} is not in tradable coin.".format(sell_ticker)
                    },
                }
                return web.Response(
                    text=json.dumps(result).replace("\\", ""), status=500
            ...
            if buy_ticker not in ENABLE_TRADE_COIN:
                result = {
                    "success": False,
                    "result": {
                        "error": "{} is not in tradable coin.".format(buy_ticker)
                    },
                }
                return web.Response(
                    text=json.dumps(result).replace("\\", ""), status=500
            ...
            try:
                ...
                result = {
                    "success": False,
                    "result": {"error": "Invalid trade amount(s)."},
                }
                return web.Response(
                    text=json.dumps(result).replace("\\", ""), status=500
                )
            ...
            if not is_tradeable_coin(sell_ticker):
                result = {
                    "success": False,
                    "result": {"error": "{} is not tradable.".format(sell_ticker)},
                }
                return web.Response(
                    text=json.dumps(result).replace("\\", ""), status=500
                )
            if not is_tradeable_coin(buy_ticker):
                result = {
                    "success": False,
                    "result": {"error": "{} is not tradable.".format(buy_ticker)},
                }
                return web.Response(
                    text=json.dumps(result).replace("\\", ""), status=500
                )
            ...
            if user_count_order >= config.trade.Max_Open_Order:
                result = {
                    "success": False,
                    "result": {"error": "Reached max opened orders."},
                }
                return web.Response(
                    text=json.dumps(result).replace("\\", ""), status=500
                )
            ...
            real_amount_sell = (
                int(sell_amount * get_decimal(sell_ticker))
                if coin_family_sell in ["BCN", "XMR", "TRTL", "NANO", "XCH"]
                else float(sell_amount)
            )
            ...
            if real_amount_sell < get_min_sell(sell_ticker, sell_token_info):
                result = {
                    "success": False,
                    "result": {
                        "error": "Below minimum trade {} {}.".format(
                            num_format_coin(
                                get_min_sell(sell_ticker, sell_token_info), sell_ticker
                            ),
                            sell_ticker,
                        )
                    },
            ...
            if real_amount_sell > get_max_sell(sell_ticker, sell_token_info):
                result = {
                    "success": False,
                    "result": {
                        "error": "Above maximum trade {} {}.".format(
                            num_format_coin(
                                get_max_sell(sell_ticker, sell_token_info), sell_ticker
                            ),
                            sell_ticker,
                        )
                    },
            ...
            real_amount_buy = (
                int(buy_amount * get_decimal(buy_ticker))
                if coin_family_buy in ["BCN", "XMR", "TRTL", "NANO", "XCH"]
                else float(buy_amount)
            )
            ...
            if real_amount_buy < get_min_sell(buy_ticker, buy_token_info):
                result = {
                    ...
                    "result": {
                        "error": "Below minimum trade {} {}.".format(
                            num_format_coin(
                                get_min_sell(buy_ticker, buy_token_info), buy_ticker
                            ),
                            sell_ticker,
                        )
                    },
            ...
            if real_amount_buy > get_max_sell(buy_ticker, buy_token_info):
                result = {
                    ...
                    "result": {
                        "error": "Above maximum trade {} {}.".format(
                            num_format_coin(
                                get_max_sell(buy_ticker, buy_token_info), buy_ticker
                            ),
                            sell_ticker,
                        )
                    },
            ...
            if not is_maintenance_coin(sell_ticker):
                ...
                if sell_ticker in ENABLE_COIN_DOGE + ENABLE_COIN_ERC + ENABLE_COIN_TRC:
                    ...
                    actual_balance = int(xfer_in) + int(userdata_balance["Adjust"])
                    actual_balance = round(
                        actual_balance / get_decimal(sell_ticker), 6
                    ) * get_decimal(sell_ticker)
                else:
                    actual_balance = int(xfer_in) + int(userdata_balance["Adjust"])
                ...
                if actual_balance < real_amount_sell:
                    result = {
                        "success": False,
                        "result": {
                            "error": "Not sufficient balance. Having {} {}.".format(
                                num_format_coin(actual_balance, sell_ticker),
                                sell_ticker,
                            )
                        },
                    }
                ...
                if (sell_amount / buy_amount) < MIN_TRADE_RATIO or (
        ...
        if "ref_number" in data:
            ...
            if get_order_num:
                # check if own order
                if get_order_num["sell_user_server"] == "DISCORD" and int(
                    userid
                ) == int(get_order_num["userid_sell"]):
                    result = {
                        "success": False,
                        "result": {"error": "You can not buy your own order."},
                    }
                    return web.Response(
                        text=json.dumps(result).replace("\\", ""), status=400
                    )
                else:
                    # check if sufficient balance
                    wallet = await store.sql_get_userwallet(
                        userid, get_order_num["coin_get"], "DISCORD"
                    )
                    if wallet is None:
                        if get_order_num["coin_get"] in ENABLE_COIN_ERC:
                            w = create_eth_wallet()
                            await store.sql_register_user(
                                userid, get_order_num["coin_get"], "DISCORD", 0, w
                            )
                        elif get_order_num["coin_get"] in ENABLE_COIN_TRC:
                            result = await store.create_address_trx()
                            await store.sql_register_user(
                                userid, get_order_num["coin_get"], "DISCORD", 0, result
                            )
                        else:
                            await store.sql_register_user(
                                userid, get_order_num["coin_get"], "DISCORD", 0
                            )
                        wallet = await store.sql_get_userwallet(
                            userid, get_order_num["coin_get"], "DISCORD"
                        )
                    if wallet:
                        userdata_balance = await store.sql_user_balance(
                            userid, get_order_num["coin_get"], "DISCORD"
                        )
                        xfer_in = 0
                        if (
                            get_order_num["coin_get"]
                            not in ENABLE_COIN_ERC + ENABLE_COIN_TRC
                        ):
                            xfer_in = await store.sql_user_balance_get_xfer_in(
                                userid, get_order_num["coin_get"]
                            )
                        if (
                            get_order_num["coin_get"]
                            in ENABLE_COIN_DOGE + ENABLE_COIN_ERC + ENABLE_COIN_TRC
                        ):
                            actual_balance = float(xfer_in) + float(
                                userdata_balance["Adjust"]
                            )
                        elif get_order_num["coin_get"] in ENABLE_COIN_NANO:
                            actual_balance = int(xfer_in) + int(
                                userdata_balance["Adjust"]
                            )
                            actual_balance = round(
                                actual_balance / get_decimal(get_order_num["coin_get"]),
                                6,
                            ) * get_decimal(get_order_num["coin_get"])
                        else:
                            actual_balance = int(xfer_in) + int(
                                userdata_balance["Adjust"]
                            )
                    ...
                    if actual_balance < get_order_num["amount_get_after_fee"]:
                        result = {
                            ...
                            "result": {
                                "error": "Not sufficient balance. Having {} {}.".format(
                                    num_format_coin(
                                        actual_balance, get_order_num["coin_get"]
                                    ),
                                    get_order_num["coin_get"],
                                )
                            },
                        ...
                        if match_order:
                            # trade webhook
                            success_msg = "{} Order completed! Get: {} {} From selling: {} {}".format(
                                ref_number,
                                num_format_coin(
                            ...
                            return web.Response(
                                text=json.dumps(result).replace("\\", ""), status=200
                            )
                        else:
                            return await respond_internal_error()
            else:
                result = {
                    "success": False,
                    "message": "Order does not exist or already completed.",
                }
                return web.Response(
                    text=json.dumps(result).replace("\\", ""), status=400
                )
        ...
        if "ref_number" in data:
            ...
            if ref_number.isnumeric():
                ...
                if cancelled == False:
                    result = {
                        "success": False,
                        "message": f"You do not have sell #{ref_number}.",
                    }
                    return web.Response(
                        text=json.dumps(result).replace("\\", ""), status=400
                    )
                    ...
                    result = {
                        "success": True,
                        "message": f"You cancelled #{ref_number}.",
                    }
                    return web.Response(
                        text=json.dumps(result).replace("\\", ""), status=400
                    )
...
async def sell_process(
    ...
    if order_add:
        ...
        result = {
            "success": False,
            "result": {"error": "Internal error during sell_process."},
        }
        return web.Response(text=json.dumps(result).replace("\\", ""), status=500)
        # response
