            ...
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if result:
                        ...
                        get_mentions = api_mentions_timeline(
                            count=200, since_id=result["twitter_id"]
                        )
                    else:
                    ...
                    if len(get_mentions) > 0:
                        ...
                        for each_rec in get_mentions:
                            int_timestamp = int(
                                datetime.strptime(
                                    each_rec["created_at"], "%a %b %d %H:%M:%S +0000 %Y"
                                ).timestamp()
                            )
                            user_mentions = None
                            if len(each_rec["entities"]["user_mentions"]) > 0:
                                user_mentions = json.dumps(
                                    each_rec["entities"]["user_mentions"]
                                )
                            data_rows.append(
                                (
                                    each_rec["id"],
                        ...
                        if data_rows:
                            ...
                            await conn.commit()
                            if cur.rowcount > 0:
                                msg = "[TWITTER] mentions_timeline - Inserted {} new records...".format(
                                    cur.rowcount
                                )
                                logchanbot(msg)
                                print(msg)
                        ...
                        i += 1
                        if i > 0 and i % 50 == 0:
                            msg = "[TWITTER] - mentions_timeline no new records. Sleep {}s".format(
                                sleep_no_records
                            )
                            logchanbot(msg)
                            print(msg)
