...
def getcontours(IM, sc=2):
    ...
    for i in range(0, len(contours)):
        for j in range(0, len(contours[i])):
            contours[i][j] = int(
                contours[i][j][0] + 10 * noise(i * 0.5, j * 0.1, 1)
            ), int(contours[i][j][1] + 10 * noise(i * 0.5, j * 0.1, 2))
...
def hatch(IM, sc=16):
    ...
    for i in range(0, len(lines)):
        for j in range(0, len(lines[i])):
            lines[i][j] = (
                int(lines[i][j][0] + sc * noise(i * 0.5, j * 0.1, 1)),
                int(lines[i][j][1] + sc * noise(i * 0.5, j * 0.1, 2)) - j,
            )
    return lines
