...
from Bot import logchanbot, text_to_num, truncate
from cogs.cexswap import (
    cexswap_count_api_usage,
    cexswap_estimate,
    cexswap_find_possible_trade,
    cexswap_get_coin_setting,
    cexswap_get_list_enable_pair_list,
    cexswap_get_pool_details,
    cexswap_get_pools,
    find_user_by_apikey,
    get_cexswap_get_coin_sell_logs,
)
from cogs.utils import get_all_coin_names, num_format_coin
...
class BackgroundRunner:
    ...
    async def fetch_paprika_price(self):
        load_config()
        while True:
            try:
...
async def get_paprika_price(token: str, request: Request):
    """
...
async def get_coingecko_price(token: str, request: Request):
    """
    token: coin name or token name. Example: BTC, LTC, DOGE, etc
    """
    if config["cexswap_api"]["api_enable"] != 1:
...
async def get_summary_detail(slug: str, request: Request):
    """
...
async def get_summary(request: Request):
    ...
    get_coin_vol = await get_cexswap_get_coin_sell_logs(
        coin_name=None, user_id=None, from_time=int(time.time()) - 1 * 24 * 3600
    )
    list_volume_1d = {}
    if len(get_coin_vol) > 0:
    ...
    get_coin_vol = await get_cexswap_get_coin_sell_logs(
        coin_name=None, user_id=None, from_time=int(time.time()) - 7 * 24 * 3600
    )
    list_volume_7d = {}
    if len(get_coin_vol) > 0:
        for v in get_coin_vol:
            ...
            list_markets[i["pairs"]] = {
                "rate": {
                    i["ticker_1_name"]: truncate(
                        i["amount_ticker_2"] / i["amount_ticker_1"], 10
                    ),
                    i["ticker_2_name"]: truncate(
                        i["amount_ticker_1"] / i["amount_ticker_2"], 10
                    ),
                },
                "liquidity": {
    ...
    if len(get_pools) > 0:
        ...
        try:
            set_cache_kv(
                app, config["kv_db"]["prefix_cexswap"], key="summary", value=data
            )
        except Exception:
            traceback.print_exc(file=sys.stdout)
        print(
...
async def get_withdraw_list(request: Request):
    """
