...
async def update_reply(
    message_id: str, replied_text: str, replied_json_dump: str, replied_date: int
):
    global pool
    try:
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                """
                await cur.execute(
                    sql, (replied_text, replied_json_dump, replied_date, message_id)
                )
                await conn.commit()
                return cur.rowcount
    except Exception:
            ...
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if result and len(result) > 0:
                        if len(result) > 0:
                            msg = "[TWITTER] - send_direct_message has {} messages to respond.".format(
                                len(result)
                            )
                            logchanbot(msg)
                        for each_msg in result:
                            try:
                                send_msg = api_send_direct_message(
                                    int(each_msg["sender_id"]),
                                    text=each_msg["draft_response_text"],
                                )
                                if send_msg:
                                    await update_reply(
                                        each_msg["message_id"],
                                        send_msg["message_create"]["message_data"][
                                            "text"
                                        ],
                                        json.dumps(send_msg),
